...
function render() { const pages = { home: homePage, projects: projectsPage, project: projectPage, dataset: datasetPage, feature: featurePage, experiments: experimentsPage, 'model-select': modelSelectPage, 'smart-selection-config': smartSelectionConfigPageV103, 'smart-selection-result': smartSelectionResultPageV103, experiment: experimentPage, tuning: tuningPage, report: reportPage, models: modelsPage, api: apiPage }; app.innerHTML = (pages[state.page] || homePage)(); bind(); }
// V10.3.0：避免早期 section 的 render() 在后续函数尚未初始化时渲染智能选型页面
const safeRenderV103 = render;
const earlyPagesV103 = new Set(['home', 'projects', 'project', 'dataset', 'feature', 'experiments', 'model-select', 'experiment', 'tuning', 'report', 'models', 'api']);
render = function () { if (earlyPagesV103.has(state.page)) return safeRenderV103(); };
...
correlationPanel = function () {
  ...
  return `<section class="correlation-panel"><div class="panel-head"><div><h2>特征相关性热力图</h2><p>仅使用训练集数值特征计算。</p></div><div class="correlation-actions"><label>预警阈值<input data-correlation-draft type="number" step="0.05" value="${state.featureThresholds.correlation}"></label><span data-correlation-unsaved hidden>尚未应用</span>${button('确定修改', 'confirm-correlation-threshold')}${button('自动处理高相关特征', 'preview-correlation')}</div></div><div class="heat-legend"><span>低相关</span><i></i><span>高相关</span></div><div class="correlation-grid" style="--count:${columns.length + 1}"><i></i>${columns.map(column => `<b>${esc(column.name)}</b>`).join('')}${columns.map(row => `<b>${esc(row.name)}</b>${columns.map(column => { const score = scoreFor(row, column); const lightness = 97 - Math.round(Math.abs(score) * 48); return `<span style="background:hsl(213 88% ${lightness}%);color:${Math.abs(score) > .62 ? '#fff' : '#17345f'}" class="${row !== column && Math.abs(score) >= state.featureThresholds.correlation ? 'warning' : ''}">${score.toFixed(2)}</span>`; }).join('')}`).join('')}</div><small>达到已确认阈值时预警；修改输入框后需点击“确定修改”。</small></section>`;
};
...
function routeHashV910() {
  ...
  if (state.page === 'smart-selection-config' && owner && set) return `#/smart-selection-config/${routeIdV910(owner.id)}/${routeIdV910(set.id)}`;
  if (state.page === 'smart-selection-result' && owner && set) return `#/smart-selection-result/${routeIdV910(owner.id)}/${routeIdV910(set.id)}/${routeIdV910(state.smartSelectionId)}`;
...
function applyHashRouteV910() {
  ...
  } else if (['dataset', 'feature', 'experiments', 'model-select', 'smart-selection-config'].includes(page)) {
    const context = resolveRouteContextV910(projectId, datasetId);
    state.page = context.ok ? page : context.fallback;
  } else if (page === 'smart-selection-result') {
    const context = resolveRouteContextV910(projectId, datasetId);
    if (context.ok) {
      const record = state.smartSelections.find(r => r.id === experimentId);
      if (record && record.datasetId === datasetId && record.projectId === projectId) {
        state.smartSelectionId = record.id;
        state.page = 'smart-selection-result';
      } else {
        state.page = 'experiments';
        toast('选型记录不存在或已删除', 'warning');
      }
    } else state.page = context.fallback;
...
function forecastSeriesV10(set = dataset()) {
  ...
  times.forEach((value, index) => { const timestamp = parseForecastDateV10(value), rawTarget = values[index], target = Number(rawTarget); if (Number.isFinite(timestamp) && String(rawTarget ?? '').trim() && Number.isFinite(target)) map.set(timestamp, target); });
...
datasetSourceModal = function () {
  ...
  modal(`<h2>添加时序数据集</h2><p>上传单序列、多变量 CSV，或使用内置的商品每日销量示例。</p><div class="source-tabs"><button class="active" data-source-tab="upload">上传 CSV</button><button data-source-tab="example">示例数据集</button></div><div class="source-panel active" data-source-panel="upload"><div class="upload-box">${button('选择 CSV 文件', 'choose-csv', 'primary')}<span>一个时间列、一个目标列，可包含多个外部特征；最多 5,000 行</span></div></div><div class="source-panel" data-source-panel="example"><div class="example-dataset-grid"><button class="example-dataset-card" data-action="use-example:forecast-sales-v10"><b>商品每日销量示例</b><span>365 期历史 + 7 期未来特征 · 目标：销量</span></button></div></div><div class="modal-actions">${button('取消', 'close-modal')}</div>`);
  const root = document.querySelector('.modal-backdrop');
  root.querySelectorAll('[data-action]').forEach(control => control.onclick = event => { event.stopPropagation(); action(control.dataset.action); });
  root.querySelectorAll('[data-source-tab]').forEach(tab => tab.onclick = () => { root.querySelectorAll('[data-source-tab]').forEach(item => item.classList.toggle('active', item === tab)); root.querySelectorAll('[data-source-panel]').forEach(panel => panel.classList.toggle('active', panel.dataset.sourcePanel === tab.dataset.sourceTab)); });
};
...
// ============================================================================
const FORECAST_HORIZON_RULES_V102 = { day: { cap: 90, defaultValue: 7 }, week: { cap: 26, defaultValue: 4 }, month: { cap: 12, defaultValue: 3 } };
if (!state.forecastV102MetricDefaultsApplied) { state.forecastVisibleMetricsV10 = ['mae', 'rmse']; state.forecastLibraryVisibleMetricsV10 = ['mae', 'rmse']; state.forecastV102MetricDefaultsApplied = true; }

function forecastPeriodStartV102(timestamp, frequency) {
  const date = new Date(timestamp);
  if (frequency === 'month') return Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), 1);
  if (frequency === 'week') { const offset = (date.getUTCDay() + 6) % 7; return Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate() - offset); }
  return Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate());
}
forecastSummaryV10 = function (set) {
  const timeColumn = set.columns?.find(column => column.name === set.timeColumn && column.type === 'date'), targetColumn = set.columns?.find(column => column.name === set.target && column.type === 'number');
  const times = timeColumn ? forecastValuesV10(set, timeColumn.name) : [], targets = targetColumn ? forecastValuesV10(set, targetColumn.name) : [], parsedInOrder = times.map(parseForecastDateV10), validTimes = parsedInOrder.filter(Number.isFinite);
  const inferred = validTimes.length ? inferForecastFrequencyV10(times) : 'day', frequency = FORECAST_FREQUENCIES_V10[set.frequency] ? set.frequency : inferred, periodValues = validTimes.map(value => forecastPeriodStartV102(value, frequency)), uniquePeriods = [...new Set(periodValues)].sort((a, b) => a - b);
  const invalidTimeCount = Math.max(0, times.length - validTimes.length), duplicateCount = Math.max(0, periodValues.length - uniquePeriods.length), unsorted = parsedInOrder.some((value, index) => index > 0 && Number.isFinite(value) && Number.isFinite(parsedInOrder[index - 1]) && value < parsedInOrder[index - 1]);
  const invalidTargetCount = targets.filter(value => String(value ?? '').trim() && !Number.isFinite(Number(value))).length, missingTargetCount = targets.filter(value => !String(value ?? '').trim()).length;
  let missingPointCount = 0;
  if (uniquePeriods.length > 1) { const available = new Set(uniquePeriods); for (let cursor = addForecastStepV10(uniquePeriods[0], frequency); cursor < uniquePeriods.at(-1); cursor = addForecastStepV10(cursor, frequency)) { if (!available.has(cursor)) missingPointCount += 1; if (missingPointCount > 10000) break; } }
  const minimum = FORECAST_FREQUENCIES_V10[frequency]?.min || 60, validPoints = Math.max(0, Math.min(uniquePeriods.length, targets.length) - invalidTargetCount - missingTargetCount), blockers = [];
  if (!timeColumn) blockers.push('没有可用的日期型时间列'); if (!targetColumn) blockers.push('没有可用的数值型目标列'); if (invalidTimeCount) blockers.push(`${invalidTimeCount} 个时间值无法解析`); if (duplicateCount) blockers.push(`${duplicateCount} 个时间周期存在多条记录`); if (missingPointCount) blockers.push(`${missingPointCount} 个历史时间点缺失，请修复后重新上传`); if (invalidTargetCount) blockers.push(`${invalidTargetCount} 个目标值不是数值`); if (missingTargetCount && set.forecastConfig?.missingTargetPolicy === 'pending') blockers.push(`${missingTargetCount} 个目标值尚未选择处理方式`); if (validPoints < minimum) blockers.push(`有效历史时间点少于 ${minimum} 个`);
  return { inferred, frequency, invalidTimeCount, duplicateCount, invalidTargetCount, missingTargetCount, missingPointCount, validPoints, minimum, start: uniquePeriods[0], end: uniquePeriods.at(-1), totalEnd: uniquePeriods.at(-1), futureRows: 0, unsorted, blockers };
};
function forecastHorizonLimitV102(set) { const summary = set.timeSummary || forecastSummaryV10(set), rule = FORECAST_HORIZON_RULES_V102[set.frequency] || FORECAST_HORIZON_RULES_V102.day; return Math.max(0, Math.min(rule.cap, Math.floor((summary.validPoints - summary.minimum) / 3))); }
function forecastHorizonValidationV102(set, rawValue = set.forecastConfig?.horizon) { const value = Number(rawValue), maximum = forecastHorizonLimitV102(set), valid = Number.isInteger(value) && value >= 1 && value <= maximum; return { value, maximum, valid, message: maximum < 1 ? '当前历史长度不足以完成 3 轮回测；至少还需要 3 个有效时间点。' : `预测期数请输入 1–${maximum}。` }; }
ensureForecastDatasetV10 = function (set) {
  if (!set || !isForecastingV10(state.projects.find(owner => owner.id === set.projectId))) return set;
  set.columns ||= []; const selectedTime = set.columns.find(column => column.name === set.timeColumn && column.type === 'date'); set.timeColumn = selectedTime?.name || set.columns.find(column => column.type === 'date')?.name || null;
  const selectedTarget = set.columns.find(column => column.name === set.target && column.type === 'number' && column.name !== set.timeColumn); set.target = selectedTarget?.name || set.columns.filter(column => column.type === 'number' && column.name !== set.timeColumn).at(-1)?.name || null;
  set.frequency = FORECAST_FREQUENCIES_V10[set.frequency] ? set.frequency : inferForecastFrequencyV10(forecastValuesV10(set, set.timeColumn)); set.feature ||= { revision: 1 }; set.forecastConfig ||= {};
  const rule = FORECAST_HORIZON_RULES_V102[set.frequency] || FORECAST_HORIZON_RULES_V102.day; if (!Number.isFinite(Number(set.forecastConfig.horizon))) set.forecastConfig.horizon = rule.defaultValue;
  set.forecastConfig.horizon = Number(set.forecastConfig.horizon); set.forecastConfig.backtestRounds = 3; delete set.forecastConfig.missingTimePolicy; set.forecastConfig.missingTargetPolicy ||= 'pending'; set.forecastConfig.calendarFeatures = set.forecastConfig.calendarFeatures !== false;
  set.forecastFeatureThresholds = { missing: Number(set.forecastFeatureThresholds?.missing ?? 90), variance: Number(set.forecastFeatureThresholds?.variance ?? .01), correlation: Number(set.forecastFeatureThresholds?.correlation ?? .8) }; delete set.forecastFuturePolicies;
  set.columns.forEach(column => { column.target = column.name === set.target; if (column.name === set.timeColumn || column.target) { column.included = false; column.trainable = false; return; } column.trainable = !['text', 'date'].includes(column.type); column.reason = column.trainable ? '' : column.type === 'date' ? '未选作时间列的日期字段' : '自由文本字段'; if (column.included === undefined) column.included = column.trainable; });
  set.timeSummary = forecastSummaryV10(set); set.horizonValidationV102 = forecastHorizonValidationV102(set); forecastExternalColumnsV101(set).forEach(column => { column.forecastMeta = forecastFeatureStatsV101(set, column); }); return set;
};

createForecastExampleV10 = function () {
  const owner = project(), id = uid('d'), start = Date.UTC(2025, 4, 1), preview = [], dates = [], sales = [], prices = [], promotions = [], advertising = [], holidays = [];
  for (let index = 0; index < 365; index += 1) {
    const timestamp = start + index * 86400000, date = new Date(timestamp), weekend = [0, 6].includes(date.getUTCDay()), promotion = index % 19 < 3 || weekend, holiday = index % 61 === 0 ? '节假日' : '普通日', price = +(13.8 + 1.1 * Math.sin(index * Math.PI * 2 / 45) - (promotion ? 1.2 : 0)).toFixed(2), ad = +(20 + 8 * Math.sin(index * Math.PI * 2 / 28) + (promotion ? 12 : 0)).toFixed(1);
    const target = Math.round(92 + index * .05 + 10 * Math.sin(index * Math.PI * 2 / 7) - price * 2.1 + (promotion ? 19 : 0) + ad * .32 + (holiday === '节假日' ? 12 : 0) + ((index * 17) % 9 - 4) * .6), dateText = formatForecastDateV10(timestamp);
    dates.push(dateText); sales.push(target); prices.push(price); promotions.push(promotion ? '是' : '否'); advertising.push(ad); holidays.push(holiday); preview.push([dateText, target, price, promotion ? '是' : '否', ad, holiday]);
  }
  const columns = [
    { name: '日期', type: 'date', missing: 0, unique: 365, trainable: false, included: false },
    { name: '销量', type: 'number', missing: 0, unique: new Set(sales).size, trainable: false, target: true, included: false },
    { name: '商品价格', type: 'number', missing: 0, unique: new Set(prices).size, trainable: true, included: true },
    { name: '是否促销', type: 'boolean', missing: 0, unique: 2, trainable: true, included: true },
    { name: '广告费用', type: 'number', missing: 0, unique: new Set(advertising).size, trainable: true, included: true },
    { name: '节假日类型', type: 'category', missing: 0, unique: 2, trainable: true, included: true }
  ];
  const set = { id, projectId: owner.id, name: '商品每日销量示例', uploadedAt: now(), rows: 365, timeColumn: '日期', target: '销量', frequency: 'day', columns, preview: preview.slice(0, 150), forecastValues: { 日期: dates, 销量: sales, 商品价格: prices, 是否促销: promotions, 广告费用: advertising, 节假日类型: holidays }, experiments: [], feature: { revision: 1 }, forecastConfig: { horizon: 7, backtestRounds: 3, missingTargetPolicy: 'none', calendarFeatures: true }, forecastFeatureThresholds: { missing: 90, variance: .01, correlation: .8 }, dataSource: 'example-v102' };
  ensureForecastDatasetV10(set); state.datasets[id] = set; owner.datasets.push(id); owner.updatedAt = now(); state.datasetId = id; state.featureStep = 0; state.previewPagesV910[id] = 0; save(); go('dataset'); setTimeout(() => toast('已创建包含 365 期完整历史数据的销量示例。', 'success'), 0);
};
buildForecastCsvDatasetV10 = function (rows, name) {
  if (rows.length < 2) throw new Error('CSV 至少需要表头和一行数据。'); if (rows.length - 1 > 5000) throw new Error('时序前端演示最多支持 5,000 个时间点。');
  const headers = rows[0].map(value => String(value).trim()); if (!headers.every(Boolean) || new Set(headers).size !== headers.length) throw new Error('CSV 表头不能为空或重复。');
  const data = rows.slice(1).filter(row => row.some(value => String(value ?? '').trim()));
  const columns = headers.map((header, index) => { const values = data.map(row => String(row[index] ?? '').trim()), nonempty = values.filter(Boolean), dateRate = nonempty.filter(value => Number.isFinite(parseForecastDateV10(value))).length / Math.max(1, nonempty.length), numberRate = nonempty.filter(value => Number.isFinite(Number(value))).length / Math.max(1, nonempty.length), unique = new Set(nonempty).size; const type = dateRate >= .9 ? 'date' : numberRate >= .9 ? 'number' : unique <= Math.max(20, nonempty.length * .1) ? (unique === 2 ? 'boolean' : 'category') : 'text'; return { name: header, type, missing: 1 - nonempty.length / Math.max(1, data.length), unique, trainable: !['text', 'date'].includes(type), included: !['text', 'date'].includes(type) }; });
  const timeColumn = columns.find(column => column.type === 'date')?.name || null, numericCandidates = columns.filter(column => column.type === 'number' && column.name !== timeColumn), target = numericCandidates.at(-1)?.name;
  if (!target) throw new Error('未识别到可用的数值目标列，请检查 CSV。'); columns.find(column => column.name === target).target = true; columns.find(column => column.name === target).included = false;
  const forecastValues = Object.fromEntries(headers.map((header, index) => [header, data.map(row => row[index] ?? '')])), id = uid('d');
  return ensureForecastDatasetV10({ id, projectId: project().id, name: name.replace(/\.csv$/i, ''), uploadedAt: now(), rows: data.length, timeColumn, target, frequency: timeColumn ? inferForecastFrequencyV10(forecastValues[timeColumn]) : 'day', columns, preview: data.slice(0, 150), forecastValues, experiments: [], feature: { revision: 1 }, forecastConfig: { horizon: 7, backtestRounds: 3, missingTargetPolicy: 'pending', calendarFeatures: true }, forecastFeatureThresholds: { missing: 90, variance: .01, correlation: .8 }, dataSource: 'upload' });
};
function forecastHistoryChartV102(set) {
  const points = forecastSeriesV10(set).slice(-180); if (points.length < 2) return `<div class="notice"><b>历史趋势</b><span>至少需要两个有效目标值才能绘制。</span></div>`;
  const width = 900, height = 230, pad = { left: 52, right: 20, top: 18, bottom: 34 }, values = points.map(point => point.value), min = Math.min(...values), max = Math.max(...values), span = Math.max(1, max - min), x = index => pad.left + index * (width - pad.left - pad.right) / (points.length - 1), y = value => pad.top + (max - value) * (height - pad.top - pad.bottom) / span, path = points.map((point, index) => `${index ? 'L' : 'M'}${x(index).toFixed(1)},${y(point.value).toFixed(1)}`).join(' ');
  return `<section class="section-block forecast-history-overview-v102"><div class="section-title"><div><h2>历史目标趋势</h2><p>展示最近 ${points.length} 个有效时间点；统计仍基于完整历史数据。</p></div><span>${esc(set.target || '目标列')}</span></div><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="历史目标趋势图"><path d="${path}"></path><text x="${pad.left}" y="${height - 9}">${esc(points[0].date)}</text><text x="${width - pad.right}" y="${height - 9}" text-anchor="end">${esc(points.at(-1).date)}</text></svg></section>`;
}
forecastDatasetPageV10 = function () {
  const set = ensureForecastDatasetV10(dataset()), summary = set.timeSummary, locked = datasetHasTrainingArtifactsV909(set), dateCandidates = set.columns.filter(column => column.type === 'date'), numericCandidates = set.columns.filter(column => column.type === 'number' && column.name !== set.timeColumn), blocking = summary.blockers.length > 0;
  const timeControl = dateCandidates.length ? `<select data-forecast-time ${locked ? 'disabled' : ''}>${dateCandidates.map(column => `<option value="${esc(column.name)}" ${column.name === set.timeColumn ? 'selected' : ''}>${esc(column.name)}</option>`).join('')}</select>` : '<select disabled><option>未识别到日期型字段</option></select>', targetControl = numericCandidates.length ? `<select data-forecast-target ${locked ? 'disabled' : ''}>${numericCandidates.map(column => `<option value="${esc(column.name)}" ${column.name === set.target ? 'selected' : ''}>${esc(column.name)}</option>`).join('')}</select>` : '<select disabled><option>未识别到数值型字段</option></select>';
  const validationRows = [['时间字段', set.timeColumn || '未指定', Boolean(set.timeColumn)], ['时间格式', summary.invalidTimeCount ? `${summary.invalidTimeCount} 个无效值` : '全部可解析', Boolean(set.timeColumn) && !summary.invalidTimeCount], ['频率内唯一性', summary.duplicateCount ? `${summary.duplicateCount} 个周期重复` : '每个周期一条记录', !summary.duplicateCount], ['时间连续性', summary.missingPointCount ? `缺少 ${summary.missingPointCount} 个历史时间点` : '连续', !summary.missingPointCount], ['目标列数值', summary.invalidTargetCount ? `${summary.invalidTargetCount} 个非数值` : '全部为数值', !summary.invalidTargetCount], ['目标列缺失', summary.missingTargetCount ? `${summary.missingTargetCount} 个缺失` : '无缺失', !summary.missingTargetCount || set.forecastConfig.missingTargetPolicy !== 'pending'], ['历史长度', `${summary.validPoints} 个有效时间点（最低 ${summary.minimum}）`, summary.validPoints >= summary.minimum]];
  const fieldRows = set.columns.map((column, index) => { const isTime = column.name === set.timeColumn, isTarget = column.name === set.target, usable = !isTime && !isTarget && !['text', 'date'].includes(column.type), role = isTime ? '<span class="status target">时间列</span>' : isTarget ? '<span class="status target">目标列</span>' : usable ? '<span class="status good">可作外部特征</span>' : '<span class="status blocked">不参与训练</span>'; return `<tr><td><b>${esc(column.name)}</b>${isTime ? '<small>用于排序与回测</small>' : isTarget ? '<small>唯一预测目标</small>' : ''}</td><td><select data-forecast-column-type-v102="${index}" ${locked ? 'disabled' : ''}>${['number', 'category', 'date', 'boolean', 'text'].map(type => `<option value="${type}" ${type === column.type ? 'selected' : ''}>${typeLabel(type)}</option>`).join('')}</select></td><td>${(Number(column.missing || 0) * 100).toFixed(1)}%</td><td>${Number(column.unique || 0).toLocaleString()}</td><td>${role}</td></tr>`; }).join('');
  return shell(`${steps('dataset')}${pageHead('数据检查', `${esc(set.name)} · 单序列、多变量时序预测`)}<div class="forecast-config-grid-v10"><label class="panel"><span>时间列</span>${timeControl}<small>只能选择已确认为日期类型的字段</small></label><label class="panel"><span>预测目标</span>${targetControl}<small>只能选择一个数值型目标列</small></label><label class="panel"><span>数据频率</span><select data-forecast-frequency ${locked ? 'disabled' : ''}>${Object.entries(FORECAST_FREQUENCIES_V10).map(([key, value]) => `<option value="${key}" ${key === set.frequency ? 'selected' : ''}>${value.label}</option>`).join('')}</select><small>系统推断：${FORECAST_FREQUENCIES_V10[summary.inferred]?.label || '无法推断'}，请确认</small></label></div>${locked ? `<div class="notice"><b>结构已锁定</b><span>已有训练产物。如需修改时间列、目标列或字段类型，请复制数据集。</span>${button('复制并修改时序结构', 'copy-forecast-dataset-v10')}</div>` : ''}<div class="forecast-summary-strip-v101"><span><b>${summary.validPoints}</b>历史训练点</span><span><b>${set.columns.length}</b>字段</span><span><b>${forecastExternalColumnsV101(set).length}</b>候选外部特征</span><span><b>1</b>预测目标</span></div>${blocking ? `<div class="notice danger"><b>完成以下检查后才能继续</b><span>${summary.blockers.map(esc).join('；')}</span></div>` : `<div class="notice success"><b>时间序列检查通过</b><span>${summary.unsorted ? '原始行顺序未按时间升序，平台训练时会自动排序。' : '可以继续配置预测范围与回测。'}</span></div>`}<div class="section-title"><h2>字段类型检查</h2><span>先确认字段类型，再选择时间列和目标列</span></div><div class="table-card forecast-field-table-v102"><table><thead><tr><th>字段</th><th>确认类型</th><th>缺失率</th><th>唯一值</th><th>训练可用性</th></tr></thead><tbody>${fieldRows}</tbody></table></div>${forecastHistoryChartV102(set)}<div class="section-title"><h2>时序有效性检查</h2><span>历史时间缺口不自动补齐，需修复源数据后重新上传</span></div><div class="table-card"><table><thead><tr><th>检查项</th><th>结果</th><th>状态</th></tr></thead><tbody>${validationRows.map(row => `<tr><td><b>${row[0]}</b></td><td>${esc(row[1])}</td><td><span class="status ${row[2] ? 'good' : 'blocked'}">${row[2] ? '通过' : '待处理'}</span></td></tr>`).join('')}</tbody></table></div>${summary.missingTargetCount ? `<section class="panel forecast-missing-v10"><div class="panel-head"><div><h2>历史目标缺失处理</h2><p>该规则由同一数据集下所有模型实验共享。</p></div></div><label>缺失目标值<select data-forecast-missing-target><option value="pending" ${set.forecastConfig.missingTargetPolicy === 'pending' ? 'selected' : ''}>请选择</option><option value="zero" ${set.forecastConfig.missingTargetPolicy === 'zero' ? 'selected' : ''}>补 0</option><option value="linear" ${set.forecastConfig.missingTargetPolicy === 'linear' ? 'selected' : ''}>线性插值</option></select></label></section>` : ''}${previewSectionV910(set)}`);
};
const datasetSourceModalBeforeV102 = datasetSourceModal;
datasetSourceModal = function () { datasetSourceModalBeforeV102(); if (!isForecastingV10()) return; const note = document.querySelector('.example-dataset-card span'); if (note) note.textContent = '365 期完整历史数据 · 1 个目标列 + 4 个外部特征'; };

function forecastFirstTrainingRowsV102(set) {
  const context = forecastHistoryContextV101(set), horizon = Math.max(1, Number(set.forecastConfig.horizon || 1)), rows = context.historyRows.filter(row => Number.isFinite(row.timestamp)).sort((a, b) => a.timestamp - b.timestamp), count = Math.max(0, rows.length - horizon * 3);
  return rows.slice(0, Math.max(set.timeSummary?.minimum || 0, count));
}
forecastFeatureStatsV101 = function (set, column) {
  const rows = forecastFirstTrainingRowsV102(set), values = forecastValuesV10(set, column.name), targets = forecastValuesV10(set, set.target), indexes = rows.map(row => row.index), raw = indexes.map(index => values[index]), nonempty = raw.filter(value => String(value ?? '').trim()), missingRate = 1 - nonempty.length / Math.max(1, raw.length), numericValues = raw.map(Number).filter(Number.isFinite);
  let variance = null;
  if (column.type === 'number' && numericValues.length) { const min = Math.min(...numericValues), max = Math.max(...numericValues), normalized = numericValues.map(value => max === min ? 0 : (value - min) / (max - min)), mean = normalized.reduce((sum, value) => sum + value, 0) / normalized.length; variance = normalized.reduce((sum, value) => sum + (value - mean) ** 2, 0) / normalized.length; }
  let temporalCorrelation = null, bestLag = null;
  if (column.type === 'number') {
    const maximumLag = Math.min(14, Math.max(0, Math.floor(indexes.length / 4)));
    for (let lag = 0; lag <= maximumLag; lag += 1) {
      const featureSeries = [], targetSeries = [];
      for (let position = lag; position < indexes.length; position += 1) { const featureValue = Number(values[indexes[position - lag]]), targetValue = Number(targets[indexes[position]]); if (Number.isFinite(featureValue) && Number.isFinite(targetValue)) { featureSeries.push(featureValue); targetSeries.push(targetValue); } }
      const score = forecastPearsonV101(featureSeries, targetSeries); if (score !== null && (temporalCorrelation === null || Math.abs(score) > Math.abs(temporalCorrelation))) { temporalCorrelation = score; bestLag = lag; }
    }
  }
  return { missingRate, variance, temporalCorrelation, bestLag, trainable: !['text', 'date'].includes(column.type), trainingPoints: indexes.length };
};
function forecastCorrelationMatrixV102(set, columns) {
  const numeric = columns.filter(column => column.type === 'number').slice(0, 8), rows = forecastFirstTrainingRowsV102(set), threshold = set.forecastFeatureThresholds.correlation;
  if (numeric.length < 2) return `<section class="forecast-correlation-v102"><div class="panel-head"><div><h2>特征相关性热力图</h2><p>至少需要两个数值型外部特征才能计算。</p></div></div></section>`;
  const values = Object.fromEntries(numeric.map(column => [column.name, forecastValuesV10(set, column.name)])), score = (first, second) => { const x = [], y = []; rows.forEach(row => { const a = Number(values[first][row.index]), b = Number(values[second][row.index]); if (Number.isFinite(a) && Number.isFinite(b)) { x.push(a); y.push(b); } }); return forecastPearsonV101(x, y) ?? 0; };
  return `<section class="forecast-correlation-v102"><div class="panel-head"><div><h2>特征相关性热力图</h2><p>仅使用首个回测训练窗口中的数值型外部特征；高相关只提示，不自动删除。</p></div><div class="correlation-actions"><label>预警阈值<input data-forecast-correlation-threshold-v102 type="number" min="0.5" max="0.99" step="0.01" value="${threshold}"><small class="threshold-range-error" hidden></small></label><span data-forecast-correlation-unsaved-v1032 hidden>尚未应用</span>${button('确定修改', 'forecast-apply-correlation-v1024', 'primary')}</div></div><div class="heat-legend"><span>低相关</span><i></i><span>高相关</span></div><div class="forecast-correlation-scroll-v101"><table><thead><tr><th></th>${numeric.map(column => `<th>${esc(column.name)}</th>`).join('')}</tr></thead><tbody>${numeric.map(first => `<tr><th>${esc(first.name)}</th>${numeric.map(second => { const value = first === second ? 1 : score(first.name, second.name), strength = Math.max(.08, Math.abs(value)), warning = first !== second && Math.abs(value) >= threshold; return `<td class="${warning ? 'warning' : ''}" style="--corr:${strength.toFixed(2)}" title="${esc(first.name)} × ${esc(second.name)}：${value.toFixed(2)}">${value.toFixed(2)}</td>`; }).join('')}</tr>`).join('')}</tbody></table></div><small>最多展示 8 个数值型外部特征。达到阈值时仅标记风险，由用户决定是否排除。</small></section>`;
}
forecastFeaturePageV10 = function () {
  const set = ensureForecastDatasetV10(dataset()), step = Number(state.featureStep || 0) > 0 ? 1 : 0, validation = forecastHorizonValidationV102(set);
  if (!step) {
    const windows = validation.valid ? forecastBacktestWindowsV10(set) : [];
    return shell(`${steps('feature')}${pageHead('特征准备', `${esc(set.name)} · 先确认预测范围，再管理日历与外部特征`)}<div class="forecast-feature-tabs-v10"><button data-forecast-feature-step="0" class="active"><i>1</i>时间与回测</button><button data-forecast-feature-step="1"><i>2</i>特征管理</button></div><section class="panel"><div class="panel-head"><div><h2>时间与回测</h2><p>平台按时间顺序自动生成 3 轮扩展窗口回测。</p></div><span class="status ${validation.valid ? 'good' : 'blocked'}">${validation.valid ? '3 轮可用' : '需要调整'}</span></div><label class="field forecast-horizon-field-v10"><span>预测未来多少期</span><input data-forecast-horizon-v102 type="number" min="1" max="${Math.max(1, validation.maximum)}" value="${set.forecastConfig.horizon}" class="${validation.valid ? '' : 'range-invalid'}" aria-invalid="${!validation.valid}"><small>上限同时受数据频率和 3 轮回测所需历史长度限制；${set.frequency === 'day' ? '按日最多 90 期' : set.frequency === 'week' ? '按周最多 26 期' : '按月最多 12 期'}。</small><small class="threshold-range-error" ${validation.valid ? 'hidden' : ''}>${esc(validation.message)}</small></label>${validation.valid ? `<div class="table-card"><table><thead><tr><th>回测轮次</th><th>训练数据截止</th><th>验证区间</th><th>验证长度</th></tr></thead><tbody>${windows.map(window => `<tr><td><b>第 ${window.round} 轮</b></td><td>${formatForecastDateV10(window.trainingEnd, set.frequency)}</td><td>${formatForecastDateV10(window.start, set.frequency)} 至 ${formatForecastDateV10(window.end, set.frequency)}</td><td>${set.forecastConfig.horizon} 期</td></tr>`).join('')}</tbody></table></div>` : `<div class="notice danger"><b>无法生成回测窗口</b><span>${esc(validation.message)}</span></div>`}<div class="notice"><b>排名规则</b><span>训练结果默认按 3 轮回测平均 MAE 从小到大排序；稳定性表示各轮 MAE 的波动。</span></div></section>`);
  }
  const columns = forecastExternalColumnsV101(set), thresholds = set.forecastFeatureThresholds, included = columns.filter(column => column.included).length;
  const rows = columns.map(column => { const meta = forecastFeatureStatsV101(set, column), passesMissing = meta.missingRate * 100 <= thresholds.missing, passesVariance = meta.variance === null || meta.variance >= thresholds.variance, recommended = meta.trainable && passesMissing && passesVariance, status = !meta.trainable ? ['不可训练', 'blocked'] : column.included ? ['已纳入', 'good'] : recommended ? ['手动排除', 'muted'] : ['建议排除', 'blocked']; column.forecastMeta = meta; return `<tr><td><label class="forecast-feature-check-v101"><input type="checkbox" data-forecast-feature-toggle="${esc(column.name)}" aria-label="纳入外部特征${esc(column.name)}" ${column.included ? 'checked' : ''} ${meta.trainable ? '' : 'disabled'}><span aria-hidden="true"></span></label></td><td><b>${esc(column.name)}</b><small>上传字段</small></td><td>${typeLabel(column.type)}</td><td class="forecast-number-cell-v101 ${passesMissing ? '' : 'threshold-failed'}">${(meta.missingRate * 100).toFixed(1)}%</td><td class="forecast-number-cell-v101 ${passesVariance ? '' : 'threshold-failed'}">${meta.variance === null ? '不适用' : meta.variance.toFixed(3)}</td><td class="forecast-number-cell-v101">${meta.temporalCorrelation === null ? '不适用' : `${meta.temporalCorrelation.toFixed(2)}<small>${meta.bestLag ? `滞后 ${meta.bestLag} 期` : '同期'}</small>`}</td><td><span class="status ${status[1]}">${status[0]}</span></td></tr>`; }).join('');
  const body = `<section class="panel"><div class="panel-head"><div><h2>特征管理</h2><p>「自动筛选」按系统推荐值一键筛选；也可修改下方阈值后点击「确定修改」。</p></div><div class="head-actions">${button('自动筛选', 'forecast-auto-filter-v1023', 'primary')}</div></div><div class="calendar-toggle-v10"><div><b>自动生成日历特征</b><span>根据时间列生成星期、月份、季度、周末等特征，不修改原始数据。</span></div><label class="switch"><input type="checkbox" data-forecast-calendar ${set.forecastConfig.calendarFeatures ? 'checked' : ''}><span></span></label></div><div class="forecast-calendar-list-v10">${forecastCalendarLabelsV10(set.frequency).map(label => `<span class="${set.forecastConfig.calendarFeatures ? '' : 'disabled'}">${label}</span>`).join('')}</div><div class="forecast-thresholds-v101"><label><span>历史缺失率超过</span><span class="threshold-input-wrap-v101"><input data-forecast-threshold="missing" type="number" min="0" max="99" step="1" value="${thresholds.missing}"><i aria-hidden="true">%</i></span></label><label><span>归一化方差低于</span><input data-forecast-threshold="variance" type="number" min="0" max="0.25" step="0.001" value="${thresholds.variance}"></label><div>${button('推荐阈值', 'forecast-recommend-thresholds-v101')}${button('确定修改', 'forecast-apply-thresholds-v101', 'primary')}</div></div><details class="metric-guide compact-guide"><summary>查看指标说明</summary><div><b>历史缺失率</b><span>首个训练窗口内的缺失比例；超过阈值时建议排除。</span></div><div><b>归一化方差</b><span>仅数值特征计算；Min-Max 归一化后低于阈值表示变化不足。</span></div><div><b>时序相关性</b><span>比较同期与最多 14 期滞后相关性，只作参考，不自动筛选。</span></div></details><div class="table-card forecast-feature-table-wrap-v101"><table class="forecast-feature-table-v101"><thead><tr><th>纳入</th><th>特征</th><th>类型</th><th>历史缺失率</th><th>归一化方差</th><th>时序相关性</th><th>状态</th></tr></thead><tbody>${rows || '<tr><td colspan="7"><div class="empty-state"><p>没有可作为外部特征的字段，仍可进行单变量预测。</p></div></td></tr>'}</tbody></table></div>${forecastCorrelationMatrixV102(set, columns)}</section>`;
  return shell(`${steps('feature')}${pageHead('特征准备', `${esc(set.name)} · 管理日历与外部特征`)}<div class="forecast-feature-tabs-v10"><button data-forecast-feature-step="0"><i>1</i>时间与回测</button><button data-forecast-feature-step="1" class="active"><i>2</i>特征管理</button></div>${body}`);
};
forecastThresholdFeedbackV1011 = function (input) {
  const key = input.dataset.forecastThreshold, limits = { missing: '请输入 0–99', variance: '请输入 0–0.25' }, invalid = !input.value.trim() || !input.checkValidity(), label = input.closest('label'); input.classList.toggle('range-invalid', invalid); input.setAttribute('aria-invalid', String(invalid)); let message = label?.querySelector('.threshold-range-error'); if (!message && label) { message = document.createElement('small'); message.className = 'threshold-range-error'; label.appendChild(message); } if (message) { message.textContent = invalid ? limits[key] : ''; message.hidden = !invalid; } return !invalid;
};

forecastExperimentPageV10 = function () {
  const item = experiment(), set = ensureForecastDatasetV10(dataset()), locked = item.status === 'completed' && item.results?.length, selected = forecastSelectedFeaturesV101(set), title = forecastDisplayModelNameV101(item.type, set);
  const external = `<div class="notice ${forecastUsesExternalV101(item.type) && selected.length ? 'success' : ''}"><b>外部特征</b><span>${forecastUsesExternalV101(item.type) ? (selected.length ? `训练将使用：${selected.map(column => esc(column.name)).join('、')}` : '当前未纳入外部特征，将按单变量模式训练。') : '该模型作为目标历史基准，不使用外部特征。'}</span></div>`;
  return shell(`${steps('experiments')}${pageHead(esc(item.name), `${esc(title)} · ${esc(set.name)}${locked ? ' · 成功实验已锁定' : ''}`)}${locked ? `<div class="notice"><b>实验配置已锁定</b><span>训练结果和报告保持不变。如需调整参数或特征，请复制为新模型实验。</span>${button('复制为新模型实验', 'clone-experiment', 'primary')}</div>` : ''}<div class="config-grid forecast-experiment-config-v10"><section class="panel"><div class="panel-head"><div><h2>实验设置</h2><p>数据频率、预测范围和回测窗口继承自数据集。</p></div></div><label class="field"><span>实验名称</span><input data-forecast-experiment-name value="${esc(item.name)}" maxlength="80" ${locked ? 'disabled' : ''}></label><div class="forecast-inherited-v10"><span><b>${FORECAST_FREQUENCIES_V10[set.frequency].label}</b>数据频率</span><span><b>${set.forecastConfig.horizon} 期</b>预测范围</span><span><b>3 轮</b>滚动回测</span></div>${external}<details class="advanced"><summary>模型自动处理</summary>${forecastProcessingSummaryV10(item.type)}</details>${forecastAdvancedParamsV10(item, set)}</section><section class="panel"><div class="panel-head"><div><h2>调参方式</h2><p>只展示当前模型支持的选项。</p></div><span class="mock">模拟训练</span></div>${locked ? '<div class="notice">当前实验已完成，调参方式不可修改。</div>' : forecastTuningOptionsV10(item)}</section></div>${state.training ? trainingPanel() : ''}`);
};
startForecastTrainingV10 = function () {
  const item = experiment(), set = ensureForecastDatasetV10(dataset()), horizon = forecastHorizonValidationV102(set), invalidParam = [...app.querySelectorAll('[data-forecast-param]')].find(input => !input.checkValidity());
  if (item.status === 'completed' && item.results?.length) return toast('已完成实验不能直接重训，请复制为新模型实验。', 'warning');
  if (set.timeSummary.blockers.length) return toast('请先完成数据检查中的阻断项。', 'warning');
  if (!horizon.valid) return toast(horizon.message, 'warning');
  if (invalidParam) { invalidParam.focus(); invalidParam.classList.add('range-invalid'); return toast('请修正标红的模型参数。', 'warning'); }
  const run = ++forecastTrainingRunV10; item.status = 'training'; state.training = { progress: 8, label: '正在校验时间顺序', minimized: false }; render();
  const stages = [[28, '正在生成滚动回测窗口'], [52, '正在执行模型专属时序处理'], [78, '正在计算三轮回测指标'], [100, '正在生成训练结果']]; let index = 0;
  const timer = setInterval(() => { if (run !== forecastTrainingRunV10) return clearInterval(timer); state.training = { ...state.training, progress: stages[index][0], label: stages[index][1] }; render(); index += 1; if (index === stages.length) { clearInterval(timer); setTimeout(() => { if (run !== forecastTrainingRunV10) return; item.results = makeForecastResultsV10(item); item.selected = item.results[0].id; item.status = 'completed'; item.updatedAt = now(); state.training = null; state.tuningSelections[item.id] = [item.selected]; state.tuningSelectionTouched[item.id] = false; save(); go('tuning'); }, 300); } }, 450);
};
forecastTuningPageV10 = function () {
  const item = experiment(); if (!item?.results?.length) return shell(`${steps('tuning')}${pageHead('训练结果', '完成模型训练后查看滚动回测结果。')}<div class="empty-state"><h2>还没有结果</h2>${button('返回模型训练', 'go-experiment', 'primary')}</div>`);
  const metrics = state.forecastVisibleMetricsV10.filter(key => FORECAST_METRICS_V10[key]), sorted = sortedForecastResultsV10(item), rows = state.showAllResults ? sorted : sorted.slice(0, 3), selected = new Set(state.tuningSelections[item.id] || []);
  const header = key => { const metric = FORECAST_METRICS_V10[key], active = state.forecastSortV10 === key, arrow = active ? (state.forecastSortDirectionV10 === 'asc' ? '↑' : '↓') : '↕'; return `<th class="result-metric-column-v101 ${active ? 'active-sort' : ''}"><button class="metric-sort-v10" data-action="sort-forecast-results-v10:${key}">${metric.label} ${metric.direction} <i>${arrow}</i></button></th>`; };
  return shell(`${steps('tuning')}${pageHead('训练结果', `${esc(project().name)} / ${esc(dataset().name)} / ${esc(item.name)}`, button(state.showAllResults ? '仅看 Top 3' : '查看全部结果', 'toggle-results'))}<div class="metric-direction"><span>MAE / RMSE / sMAPE ↓ 越小越好</span><span>稳定性 ↓ 越小表示三轮回测波动越小</span><span>默认按平均 MAE 排名</span></div><div class="forecast-result-toolbar-v10">${forecastMetricPickerV10('result')}<span data-forecast-selected-count-v1023>已选 ${selected.size} 个方案</span>${button('保存到模型库', 'save-library-results', selected.size ? 'primary' : '')}</div><div class="table-card forecast-table-scroll-v102"><table class="forecast-result-table-v10 result-table"><thead><tr><th class="result-save-column-v101">保存</th><th class="result-rank-column-v101">排名</th><th class="result-scheme-column-v101">参数方案</th>${metrics.map(header).join('')}<th class="result-action-column-v101">操作</th></tr></thead><tbody>${rows.map((result, index) => `<tr><td class="result-save-column-v101"><label class="forecast-save-check-v10"><input type="checkbox" data-save-result="${result.id}" ${selected.has(result.id) ? 'checked' : ''}></label></td><td class="result-rank-column-v101"><b>#${index + 1}</b></td><td class="result-scheme-column-v101"><b>方案 #${result.id}</b><small title="${esc(result.params)}">${esc(result.params)}</small></td>${metrics.map(key => `<td class="result-metric-column-v101"><b>${forecastMetricValueV10(result, key)}</b></td>`).join('')}<td class="result-action-column-v101"><div class="row-actions">${button('回测明细', `forecast-backtests-v10:${result.id}`)}${button('查看模型报告', `report-result:${result.id}`, 'primary')}</div></td></tr>`).join('')}</tbody></table></div><details class="metric-guide compact-guide"><summary>查看指标说明</summary><div><b>MAE</b><span>预测值与真实值绝对误差的平均值，与目标列单位一致。</span></div><div><b>RMSE</b><span>对较大误差更敏感，与目标列单位一致。</span></div><div><b>sMAPE</b><span>对称平均绝对百分比误差，便于理解相对误差。</span></div><div><b>稳定性</b><span>三轮 MAE 的标准差，越小表示不同时间窗口表现越稳定。</span></div></details>`);
};
forecastBacktestModalV10 = function (result) {
  const rounds = result.backtests.map(round => { const errors = round.actual.map((point, index) => ({ date: point.date, error: Math.abs(round.predicted[index].value - point.value) })), largest = errors.sort((a, b) => b.error - a.error)[0]; return `<section class="forecast-backtest-round-v102"><div class="section-title"><div><h3>第 ${round.round} 轮 · ${round.start} 至 ${round.end}</h3><p>最大绝对误差：${largest?.error.toFixed(2) || '—'}${largest ? `（${esc(largest.date)}）` : ''}</p></div><span>MAE ${round.mae.toFixed(2)} · RMSE ${round.rmse.toFixed(2)} · sMAPE ${round.smape.toFixed(1)}%</span></div>${forecastSvgV10({ backtests: [round] }, 'backtest')}</section>`; }).join('');
  modal(`<h2>滚动回测明细</h2><p>${esc(experiment().name)} · ${esc(result.params)}</p><div class="forecast-backtest-modal-v102">${rounds}</div><div class="modal-actions">${button('关闭', 'close-modal', 'primary')}</div>`); bindDynamicModalV909(); document.querySelector('.modal')?.classList.add('backtest-modal-v1033');
};
trainingSnapshotPanelV910 = function (snapshot) {
  if (!snapshot) return ''; const featureCount = snapshot.features?.length || snapshot.externalFeatures?.length || 0, targetDetail = snapshot.positive ? `${snapshot.target}（正类：${snapshot.positive}）` : snapshot.target, task = snapshot.task === 'forecasting' ? '时序预测' : snapshot.task === 'multiclass' ? '多分类' : snapshot.task === 'classification' ? '二分类' : '回归', split = snapshot.task === 'forecasting' ? '3 轮滚动回测' : snapshot.split;
  return `<section class="section-block report-training-snapshot"><div class="section-title"><h2>训练信息</h2><span>保存模型时的配置快照</span></div><div class="report-snapshot-grid"><div><span>任务</span><b>${esc(task)}</b></div><div><span>目标列</span><b>${esc(targetDetail)}</b></div><div><span>入模特征</span><b>${featureCount} 个</b></div><div><span>数据划分</span><b>${esc(split || '—')}</b></div><div><span>训练时间</span><b>${esc(snapshot.trainedAt)}</b></div><div><span>结果来源</span><b>前端模拟</b></div></div></section>`;
};
forecastReportPageV10 = function () {
  const owner = project(), set = ensureForecastDatasetV10(dataset()), item = experiment(), snapshot = state.savedResultSnapshots?.[state.forecastReportSnapshotKeyV1011], result = snapshot?.result || item?.results?.find(row => row.id === item.selected) || item?.results?.[0];
  if (!result) return shell(`${pageHead('模型报告', '完成训练后查看报告。')}<div class="empty-state"><h2>还没有报告</h2></div>`);
  const config = result.forecastConfig || set.forecastConfig, frequency = result.frequency || set.frequency, summary = result.timeSummary || set.timeSummary, external = result.externalFeatures || [], allErrors = result.backtests.flatMap(round => round.actual.map((point, index) => ({ date: point.date, signed: round.predicted[index].value - point.value, absolute: Math.abs(round.predicted[index].value - point.value) }))), bias = allErrors.reduce((sum, value) => sum + value.signed, 0) / Math.max(1, allErrors.length), largest = [...allErrors].sort((a, b) => b.absolute - a.absolute)[0], title = result.modelDisplayName || forecastDisplayModelNameV101(item.type, set);
  const future = `<section class="section-block forecast-chart-card-v10"><div class="section-title"><div><h2>未来预测</h2><p>虚线为预测值，深浅阴影分别表示 80% 和 95% 预测区间。</p></div><div class="segmented-v10">${[30, 60, 180].map(value => `<button data-action="forecast-history-window-v10:${value}" class="${Number(state.forecastHistoryWindowV10) === value ? 'active' : ''}">${value === 180 ? '全部' : `近 ${value} 期`}</button>`).join('')}</div></div><div class="forecast-legend-v10"><span class="actual">历史真实值</span><span class="prediction">未来预测</span><span class="band80">80% 区间</span><span class="band95">95% 区间</span></div>${forecastSvgV10(result)}</section>`;
  const contribution = external.length ? `<section class="section-block"><div class="section-title"><div><h2>外部特征使用情况</h2><p>来自本次不可变训练快照。</p></div><span>${external.length} 个</span></div><div class="forecast-report-feature-list-v101">${(result.featureContributions || []).map(feature => `<div><b>${esc(feature.name)}</b><span>${esc(feature.role)}</span><strong>${Number(feature.importance).toFixed(3)}</strong></div>`).join('')}</div></section>` : '';
  return shell(`${pageHead('模型报告', `${esc(owner.name)} / ${esc(set.name)} / ${esc(item.name)}`, button('导出报告', 'export-report-v910', 'primary'))}<section class="metric-overview forecast-report-overview-v10"><div class="section-title"><div><h2>指标总览</h2><p>均为 3 轮滚动回测平均值，默认以 MAE 排名。</p></div><span>${esc(result.params)}</span></div><div class="metrics-grid">${Object.keys(FORECAST_METRICS_V10).map(key => `<div class="metric-card"><span>${FORECAST_METRICS_V10[key].label} ${FORECAST_METRICS_V10[key].direction}</span><b>${forecastMetricValueV10(result, key)}</b><small>${key === 'stability' ? '三轮 MAE 标准差' : '三轮回测平均值'}</small></div>`).join('')}</div></section><section class="section-block"><div class="section-title"><h2>数据与预测配置</h2><span>训练时配置快照</span></div><div class="forecast-report-config-v10"><span><b>时间列</b>${esc(result.timeColumn || set.timeColumn)}</span><span><b>预测目标</b>${esc(result.target || set.target)}</span><span><b>频率</b>${FORECAST_FREQUENCIES_V10[frequency].label}</span><span><b>历史范围</b>${formatForecastDateV10(summary.start, frequency)} 至 ${formatForecastDateV10(summary.end, frequency)}</span><span><b>预测范围</b>未来 ${config.horizon || result.forecast.length} 期</span><span><b>日历特征</b>${config.calendarFeatures ? '已开启' : '未开启'}</span></div></section>${future}<section class="section-block"><div class="section-title"><div><h2>滚动回测</h2><p>展示最近一轮验证区间的真实值与预测值。</p></div>${button('查看三轮明细', `forecast-backtests-v10:${result.id}`)}</div>${forecastSvgV10(result, 'backtest')}</section><section class="section-block"><div class="section-title"><h2>误差分析</h2><span>用于识别预测方向与极端误差</span></div><div class="forecast-error-grid-v10"><div><b>${bias.toFixed(2)}</b><span>平均偏差</span></div><div><b>${largest?.absolute.toFixed(2) || '—'}</b><span>最大绝对误差${largest ? ` · ${esc(largest.date)}` : ''}</span></div><div><b>${result.stability.toFixed(2)}</b><span>回测波动</span></div></div></section>${contribution}<section class="section-block"><div class="section-title"><h2>模型说明</h2><span>${esc(title)}</span></div>${forecastProcessingSummaryV10(item.type)}</section><details class="metric-guide v8-report-guide"><summary>指标说明与参考</summary><div><b>MAE</b><span>平均绝对误差，与目标单位一致。</span><em>越小越好</em></div><div><b>RMSE</b><span>对较大误差更敏感，与目标单位一致。</span><em>越小越好</em></div><div><b>sMAPE</b><span>对称百分比误差，用于理解相对误差。</span><em>越小越好</em></div><div><b>稳定性</b><span>三轮 MAE 的标准差。</span><em>越小越稳定</em></div></details>`);
};

forecastLibraryDetailV10 = function (detail) {
  const { scope, owner, set, item } = detail, allDatasetIds = [...new Set(detail.rows.map(row => row.item.datasetId))], allExperimentIds = [...new Set(detail.rows.map(row => row.item.id))], allModelTypes = [...new Set(detail.rows.map(row => row.item.type))];
  if (scope.type !== 'project') state.libraryDetailDatasetId = 'all'; if (scope.type === 'experiment') { state.libraryDetailExperimentId = 'all'; state.libraryDetailModelType = 'all'; }
  const rows = forecastLibraryRowsV10(detail), metrics = state.forecastLibraryVisibleMetricsV10.filter(key => FORECAST_METRICS_V10[key]), datasetFilter = libraryDetailFilterV909('dataset', state.libraryDetailDatasetId === 'all' ? '全部数据集' : state.datasets[state.libraryDetailDatasetId]?.name || '数据集已删除', [{ value: 'all', label: '全部数据集', selected: state.libraryDetailDatasetId === 'all' }, ...allDatasetIds.map(id => ({ value: id, label: state.datasets[id]?.name || '数据集已删除', selected: id === state.libraryDetailDatasetId }))]), experimentFilter = libraryDetailFilterV909('experiment', state.libraryDetailExperimentId === 'all' ? '全部模型实验' : state.experiments[state.libraryDetailExperimentId]?.name || '模型已删除', [{ value: 'all', label: '全部模型实验', selected: state.libraryDetailExperimentId === 'all' }, ...allExperimentIds.map(id => ({ value: id, label: state.experiments[id]?.name || '模型已删除', selected: id === state.libraryDetailExperimentId }))]), modelFilter = libraryDetailFilterV909('model', state.libraryDetailModelType === 'all' ? '全部模型' : modelName(state.libraryDetailModelType), [{ value: 'all', label: '全部模型', selected: state.libraryDetailModelType === 'all' }, ...allModelTypes.map(type => ({ value: type, label: modelName(type), selected: type === state.libraryDetailModelType }))]);
  const leading = `${scope.type === 'project' ? libraryDetailHeaderV909('数据集', 'dataset', datasetFilter) : ''}${scope.type !== 'experiment' ? `${libraryDetailHeaderV909('模型实验', 'experiment', experimentFilter)}${libraryDetailHeaderV909('模型类型', 'modelType', modelFilter)}` : ''}`, header = key => { const metric = FORECAST_METRICS_V10[key], active = state.forecastLibrarySortV10 === key, arrow = active ? (state.forecastLibrarySortDirectionV10 === 'asc' ? '↑' : '↓') : '↕'; return `<th class="result-metric-column-v101"><button class="metric-sort-v10" data-action="sort-forecast-library-v10:${key}">${metric.label} ${metric.direction} <i>${arrow}</i></button></th>`; };
  const datasetCount = new Set(rows.map(row => row.item.datasetId)).size, experimentCount = new Set(rows.map(row => row.item.id)).size, context = `<div class="library-detail-context"><span><b>项目</b>${esc(owner.name)}</span>${scope.type !== 'project' ? `<span><b>数据集</b>${esc(set?.name || state.datasets[item?.datasetId]?.name || '数据集已删除')}</span>` : `<span><b>数据集</b>${datasetCount} 个</span>`}<span><b>模型实验</b>${scope.type === 'experiment' ? esc(item?.name || '模型已删除') : `${experimentCount} 个`}</span><span><b>已保存方案</b>${rows.length} 个</span></div>`, risk = scope.type === 'project' && allDatasetIds.length > 1 ? '<div class="library-risk-note"><b>跨数据集比较风险</b><span>不同数据集的历史范围、目标尺度和预测范围可能不同，请优先在同一数据集内比较。</span></div>' : '', configs = new Set(rows.map(row => `${row.result.frequency || state.datasets[row.item.datasetId]?.frequency}|${row.result.forecastConfig?.horizon || row.result.forecast?.length}`)), configWarning = configs.size > 1 ? '<div class="notice"><b>预测配置不同</b><span>当前范围包含不同频率或预测长度，指标仅供查看，不给出跨配置全局最佳结论。</span></div>' : '';
  const body = rows.length ? rows.map(({ item: rowItem, result }) => { const leadingCells = `${scope.type === 'project' ? `<td>${esc(state.datasets[rowItem.datasetId]?.name || '数据集已删除')}</td>` : ''}${scope.type !== 'experiment' ? `<td>${esc(rowItem.name)}</td><td>${esc(result.modelDisplayName || modelName(rowItem.type))}</td>` : ''}`; return `<tr>${leadingCells}<td class="library-scheme-column-v101"><b>方案 #${result.id}</b><small>${esc(result.params)}</small></td><td>${result.forecastConfig?.horizon || result.forecast?.length || '—'} 期</td>${metrics.map(key => `<td class="result-metric-column-v101"><b>${forecastMetricValueV10(result, key)}</b></td>`).join('')}<td>${esc(result.cutoff || '—')}</td><td>${esc(result.generatedAt || savedResultCreatedAtV7(rowItem, result))}</td><td class="result-action-column-v101"><div class="row-actions">${button('查看训练结果', `library-result:${rowItem.id}:${result.id}`)}${button('查看模型报告', `library-report-v7:${rowItem.id}:${result.id}`, 'primary')}${button('生成 API 配置', `open-api-config:${rowItem.id}:${result.id}`)}</div></td></tr>`; }).join('') : `<tr><td colspan="${metrics.length + 9}"><div class="empty-state"><p>当前筛选范围暂无已保存结果。</p></div></td></tr>`;
  return `${context}${risk}${configWarning}<div class="library-compact-tools">${forecastMetricPickerV10('library')}<span class="validation-sort-note">筛选集成在表头；指标按当前列排序</span></div><div class="table-card forecast-table-scroll-v102"><table class="forecast-library-table-v10"><thead><tr>${leading}${libraryDetailHeaderV909('参数方案', 'scheme')}<th><button class="metric-sort-v10" data-action="sort-forecast-library-v10:horizon">预测范围 ↕</button></th>${metrics.map(header).join('')}<th><button class="metric-sort-v10" data-action="sort-forecast-library-v10:cutoff">数据截止 ↕</button></th><th><button class="metric-sort-v10" data-action="sort-forecast-library-v10:createdAt">创建时间 ↕</button></th><th class="result-action-column-v101">操作</th></tr></thead><tbody>${body}</tbody></table></div>`;
};
forecastResolveFutureValueV1011 = function () { return ''; };
forecastApiDefaultRowsV1011 = function (set, result, horizon, key) { const features = result.externalFeatures || [], stored = state.forecastApiInputsV1011[key] || [], rows = result.forecast.slice(0, horizon).map((point, index) => { const existing = stored[index] || {}, values = {}; features.forEach(name => { values[name] = existing.values?.[name] ?? ''; }); return { date: point.date, timestamp: point.timestamp, values }; }); state.forecastApiInputsV1011[key] = cloneV7(rows); return rows; };
function forecastApiFeatureInputV102(set, row, rowIndex, name) {
  const column = set.columns.find(item => item.name === name), value = row.values[name] ?? '', attributes = `data-forecast-api-feature-row="${rowIndex}" data-forecast-api-feature-name="${esc(name)}" name="forecast-api-${rowIndex}-${esc(name)}" aria-label="${esc(row.date)}的${esc(name)}"`;
  if (column?.type === 'boolean') return `<select ${attributes}><option value="">请选择</option><option value="是" ${value === '是' ? 'selected' : ''}>是</option><option value="否" ${value === '否' ? 'selected' : ''}>否</option></select>`;
  if (column?.type === 'category') { const choices = [...new Set(forecastValuesV10(set, name).filter(item => String(item ?? '').trim()).map(String))].slice(0, 30); return `<select ${attributes}><option value="">请选择</option>${choices.map(choice => `<option value="${esc(choice)}" ${String(value) === choice ? 'selected' : ''}>${esc(choice)}</option>`).join('')}</select>`; }
  return `<input ${attributes} autocomplete="off" type="${column?.type === 'number' ? 'number' : 'text'}" ${column?.type === 'number' ? 'step="any"' : ''} value="${esc(value)}">`;
}
openForecastApiModalV10 = function (experimentId, resultId) {
  const item = state.experiments[experimentId], set = ensureForecastDatasetV10(state.datasets[item.datasetId]), snapshots = Object.values(state.savedResultSnapshots || {}).filter(snapshot => snapshot.experimentId === item.id), resultIds = snapshots.length ? snapshots.map(snapshot => snapshot.result.id) : item.results.map(result => result.id), uniqueIds = [...new Set(resultIds)], selected = forecastSavedResultV1011(item, resultId), externalCount = selected?.externalFeatures?.length || 0;
  modal(`<h2>生成 API 接入配置</h2><p id="forecast-api-requirement-v102">${externalCount ? `该方案使用 ${externalCount} 个外部特征；请求时需逐期提交完整未来特征。` : '该方案只使用目标历史；请求时填写预测期数。'}</p><label class="field"><span>服务名称</span><input id="api-service-name" autocomplete="off" value="${esc(item.name)} API" maxlength="80"></label><label class="field"><span>参数方案</span><select id="api-result-id">${uniqueIds.map(id => { const result = forecastSavedResultV1011(item, id); return `<option value="${id}" ${id === Number(resultId) ? 'selected' : ''}>#${id} · ${esc(result?.params || '参数方案')}</option>`; }).join('')}</select></label><label class="field"><span>默认预测期数</span><input id="forecast-api-horizon-v10" type="number" min="1" max="${selected?.forecast?.length || forecastHorizonLimitV102(set)}" value="${Math.min(set.forecastConfig.horizon, selected?.forecast?.length || set.forecastConfig.horizon)}"></label><div class="modal-actions">${button('取消', 'close-modal')}${button('确认生成', `confirm-forecast-api-v10:${experimentId}`, 'primary')}</div>`); bindDynamicModalV909();
  const select = document.querySelector('#api-result-id'); select?.addEventListener('change', () => { const result = forecastSavedResultV1011(item, Number(select.value)), count = result?.externalFeatures?.length || 0, input = document.querySelector('#forecast-api-horizon-v10'), note = document.querySelector('#forecast-api-requirement-v102'); if (input) { input.max = result?.forecast?.length || 1; input.value = Math.min(Number(input.value), Number(input.max)); } if (note) note.textContent = count ? `该方案使用 ${count} 个外部特征；请求时需逐期提交完整未来特征。` : '该方案只使用目标历史；请求时填写预测期数。'; });
};
forecastApiPageV10 = function () {
  const item = experiment(), set = ensureForecastDatasetV10(dataset()), config = [...state.apiConfigs].reverse().find(entry => entry.experimentId === item.id), result = forecastSavedResultV1011(item, config?.resultId || item.selected), horizon = Math.min(Math.max(1, Number(config?.horizon || set.forecastConfig.horizon)), result.forecast.length), key = config?.id || `${item.id}:${result.id}`, features = result.externalFeatures || [], rows = forecastApiDefaultRowsV1011(set, result, horizon, key), request = forecastApiRequestV1011(result, rows), response = state.forecastApiResponsesV1011?.[key] || forecastApiResponseForRowsV1011(result, rows), endpoint = `https://demo.ml-studio.local/v1/forecast/${item.id}`, featureTable = features.length ? `<div class="forecast-api-feature-table-v1011"><div class="section-title"><div><h3>未来外部特征</h3><p>这些值是预测请求输入，不属于上传的历史训练数据。</p></div><span>${features.length} 个特征</span></div><div class="table-card"><table><thead><tr><th>日期</th>${features.map(name => `<th>${esc(name)}</th>`).join('')}</tr></thead><tbody>${rows.map((row, rowIndex) => `<tr><td><b>${esc(row.date)}</b></td>${features.map(name => `<td>${forecastApiFeatureInputV102(set, row, rowIndex, name)}</td>`).join('')}</tr>`).join('')}</tbody></table></div></div>` : '';
  const json = JSON.stringify(request, null, 2), curl = `curl -X POST "${endpoint}" -H "Content-Type: application/json" -d '${JSON.stringify(request)}'`, python = `import requests\nresponse = requests.post("${endpoint}", json=${JSON.stringify(request)})\nprint(response.json())`, javascript = `const response = await fetch("${endpoint}", {\n  method: "POST",\n  headers: { "Content-Type": "application/json" },\n  body: JSON.stringify(${JSON.stringify(request)})\n});\nconsole.log(await response.json());`;
  return shell(`${pageHead('API 接入说明', `${esc(item.name)} · ${esc(result.params)}`)}<div class="api-grid forecast-api-grid-v1011"><section class="panel"><h2>接口信息</h2><div class="endpoint"><span>POST</span><code>${endpoint}</code>${button('复制', 'copy-endpoint')}</div><h3>请求 JSON</h3><pre>${esc(json)}</pre><h3>响应 JSON</h3><pre>${esc(JSON.stringify(response, null, 2))}</pre><details class="api-code-examples-v102"><summary>查看调用示例</summary><h4>cURL</h4><pre>${esc(curl)}</pre><h4>Python</h4><pre>${esc(python)}</pre><h4>JavaScript</h4><pre>${esc(javascript)}</pre></details><h3>响应字段</h3><div class="table-card"><table><tbody><tr><td><b>date</b></td><td>预测日期</td></tr><tr><td><b>prediction</b></td><td>预测值</td></tr><tr><td><b>interval80 / interval95</b></td><td>80% / 95% 预测区间</td></tr></tbody></table></div></section><section class="panel api-test"><h2>测试请求</h2><p>${features.length ? '填写预测期数以及每个未来日期的全部外部特征。' : '该模型只使用目标历史，填写预测期数即可。'}</p><label>预测未来多少期<input data-forecast-api-horizon type="number" min="1" max="${result.forecast.length}" value="${horizon}"></label>${featureTable}${button('发送模拟请求', 'test-forecast-api-v10', 'primary')}<div class="api-response"><b>已绑定不可变模型快照</b><span>${esc(result.modelDisplayName || modelName(item.type))} · 数据截止 ${esc(result.cutoff)}</span></div></section></div>`);
};

// V10.2 is declared before the historical V10.1 compatibility layers below.
// Capture the implementations here, then reactivate them once every legacy
// override has finished loading at the real end of this file.
const forecastSummaryV102 = forecastSummaryV10;
const ensureForecastDatasetV102 = ensureForecastDatasetV10;
const createForecastExampleV102 = createForecastExampleV10;
const buildForecastCsvDatasetV102 = buildForecastCsvDatasetV10;
const forecastDatasetPageV102 = forecastDatasetPageV10;
const datasetSourceModalV102 = datasetSourceModal;
const forecastFeatureStatsV102 = forecastFeatureStatsV101;
const forecastFeaturePageV102 = forecastFeaturePageV10;
const forecastThresholdFeedbackV102 = forecastThresholdFeedbackV1011;
const forecastExperimentPageV102 = forecastExperimentPageV10;
const startForecastTrainingV102 = startForecastTrainingV10;
const forecastTuningPageV102 = forecastTuningPageV10;
const forecastBacktestModalV102 = forecastBacktestModalV10;
const trainingSnapshotPanelV102 = trainingSnapshotPanelV910;
const forecastReportPageV102 = forecastReportPageV10;
const forecastLibraryDetailV102 = forecastLibraryDetailV10;
const forecastResolveFutureValueV102 = forecastResolveFutureValueV1011;
const forecastApiDefaultRowsV102 = forecastApiDefaultRowsV1011;
const openForecastApiModalV102 = openForecastApiModalV10;
const forecastApiPageV102 = forecastApiPageV10;

// ============================================================================
// V10.1.1 — forecasting consistency, immutable reports and full-feature API
// ============================================================================
state.forecastReportSnapshotKeyV1011 ||= null;
state.forecastApiInputsV1011 ||= {};
let forecastReportRenderResultV1011 = null;

function forecastHistoricalHandlingV1011(set, names, type) {
  if (!forecastUsesExternalV101(type)) return [];
  return names.map(name => {
    const column = set.columns.find(item => item.name === name), meta = column?.forecastMeta || (column ? forecastFeatureStatsV101(set, column) : null);
    if (!column || !meta || meta.missingRate <= 0) return null;
    const strategy = type === 'forecast_lgbm' ? '保留缺失并生成缺失标记' : column.type === 'number' ? '每轮训练窗口中位数填充' : '每轮训练窗口众数填充';
    return { name, type: column.type, missingRate: +(meta.missingRate * 100).toFixed(1), strategy };
  }).filter(Boolean);
}

const makeForecastResultBeforeV1011 = makeForecastResultV10;
makeForecastResultV10 = function (item, variant, index) {
  const result = makeForecastResultBeforeV1011(item, variant, index), set = ensureForecastDatasetV10(state.datasets[item.datasetId]);
  result.historicalMissingHandling = forecastHistoricalHandlingV1011(set, result.externalFeatures || [], item.type);
  return result;
};

forecastDisplayModelNameV101 = function (type, set = dataset()) {
  if (forecastReportRenderResultV1011?.modelDisplayName) return forecastReportRenderResultV1011.modelDisplayName;
  return type === 'forecast_arima' && forecastSelectedFeaturesV101(set).length ? 'ARIMAX' : modelName(type);
};

forecastProcessingSummaryV10 = function (type) {
  const set = ensureForecastDatasetV10(dataset()), snapshotResult = forecastReportRenderResultV1011, selected = snapshotResult ? (snapshotResult.externalFeatures || []).map(name => set.columns.find(column => column.name === name)).filter(Boolean) : forecastSelectedFeaturesV101(set), names = snapshotResult ? snapshotResult.externalFeatures || [] : selected.map(column => column.name), usesExternal = forecastUsesExternalV101(type) && names.length;
  const featureText = usesExternal ? names.join('、') : type === 'forecast_baseline' || type === 'forecast_ets' ? '该模型仅使用目标历史，不读取外部特征' : '当前未纳入外部特征';
  const missingHandling = snapshotResult?.historicalMissingHandling || forecastHistoricalHandlingV1011(set, names, type), missingText = !usesExternal ? '不适用' : missingHandling.length ? missingHandling.map(item => `${item.name}：${item.strategy}`).join('；') : '所选外部特征的历史区间无缺失';
  const rows = {
    forecast_baseline: [['模型比较', '历史均值、最后值、季节性最后值、漂移、移动平均'], ['外部特征', featureText]],
    forecast_ets: [['模型结构', '水平、趋势与季节性组件'], ['外部特征', featureText]],
    forecast_arima: [['模型结构', usesExternal ? 'ARIMAX：自回归、差分、移动平均与外部特征' : 'ARIMA：自回归、差分与移动平均'], ['使用特征', featureText], ['历史缺失处理', missingText], ['自动处理', '仅在每个训练窗口内确定填充值、差分与阶数']],
    forecast_prophet: [['模型结构', '趋势、季节性、日历效应与外部回归量'], ['使用特征', featureText], ['历史缺失处理', missingText], ['自动处理', '每轮训练窗口单独确定填充值、变点范围与季节性']],
    forecast_lgbm: [['模型输入', '目标滞后、滚动统计、趋势序号、日历特征与外部特征'], ['使用特征', featureText], ['历史缺失处理', missingText], ['自动处理', '每轮回测只使用当时已知的信息生成特征']]
  }[type] || [];
  return `<div class="forecast-processing-v10">${rows.map(row => `<div><b>${row[0]}</b><span>${esc(row[1])}</span></div>`).join('')}</div>`;
};

const forecastReportPageBeforeV1011 = forecastReportPageV10;
forecastReportPageV10 = function () {
  const item = experiment(), snapshot = state.savedResultSnapshots?.[state.forecastReportSnapshotKeyV1011];
  if (!item || !snapshot?.result || snapshot.experimentId !== item.id) return forecastReportPageBeforeV1011();
  const originalResults = item.results, result = cloneV7(snapshot.result);
  item.results = [...(originalResults || []).filter(row => row.id !== result.id), result];
  forecastReportRenderResultV1011 = result;
  try { return forecastReportPageBeforeV1011(); }
  finally { forecastReportRenderResultV1011 = null; item.results = originalResults; }
};

const currentReportContextBeforeV1011 = currentReportContextV910;
currentReportContextV910 = function () {
  const context = currentReportContextBeforeV1011(), snapshot = state.savedResultSnapshots?.[state.forecastReportSnapshotKeyV1011];
  return context && snapshot?.result ? { ...context, result: snapshot.result, key: state.forecastReportSnapshotKeyV1011 } : context;
};

const forecastExperimentsPageBeforeV1011 = forecastExperimentsPageV10;
forecastExperimentsPageV10 = function () {
  const set = ensureForecastDatasetV10(dataset()), label = forecastDisplayModelNameV101('forecast_arima', set);
  return forecastExperimentsPageBeforeV1011().replaceAll('<p>ARIMA ·', `<p>${label} ·`);
};

const goBeforeForecastV1011 = go;
go = function (page) { if (page !== 'report') state.forecastReportSnapshotKeyV1011 = null; return goBeforeForecastV1011(page); };

function forecastSavedResultV1011(item, resultId) {
  return state.savedResultSnapshots?.[`${item.id}:${Number(resultId)}`]?.result || item.results?.find(row => row.id === Number(resultId)) || item.results?.[0];
}

function forecastResolveFutureValueV1011(set, featureName, timestamp, step) {
  const times = forecastValuesV10(set, set.timeColumn), values = forecastValuesV10(set, featureName), directIndex = times.findIndex(value => parseForecastDateV10(value) === timestamp), direct = directIndex >= 0 ? values[directIndex] : '';
  if (String(direct ?? '').trim()) return direct;
  const policy = set.forecastFuturePolicies?.[featureName] || 'pending', column = set.columns.find(item => item.name === featureName), history = forecastHistoryContextV101(set).historyRows.map(row => values[row.index]).filter(value => String(value ?? '').trim());
  if (policy === 'recent') return history.at(-1) ?? '';
  if (policy === 'linear' && column?.type === 'number') { const last = Number(history.at(-1)), previous = Number(history.at(-2)); return Number.isFinite(last) && Number.isFinite(previous) ? +(last + (last - previous) * step).toFixed(3) : ''; }
  if (policy === 'zero') return column?.type === 'number' ? 0 : '默认值';
  return '';
}

function forecastApiDefaultRowsV1011(set, result, horizon, key) {
  const features = result.externalFeatures || [], stored = state.forecastApiInputsV1011[key] || [], rows = result.forecast.slice(0, horizon).map((point, index) => {
    const existing = stored[index] || {}, values = {};
    features.forEach(name => { values[name] = existing.values?.[name] ?? forecastResolveFutureValueV1011(set, name, point.timestamp, index + 1); });
    return { date: point.date, timestamp: point.timestamp, values };
  });
  state.forecastApiInputsV1011[key] = cloneV7(rows); return rows;
}

function forecastApiRequestV1011(result, rows) {
  const features = result.externalFeatures || [];
  if (!features.length) return { horizon: rows.length };
  return { horizon: rows.length, futureFeatures: rows.map(row => ({ date: row.date, ...row.values })) };
}

function forecastApiResponseForRowsV1011(result, rows) {
  const features = result.externalFeatures || [];
  return { mock: true, forecast: result.forecast.slice(0, rows.length).map((point, index) => { const row = rows[index], influence = features.reduce((sum, name, featureIndex) => { const raw = row.values[name], numeric = Number(raw); return sum + (Number.isFinite(numeric) ? Math.sin(numeric * .11 + featureIndex) * .32 : ((forecastHashV10(raw || name) % 9) - 4) * .07); }, 0), prediction = +(point.prediction + influence).toFixed(2), shift = prediction - point.prediction; return { date: point.date, prediction, interval80: [+(point.lower80 + shift).toFixed(2), +(point.upper80 + shift).toFixed(2)], interval95: [+(point.lower95 + shift).toFixed(2), +(point.upper95 + shift).toFixed(2)] }; }) };
}

openForecastApiModalV10 = function (experimentId, resultId) {
  const item = state.experiments[experimentId], set = state.datasets[item.datasetId], snapshots = Object.values(state.savedResultSnapshots || {}).filter(snapshot => snapshot.experimentId === item.id), resultIds = snapshots.length ? snapshots.map(snapshot => snapshot.result.id) : item.results.map(result => result.id), uniqueIds = [...new Set(resultIds)], selected = forecastSavedResultV1011(item, resultId), externalCount = selected?.externalFeatures?.length || 0;
  modal(`<h2>生成 API 接入配置</h2><p>${externalCount ? `该方案使用 ${externalCount} 个外部特征；请求时需逐期提交完整未来特征。` : '该方案只使用目标历史；请求时填写预测期数。'}</p><label class="field"><span>服务名称</span><input id="api-service-name" name="api-service-name" autocomplete="off" value="${esc(item.name)} API" maxlength="80"></label><label class="field"><span>参数方案</span><select id="api-result-id" name="api-result-id" autocomplete="off">${uniqueIds.map(id => { const result = forecastSavedResultV1011(item, id); return `<option value="${id}" ${id === Number(resultId) ? 'selected' : ''}>#${id} · ${esc(result?.params || '参数方案')}</option>`; }).join('')}</select></label><label class="field"><span>默认预测期数</span><input id="forecast-api-horizon-v10" name="forecast-api-horizon" type="number" min="1" max="${Math.max(1, selected?.forecast?.length || Math.floor(set.timeSummary.validPoints / 4))}" value="${Math.min(set.forecastConfig.horizon, selected?.forecast?.length || set.forecastConfig.horizon)}"></label><div class="modal-actions">${button('取消', 'close-modal')}${button('确认生成', `confirm-forecast-api-v10:${experimentId}`, 'primary')}</div>`);
  bindDynamicModalV909();
};

forecastApiPageV10 = function () {
  const item = experiment(), set = ensureForecastDatasetV10(dataset()), config = [...state.apiConfigs].reverse().find(entry => entry.experimentId === item.id), result = forecastSavedResultV1011(item, config?.resultId || item.selected), horizon = Math.min(Math.max(1, Number(config?.horizon || set.forecastConfig.horizon)), result.forecast.length), key = config?.id || `${item.id}:${result.id}`, features = result.externalFeatures || [], rows = forecastApiDefaultRowsV1011(set, result, horizon, key), request = forecastApiRequestV1011(result, rows), response = state.forecastApiResponsesV1011?.[key] || forecastApiResponseForRowsV1011(result, rows), featureTable = features.length ? `<div class="forecast-api-feature-table-v1011"><div class="section-title"><div><h3>未来外部特征</h3><p>每个预测日期都必须提供完整特征；修改预测期数后表格会同步刷新。</p></div><span>${features.length} 个特征</span></div><div class="table-card"><table><thead><tr><th>日期</th>${features.map(name => `<th>${esc(name)}</th>`).join('')}</tr></thead><tbody>${rows.map((row, rowIndex) => `<tr><td><b>${esc(row.date)}</b></td>${features.map(name => { const column = set.columns.find(item => item.name === name), type = column?.type === 'number' ? 'number' : 'text'; return `<td><input data-forecast-api-feature-row="${rowIndex}" data-forecast-api-feature-name="${esc(name)}" name="forecast-api-${rowIndex}-${esc(name)}" aria-label="${esc(row.date)}的${esc(name)}" autocomplete="off" type="${type}" ${type === 'number' ? 'step="any"' : ''} value="${esc(row.values[name] ?? '')}"></td>`; }).join('')}</tr>`).join('')}</tbody></table></div></div>` : '';
  return shell(`${pageHead('API 接入说明', `${esc(item.name)} · ${esc(result.params)}`)}<div class="api-grid forecast-api-grid-v1011"><section class="panel"><h2>接口信息</h2><div class="endpoint"><span>POST</span><code>https://demo.ml-studio.local/v1/forecast/${item.id}</code></div><h3>请求 JSON</h3><pre>${esc(JSON.stringify(request, null, 2))}</pre><h3>响应 JSON</h3><pre>${esc(JSON.stringify(response, null, 2))}</pre></section><section class="panel api-test"><h2>测试请求</h2><p>${features.length ? '填写预测期数以及每个未来日期的全部外部特征。' : '该模型只使用目标历史，填写预测期数即可。'}</p><label>预测未来多少期<input data-forecast-api-horizon name="forecast-api-horizon-test" type="number" min="1" max="${result.forecast.length}" value="${horizon}"></label>${featureTable}${button('发送模拟请求', 'test-forecast-api-v10', 'primary')}<div class="api-response"><b>已绑定不可变模型快照</b><span>${esc(result.modelDisplayName || modelName(item.type))} · 数据截止 ${esc(result.cutoff)}</span></div></section></div>`);
};

function forecastThresholdFeedbackV1011(input) {
  const key = input.dataset.forecastThreshold, limits = { missing: '请输入 0–99', variance: '请输入 0–0.25', futureCoverage: '请输入 0–100' }, invalid = !input.value.trim() || !input.checkValidity(), label = input.closest('label');
  input.classList.toggle('range-invalid', invalid); input.setAttribute('aria-invalid', String(invalid));
  let message = label?.querySelector('.threshold-range-error');
  if (!message && label) { message = document.createElement('small'); message.className = 'threshold-range-error'; label.appendChild(message); }
  if (message) { message.textContent = invalid ? limits[key] : ''; message.hidden = !invalid; }
  return !invalid;
}

const actionBeforeForecastV1011 = action;
action = function (name) {
  if (name.startsWith('library-report-v7:')) {
    const [, experimentId, resultId] = name.split(':'), item = state.experiments[experimentId], owner = state.projects.find(entry => entry.id === item?.projectId), key = `${experimentId}:${Number(resultId)}`;
    if (isForecastingV10(owner) && state.savedResultSnapshots?.[key]) {
      if (!item || !state.datasets[item.datasetId]) return toast('模型实验或数据集已不存在，无法打开报告。', 'error');
      state.projectId = item.projectId; state.datasetId = item.datasetId; state.experimentId = item.id; item.selected = Number(resultId); state.forecastReportSnapshotKeyV1011 = key; state.reportReturnContextV911 = reportReturnContextV911('models'); save(); return go('report');
    }
  }
  if (isForecastingV10() && (name.startsWith('report-result:') || name === 'go-report')) state.forecastReportSnapshotKeyV1011 = null;
  if (isForecastingV10() && name === 'forecast-recommend-thresholds-v101') {
    const defaults = { missing: 90, variance: .01, futureCoverage: 80 };
    app.querySelectorAll('[data-forecast-threshold]').forEach(input => { input.value = defaults[input.dataset.forecastThreshold]; forecastThresholdFeedbackV1011(input); });
    return toast('已填入推荐值，点击“应用筛选”后生效。', 'success');
  }
  if (isForecastingV10() && name === 'forecast-apply-thresholds-v101') {
    const set = ensureForecastDatasetV10(dataset()), inputs = [...app.querySelectorAll('[data-forecast-threshold]')], valid = inputs.map(forecastThresholdFeedbackV1011);
    if (valid.some(value => !value)) { inputs[valid.findIndex(value => !value)]?.focus(); return toast('请修正标红的筛选阈值。', 'warning'); }
    const values = Object.fromEntries(inputs.map(input => [input.dataset.forecastThreshold, Number(input.value)])); set.forecastFeatureThresholds = values;
    forecastExternalColumnsV101(set).forEach(column => { const meta = forecastFeatureStatsV101(set, column), policyReady = meta.futureCoverage >= 100 || ['recent', 'linear', 'zero'].includes(set.forecastFuturePolicies[column.name]), passes = meta.trainable && meta.missingRate * 100 <= values.missing && (meta.variance === null || meta.variance >= values.variance) && meta.futureCoverage >= values.futureCoverage && policyReady; column.forecastMeta = meta; column.included = passes; });
    set.feature.revision = (set.feature.revision || 0) + 1; markForecastExperimentsStaleV10(set); save(); render(); return setTimeout(() => toast('已按当前阈值和缺失策略更新外部特征。', 'success'), 0);
  }
  if (name.startsWith('confirm-forecast-api-v10:')) {
    const experimentId = name.split(':')[1], item = state.experiments[experimentId], serviceName = document.querySelector('#api-service-name')?.value.trim(), resultId = Number(document.querySelector('#api-result-id')?.value), result = forecastSavedResultV1011(item, resultId), horizon = Number(document.querySelector('#forecast-api-horizon-v10')?.value);
    if (!serviceName) return toast('请输入服务名称。', 'warning');
    if (!result || !Number.isInteger(horizon) || horizon < 1 || horizon > result.forecast.length) return toast(`预测期数请输入 1–${result?.forecast?.length || 1}。`, 'warning');
    const id = uid('api'), key = `${item.id}:${result.id}`; state.apiConfigs.push({ id, name: serviceName, experimentId, resultId: result.id, horizon, createdAt: now(), task: 'forecasting', snapshotKey: state.savedResultSnapshots?.[key] ? key : null, externalFeatures: cloneV7(result.externalFeatures || []), requestMode: result.externalFeatures?.length ? 'full-future-features' : 'horizon-only' });
    document.querySelector('.modal-backdrop')?.remove(); state.libraryTab = 'api'; state.libraryDetailScope = { type: 'experiment', id: item.id }; state.forecastApiInputsV1011[id] = forecastApiDefaultRowsV1011(state.datasets[item.datasetId], result, horizon, id); save(); return render();
  }
  if (isForecastingV10() && name === 'test-forecast-api-v10') {
    const item = experiment(), config = [...state.apiConfigs].reverse().find(entry => entry.experimentId === item.id), result = forecastSavedResultV1011(item, config?.resultId || item.selected), horizon = Number(app.querySelector('[data-forecast-api-horizon]')?.value), key = config?.id || `${item.id}:${result.id}`;
    if (!Number.isInteger(horizon) || horizon < 1 || horizon > result.forecast.length) return toast(`预测期数请输入 1–${result.forecast.length}。`, 'warning');
    const rows = forecastApiDefaultRowsV1011(dataset(), result, horizon, key), inputs = [...app.querySelectorAll('[data-forecast-api-feature-row]')], invalid = [];
    inputs.forEach(input => { const rowIndex = Number(input.dataset.forecastApiFeatureRow), name = input.dataset.forecastApiFeatureName, column = dataset().columns.find(item => item.name === name), value = input.value.trim(), bad = !value || (column?.type === 'number' && !Number.isFinite(Number(value))); input.classList.toggle('range-invalid', bad); input.setAttribute('aria-invalid', String(bad)); if (bad) invalid.push(input); else rows[rowIndex].values[name] = column?.type === 'number' ? Number(value) : value; });
    if (invalid.length) { invalid[0].focus(); return toast('请填写每个未来日期的全部外部特征。', 'warning'); }
    state.forecastApiInputsV1011[key] = cloneV7(rows); state.forecastApiResponsesV1011 ||= {}; state.forecastApiResponsesV1011[key] = forecastApiResponseForRowsV1011(result, rows); if (config) config.horizon = horizon; save(); render(); return setTimeout(() => toast('模拟请求已完成。', 'success'), 0);
  }
  return actionBeforeForecastV1011(name);
};

const bindBeforeForecastV1011 = bind;
bind = function () {
  bindBeforeForecastV1011();
  if (!isForecastingV10()) return;
  app.querySelectorAll('[data-forecast-threshold]').forEach(input => input.addEventListener('input', () => forecastThresholdFeedbackV1011(input)));
  app.querySelector('[data-forecast-api-horizon]')?.addEventListener('change', event => {
    const item = experiment(), config = [...state.apiConfigs].reverse().find(entry => entry.experimentId === item.id), result = forecastSavedResultV1011(item, config?.resultId || item.selected), value = Number(event.target.value);
    if (!Number.isInteger(value) || value < 1 || value > result.forecast.length) { toast(`预测期数请输入 1–${result.forecast.length}。`, 'warning'); return render(); }
    if (config) config.horizon = value; state.forecastApiResponsesV1011 ||= {}; delete state.forecastApiResponsesV1011[config?.id || `${item.id}:${result.id}`]; save(); render();
  });
};
...
// ============================================================================
// V10.1.0 — external forecasting features and shared result-table layout
// ============================================================================
function forecastPearsonV101(first, second) {
  if (first.length < 3 || first.length !== second.length) return null;
  const firstMean = first.reduce((sum, value) => sum + value, 0) / first.length, secondMean = second.reduce((sum, value) => sum + value, 0) / second.length;
  let numerator = 0, firstSquare = 0, secondSquare = 0;
  first.forEach((value, index) => { const firstDelta = value - firstMean, secondDelta = second[index] - secondMean; numerator += firstDelta * secondDelta; firstSquare += firstDelta ** 2; secondSquare += secondDelta ** 2; });
  const denominator = Math.sqrt(firstSquare * secondSquare);
  return denominator ? numerator / denominator : 0;
}

function forecastHistoryContextV101(set) {
  const times = forecastValuesV10(set, set.timeColumn), targets = forecastValuesV10(set, set.target), rows = times.map((value, index) => ({ index, timestamp: parseForecastDateV10(value), rawTarget: targets[index], target: Number(targets[index]) }));
  const validTargetRows = rows.filter(row => Number.isFinite(row.timestamp) && String(row.rawTarget ?? '').trim() && Number.isFinite(row.target));
  const cutoff = validTargetRows.length ? Math.max(...validTargetRows.map(row => row.timestamp)) : NaN;
  return { rows, cutoff, historyRows: rows.filter(row => Number.isFinite(row.timestamp) && row.timestamp <= cutoff), futureRows: rows.filter(row => Number.isFinite(row.timestamp) && row.timestamp > cutoff) };
}

forecastSummaryV10 = function (set) {
  const times = forecastValuesV10(set, set.timeColumn), targets = forecastValuesV10(set, set.target), context = forecastHistoryContextV101(set);
  const validTimes = times.map(parseForecastDateV10).filter(Number.isFinite).sort((a, b) => a - b), uniqueTimes = [...new Set(validTimes)], inferred = inferForecastFrequencyV10(times), frequency = set.frequency || inferred;
  const invalidTimeCount = Math.max(0, times.length - validTimes.length), duplicateCount = Math.max(0, validTimes.length - uniqueTimes.length);
  const invalidTargetCount = context.historyRows.filter(row => String(row.rawTarget ?? '').trim() && !Number.isFinite(row.target)).length;
  const missingTargetCount = context.historyRows.filter(row => !String(row.rawTarget ?? '').trim()).length;
  let missingPointCount = 0;
  if (uniqueTimes.length > 1) { const available = new Set(uniqueTimes); for (let cursor = addForecastStepV10(uniqueTimes[0], frequency); cursor < uniqueTimes.at(-1); cursor = addForecastStepV10(cursor, frequency)) { if (!available.has(cursor)) missingPointCount += 1; if (missingPointCount > 10000) break; } }
  const minimum = FORECAST_FREQUENCIES_V10[frequency]?.min || 60, validPoints = context.historyRows.filter(row => String(row.rawTarget ?? '').trim() && Number.isFinite(row.target)).length, blockers = [];
  if (invalidTimeCount) blockers.push(`${invalidTimeCount} 个时间值无法解析`);
  if (duplicateCount) blockers.push(`${duplicateCount} 个重复时间点`);
  if (invalidTargetCount) blockers.push(`${invalidTargetCount} 个历史目标值不是数值`);
  if (missingTargetCount && set.forecastConfig?.missingTargetPolicy === 'pending') blockers.push(`${missingTargetCount} 个历史目标值尚未选择处理方式`);
  if (missingPointCount && set.forecastConfig?.missingTimePolicy === 'pending') blockers.push(`${missingPointCount} 个缺失时间点尚未选择处理方式`);
  if (validPoints < minimum) blockers.push(`有效历史时间点少于 ${minimum} 个`);
  if (!Number.isFinite(context.cutoff)) blockers.push('没有可训练的历史目标值');
  return { inferred, frequency, invalidTimeCount, duplicateCount, invalidTargetCount, missingTargetCount, missingPointCount, validPoints, minimum, start: context.historyRows[0]?.timestamp, end: context.cutoff, totalEnd: uniqueTimes.at(-1), futureRows: context.futureRows.length, blockers };
};

function forecastExternalColumnsV101(set) { return set.columns.filter(column => column.name !== set.timeColumn && column.name !== set.target && !column.target); }
function forecastFeatureStatsV101(set, column) {
  const context = forecastHistoryContextV101(set), values = forecastValuesV10(set, column.name), targets = forecastValuesV10(set, set.target), historyIndexes = context.historyRows.map(row => row.index), historyRaw = historyIndexes.map(index => values[index]), historyNonempty = historyRaw.filter(value => String(value ?? '').trim());
  const missingRate = 1 - historyNonempty.length / Math.max(1, historyRaw.length), numericValues = historyRaw.map(Number).filter(Number.isFinite);
  let variance = null;
  if (column.type === 'number' && numericValues.length) { const min = Math.min(...numericValues), max = Math.max(...numericValues), normalized = numericValues.map(value => max === min ? 0 : (value - min) / (max - min)), mean = normalized.reduce((sum, value) => sum + value, 0) / normalized.length; variance = normalized.reduce((sum, value) => sum + (value - mean) ** 2, 0) / normalized.length; }
  let temporalCorrelation = null, bestLag = null;
  if (column.type === 'number') {
    const maximumLag = Math.min(14, Math.max(0, Math.floor(historyIndexes.length / 4)));
    for (let lag = 0; lag <= maximumLag; lag += 1) {
      const featureSeries = [], targetSeries = [];
      for (let position = lag; position < historyIndexes.length; position += 1) { const featureValue = Number(values[historyIndexes[position - lag]]), targetValue = Number(targets[historyIndexes[position]]); if (Number.isFinite(featureValue) && Number.isFinite(targetValue)) { featureSeries.push(featureValue); targetSeries.push(targetValue); } }
      const score = forecastPearsonV101(featureSeries, targetSeries); if (score !== null && (temporalCorrelation === null || Math.abs(score) > Math.abs(temporalCorrelation))) { temporalCorrelation = score; bestLag = lag; }
    }
  }
  const timeValues = forecastValuesV10(set, set.timeColumn), byTime = new Map(timeValues.map((value, index) => [parseForecastDateV10(value), values[index]])), horizon = set.forecastConfig.horizon, futureValues = [];
  for (let step = 1; step <= horizon; step += 1) futureValues.push(byTime.get(addForecastStepV10(context.cutoff, set.frequency, step)));
  const futureComplete = futureValues.filter(value => String(value ?? '').trim()).length, futureCoverage = futureComplete / Math.max(1, horizon) * 100;
  return { missingRate, variance, temporalCorrelation, bestLag, futureCoverage, futureMissing: horizon - futureComplete, trainable: !['text', 'date'].includes(column.type) };
}
...
ensureForecastDatasetV10 = function (set) {
  ...
  set.timeColumn ||= set.columns.find(column => column.type === 'date')?.name || set.columns[0]?.name;
  set.target ||= set.columns.find(column => column.type === 'number' && column.name !== set.timeColumn)?.name || set.columns.at(-1)?.name;
  set.frequency ||= inferForecastFrequencyV10(forecastValuesV10(set, set.timeColumn)); set.feature ||= { revision: 1 }; set.forecastConfig ||= {};
  set.forecastConfig.horizon = Math.max(1, Number(set.forecastConfig.horizon || 7)); set.forecastConfig.backtestRounds = 3; set.forecastConfig.missingTimePolicy ||= 'pending'; set.forecastConfig.missingTargetPolicy ||= 'pending'; set.forecastConfig.calendarFeatures = set.forecastConfig.calendarFeatures !== false;
  set.forecastFeatureThresholds ||= { missing: 90, variance: .01, futureCoverage: 80 }; set.forecastFuturePolicies ||= {};
  set.columns.forEach(column => { column.target = column.name === set.target; if (column.name === set.timeColumn || column.target) { column.included = false; column.trainable = false; return; } column.trainable = !['text', 'date'].includes(column.type); if (column.included === undefined) column.included = column.trainable; });
  set.timeSummary = forecastSummaryV10(set);
  const targetColumn = set.columns.find(column => column.name === set.target), historyCount = forecastHistoryContextV101(set).historyRows.length;
  if (targetColumn) targetColumn.missing = set.timeSummary.missingTargetCount / Math.max(1, historyCount);
  forecastExternalColumnsV101(set).forEach(column => {
    column.forecastMeta = forecastFeatureStatsV101(set, column);
    if (column.forecastMeta.futureCoverage < 100) {
      if (!['recent', 'linear', 'zero'].includes(set.forecastFuturePolicies[column.name])) set.forecastFuturePolicies[column.name] = 'pending';
      if (set.forecastFuturePolicies[column.name] === 'pending') column.included = false;
    } else set.forecastFuturePolicies[column.name] = 'none';
  });
  return set;
};
...
createForecastExampleV10 = function () {
  const owner = project(), id = uid('d'), start = Date.UTC(2025, 4, 1), preview = [], dates = [], sales = [], prices = [], promotions = [], advertising = [], holidays = [];
  for (let index = 0; index < 372; index += 1) {
    const timestamp = start + index * 86400000, date = new Date(timestamp), weekend = [0, 6].includes(date.getUTCDay()), promotion = index % 19 < 3 || weekend, holiday = index % 61 === 0 ? '节假日' : '普通日', price = +(13.8 + 1.1 * Math.sin(index * Math.PI * 2 / 45) - (promotion ? 1.2 : 0)).toFixed(2), ad = +(20 + 8 * Math.sin(index * Math.PI * 2 / 28) + (promotion ? 12 : 0)).toFixed(1);
    const target = index < 365 ? Math.round(92 + index * .05 + 10 * Math.sin(index * Math.PI * 2 / 7) - price * 2.1 + (promotion ? 19 : 0) + ad * .32 + (holiday === '节假日' ? 12 : 0) + ((index * 17) % 9 - 4) * .6) : '';
    const dateText = formatForecastDateV10(timestamp); dates.push(dateText); sales.push(target); prices.push(price); promotions.push(promotion ? '是' : '否'); advertising.push(ad); holidays.push(holiday); preview.push([dateText, target, price, promotion ? '是' : '否', ad, holiday]);
  }
  const columns = [
    { name: '日期', type: 'date', missing: 0, unique: 372, trainable: false, included: false },
    { name: '销量', type: 'number', missing: 0, unique: new Set(sales.filter(value => value !== '')).size, trainable: false, target: true, included: false },
    { name: '商品价格', type: 'number', missing: 0, unique: new Set(prices).size, trainable: true, included: true },
    { name: '是否促销', type: 'boolean', missing: 0, unique: 2, trainable: true, included: true },
    { name: '广告费用', type: 'number', missing: 0, unique: new Set(advertising).size, trainable: true, included: true },
    { name: '节假日类型', type: 'category', missing: 0, unique: 2, trainable: true, included: true }
  ];
  const set = { id, projectId: owner.id, name: '商品每日销量示例', uploadedAt: now(), rows: 372, timeColumn: '日期', target: '销量', frequency: 'day', columns, preview: preview.slice(0, 150), forecastValues: { 日期: dates, 销量: sales, 商品价格: prices, 是否促销: promotions, 广告费用: advertising, 节假日类型: holidays }, experiments: [], feature: { revision: 1 }, forecastConfig: { horizon: 7, backtestRounds: 3, missingTimePolicy: 'none', missingTargetPolicy: 'none', calendarFeatures: true }, forecastFeatureThresholds: { missing: 90, variance: .01, futureCoverage: 80 }, forecastFuturePolicies: { 商品价格: 'none', 是否促销: 'none', 广告费用: 'none', 节假日类型: 'none' }, dataSource: 'example-v101' };
  ensureForecastDatasetV10(set); state.datasets[id] = set; owner.datasets.push(id); owner.updatedAt = now(); state.datasetId = id; state.featureStep = 0; state.previewPagesV910[id] = 0; save(); go('dataset'); setTimeout(() => toast('已创建包含外部特征和未来 7 天计划数据的销量示例。', 'success'), 0);
};
...
buildForecastCsvDatasetV10 = function (rows, name) {
  ...
  const data = rows.slice(1).filter(row => row.some(value => String(value ?? '').trim()));
  const columns = headers.map((header, index) => { const values = data.map(row => String(row[index] ?? '').trim()), nonempty = values.filter(Boolean), dateRate = nonempty.filter(value => Number.isFinite(parseForecastDateV10(value))).length / Math.max(1, nonempty.length), numberRate = nonempty.filter(value => Number.isFinite(Number(value))).length / Math.max(1, nonempty.length), unique = new Set(nonempty).size; const type = dateRate >= .9 ? 'date' : numberRate >= .9 ? 'number' : unique <= Math.max(20, nonempty.length * .1) ? (unique === 2 ? 'boolean' : 'category') : 'text'; return { name: header, type, missing: 1 - nonempty.length / Math.max(1, data.length), unique, trainable: !['text', 'date'].includes(type), included: !['text', 'date'].includes(type) }; });
  const timeColumn = columns.find(column => column.type === 'date')?.name || headers[0], numericCandidates = columns.filter(column => column.type === 'number' && column.name !== timeColumn), target = numericCandidates.at(-1)?.name;
  if (!target) throw new Error('未识别到可用的数值目标列，请检查 CSV。'); columns.find(column => column.name === target).target = true; columns.find(column => column.name === target).included = false;
  const forecastValues = Object.fromEntries(headers.map((header, index) => [header, data.map(row => row[index] ?? '')])), id = uid('d');
  return ensureForecastDatasetV10({ id, projectId: project().id, name: name.replace(/\.csv$/i, ''), uploadedAt: now(), rows: data.length, timeColumn, target, frequency: inferForecastFrequencyV10(forecastValues[timeColumn]), columns, preview: data.slice(0, 150), forecastValues, experiments: [], feature: { revision: 1 }, forecastConfig: { horizon: 7, backtestRounds: 3, missingTimePolicy: 'pending', missingTargetPolicy: 'pending', calendarFeatures: true }, forecastFeatureThresholds: { missing: 90, variance: .01, futureCoverage: 80 }, forecastFuturePolicies: {}, dataSource: 'upload' });
};

function forecastFuturePreviewV101(set) {
  const context = forecastHistoryContextV101(set), columns = [set.timeColumn, set.target, ...forecastExternalColumnsV101(set).map(column => column.name)], horizon = Math.max(1, Number(set.forecastConfig.horizon)), rows = context.futureRows.slice(0, horizon);
  if (!rows.length) return `<section class="section-block forecast-future-preview-v101"><div class="section-title"><div><h2>未来特征数据</h2><p>当前文件只包含历史数据；实际预测时需补充已选择外部特征的未来值。</p></div><span class="status muted">尚未提供</span></div></section>`;
  return `<section class="section-block forecast-future-preview-v101"><div class="section-title"><div><h2>未来特征数据</h2><p>目标列留空表示待预测；其他列是模型预测时可使用的未来已知信息。</p></div><span class="status good">已提供 ${rows.length} 期</span></div><div class="table-card"><table><thead><tr>${columns.map(name => `<th>${esc(name)}${name === set.target ? '（待预测）' : ''}</th>`).join('')}</tr></thead><tbody>${rows.map(row => `<tr>${columns.map(name => `<td>${name === set.target ? '<span class="status muted">待预测</span>' : esc(forecastValuesV10(set, name)[row.index] ?? '—')}</td>`).join('')}</tr>`).join('')}</tbody></table></div></section>`;
}

const forecastDatasetPageBeforeV101 = forecastDatasetPageV10;
forecastDatasetPageV10 = function () {
  const set = ensureForecastDatasetV10(dataset()), summary = set.timeSummary, external = forecastExternalColumnsV101(set), content = forecastDatasetPageBeforeV101();
  const overview = `<div class="forecast-summary-strip-v101"><span><b>${summary.validPoints}</b>历史训练点</span><span><b>${summary.futureRows}</b>未来数据点</span><span><b>${external.length}</b>外部特征</span><span><b>1</b>预测目标</span></div>`;
  return content
    .replace('· 单序列时序预测', '· 单序列、多变量时序预测')
    .replace('<div class="forecast-summary-strip-v10">', `${overview}<div class="forecast-summary-strip-v10">`)
    .replace('目标列缺失', '历史目标缺失')
    .replace(previewSectionV910(set), `${forecastFuturePreviewV101(set)}${previewSectionV910(set)}`);
};

function forecastCorrelationMatrixV101(set, columns) {
  const numeric = columns.filter(column => column.type === 'number').slice(0, 8), context = forecastHistoryContextV101(set);
  if (numeric.length < 2) return `<div class="notice"><b>特征相关性热力图</b><span>至少需要两个数值型外部特征才能计算。</span></div>`;
  const values = Object.fromEntries(numeric.map(column => [column.name, forecastValuesV10(set, column.name)]));
  const score = (first, second) => { const x = [], y = []; context.historyRows.forEach(row => { const a = Number(values[first][row.index]), b = Number(values[second][row.index]); if (Number.isFinite(a) && Number.isFinite(b)) { x.push(a); y.push(b); } }); return forecastPearsonV101(x, y) ?? 0; };
  return `<details class="forecast-correlation-v101"><summary>查看特征相关性热力图</summary><p>仅显示数值型外部特征，颜色越深表示线性相关越强。</p><div class="forecast-correlation-scroll-v101"><table><thead><tr><th></th>${numeric.map(column => `<th>${esc(column.name)}</th>`).join('')}</tr></thead><tbody>${numeric.map(first => `<tr><th>${esc(first.name)}</th>${numeric.map(second => { const value = first === second ? 1 : score(first.name, second.name), strength = Math.max(.08, Math.abs(value)), color = strength >= .55 ? '#fff' : '#173259'; return `<td style="--corr:${strength.toFixed(2)};color:${color}" title="${esc(first.name)} × ${esc(second.name)}：${value.toFixed(2)}">${value.toFixed(2)}</td>`; }).join('')}</tr>`).join('')}</tbody></table></div></details>`;
}

function forecastPolicyOptionsV101(column, value) {
  const numeric = column.type === 'number';
  return `<select data-forecast-future-policy="${esc(column.name)}" name="forecast-future-policy-${esc(column.name)}" aria-label="${esc(column.name)}的未来缺失处理方式" autocomplete="off"><option value="pending" ${value === 'pending' ? 'selected' : ''}>请选择处理方式</option><option value="recent" ${value === 'recent' ? 'selected' : ''}>沿用最近值</option>${numeric ? `<option value="linear" ${value === 'linear' ? 'selected' : ''}>线性延伸</option>` : ''}<option value="zero" ${value === 'zero' ? 'selected' : ''}>使用默认值</option></select>`;
}

const forecastFeaturePageBeforeV101 = forecastFeaturePageV10;
forecastFeaturePageV10 = function () {
  const set = ensureForecastDatasetV10(dataset()), step = Number(state.featureStep || 0) > 0 ? 1 : 0;
  if (!step) return forecastFeaturePageBeforeV101();
  const columns = forecastExternalColumnsV101(set), thresholds = set.forecastFeatureThresholds, included = columns.filter(column => column.included).length;
  const rows = columns.map(column => {
    const meta = column.forecastMeta || forecastFeatureStatsV101(set, column), passesMissing = meta.missingRate * 100 <= thresholds.missing, passesVariance = meta.variance === null || meta.variance >= thresholds.variance, passesFuture = meta.futureCoverage >= thresholds.futureCoverage, policy = set.forecastFuturePolicies[column.name] || 'pending', policyReady = meta.futureCoverage >= 100 || policy !== 'pending', recommended = meta.trainable && passesMissing && passesVariance && passesFuture && policyReady;
    const status = !meta.trainable ? ['不可用于训练', 'blocked'] : column.included ? ['已纳入', 'good'] : !policyReady ? ['待处理未来缺失', 'blocked'] : recommended ? ['手动排除', 'muted'] : ['建议排除', 'blocked'];
    return `<tr><td><label class="forecast-feature-check-v101"><input type="checkbox" data-forecast-feature-toggle="${esc(column.name)}" name="forecast-feature-${esc(column.name)}" aria-label="纳入外部特征${esc(column.name)}" ${column.included ? 'checked' : ''} ${meta.trainable ? '' : 'disabled'}><span aria-hidden="true"></span></label></td><td><b>${esc(column.name)}</b><small>上传字段</small></td><td>${typeLabel(column.type)}</td><td class="forecast-number-cell-v101 ${passesMissing ? '' : 'threshold-failed'}">${(meta.missingRate * 100).toFixed(1)}%</td><td class="forecast-number-cell-v101 ${passesVariance ? '' : 'threshold-failed'}"><span title="${meta.variance === null ? '类别或布尔特征不计算数值方差' : '基于历史区间 Min-Max 归一化后计算'}">${meta.variance === null ? '不适用' : meta.variance.toFixed(3)}</span></td><td class="forecast-number-cell-v101"><span title="${meta.temporalCorrelation === null ? '仅数值特征计算时序相关性' : '历史区间内绝对相关性最高的同期或滞后结果'}">${meta.temporalCorrelation === null ? '不适用' : `${meta.temporalCorrelation.toFixed(2)}<small>${meta.bestLag ? `滞后 ${meta.bestLag} 期` : '同期'}</small>`}</span></td><td class="forecast-number-cell-v101 ${passesFuture ? '' : 'threshold-failed'}">${meta.futureCoverage.toFixed(0)}%</td><td>${meta.futureCoverage < 100 ? forecastPolicyOptionsV101(column, policy) : '<span class="status good">未来值完整</span>'}</td><td><span class="status ${status[1]}">${status[0]}</span></td></tr>`;
  }).join('');
  const body = `<section class="panel"><div class="panel-head"><div><h2>特征管理</h2><p>筛选上传的外部特征；滞后、滚动、趋势和差分仍由模型训练时自动处理。</p></div><span class="status good">已纳入 ${included} 个</span></div>
    <div class="calendar-toggle-v10"><div><b>自动生成日历特征</b><span>根据时间列生成星期、月份、季度、周末等特征，不修改原始数据。</span></div><label class="switch"><input type="checkbox" data-forecast-calendar ${set.forecastConfig.calendarFeatures ? 'checked' : ''}><span></span></label></div>
    <div class="forecast-calendar-list-v10">${forecastCalendarLabelsV10(set.frequency).map(label => `<span class="${set.forecastConfig.calendarFeatures ? '' : 'disabled'}">${label}</span>`).join('')}</div>
    <div class="forecast-thresholds-v101"><label><span>历史缺失率超过</span><span class="threshold-input-wrap-v101"><input data-forecast-threshold="missing" name="forecast-threshold-missing" autocomplete="off" type="number" min="0" max="99" step="1" value="${thresholds.missing}"><i aria-hidden="true">%</i></span></label><label><span>归一化方差低于</span><input data-forecast-threshold="variance" name="forecast-threshold-variance" autocomplete="off" type="number" min="0" max="0.25" step="0.001" value="${thresholds.variance}"></label><label><span>未来覆盖率低于</span><span class="threshold-input-wrap-v101"><input data-forecast-threshold="futureCoverage" name="forecast-threshold-future-coverage" autocomplete="off" type="number" min="0" max="100" step="1" value="${thresholds.futureCoverage}"><i aria-hidden="true">%</i></span></label><div>${button('恢复推荐值', 'forecast-recommend-thresholds-v101')}${button('应用筛选', 'forecast-apply-thresholds-v101', 'primary')}</div></div>
    <div class="notice"><b>筛选规则</b><span>历史缺失率、归一化方差和未来覆盖率用于自动筛选；时序相关性只作参考，不自动剔除。未来覆盖率默认要求 80%，不足时可选择填补策略后手动纳入。</span></div>
    <div class="table-card forecast-feature-table-wrap-v101"><table class="forecast-feature-table-v101"><thead><tr><th>纳入</th><th>特征</th><th>类型</th><th>历史缺失率</th><th>归一化方差</th><th>时序相关性</th><th>未来覆盖率</th><th>未来缺失处理</th><th>状态</th></tr></thead><tbody>${rows || '<tr><td colspan="9"><div class="empty-state"><p>没有可作为外部特征的字段。</p></div></td></tr>'}</tbody></table></div>
    ${forecastCorrelationMatrixV101(set, columns)}</section>`;
  return shell(`${steps('feature')}${pageHead('特征准备', `${esc(set.name)} · 管理日历与外部特征`)}<div class="forecast-feature-tabs-v10"><button data-forecast-feature-step="0"><i>1</i>时间与回测</button><button data-forecast-feature-step="1" class="active"><i>2</i>特征管理</button></div>${body}`);
};

function forecastSelectedFeaturesV101(set = dataset()) { return forecastExternalColumnsV101(ensureForecastDatasetV10(set)).filter(column => column.trainable && column.included); }
function forecastUsesExternalV101(type) { return ['forecast_arima', 'forecast_prophet', 'forecast_lgbm'].includes(type); }
function forecastDisplayModelNameV101(type, set = dataset()) { return type === 'forecast_arima' && forecastSelectedFeaturesV101(set).length ? 'ARIMAX' : modelName(type); }

forecastProcessingSummaryV10 = function (type) {
  const set = ensureForecastDatasetV10(dataset()), selected = forecastSelectedFeaturesV101(set), names = selected.map(column => column.name), usesExternal = forecastUsesExternalV101(type) && names.length;
  const featureText = usesExternal ? names.join('、') : type === 'forecast_baseline' || type === 'forecast_ets' ? '该模型仅使用目标历史，不读取外部特征' : '当前未纳入外部特征';
  const rows = {
    forecast_baseline: [['模型比较', '历史均值、最后值、季节性最后值、漂移、移动平均'], ['外部特征', featureText]],
    forecast_ets: [['模型结构', '水平、趋势与季节性组件'], ['外部特征', featureText]],
    forecast_arima: [['模型结构', usesExternal ? 'ARIMAX：自回归、差分、移动平均与外部特征' : 'ARIMA：自回归、差分与移动平均'], ['使用特征', featureText], ['自动处理', '仅在每个训练窗口内确定差分与阶数']],
    forecast_prophet: [['模型结构', '趋势、季节性、日历效应与外部回归量'], ['使用特征', featureText], ['自动处理', '自动设置变点范围与季节性']],
    forecast_lgbm: [['模型输入', '目标滞后、滚动统计、趋势序号、日历特征与外部特征'], ['使用特征', featureText], ['自动处理', '每轮回测只使用当时已知的信息生成特征']]
  }[type] || [];
  return `<div class="forecast-processing-v10">${rows.map(row => `<div><b>${row[0]}</b><span>${esc(row[1])}</span></div>`).join('')}</div>`;
};

forecastModelSelectPageV10 = function () {
  const set = ensureForecastDatasetV10(dataset()), count = forecastSelectedFeaturesV101(set).length;
  return shell(`${steps('experiments')}${pageHead('选择预测模型', `当前纳入 ${count} 个外部特征；不同模型按自身能力使用。`)}<div class="model-grid">${FORECAST_MODELS_V10.map(model => { const supports = forecastUsesExternalV101(model[0]), title = model[0] === 'forecast_arima' ? 'ARIMA / ARIMAX' : model[1]; return `<article class="model-card"><span aria-hidden="true">${model[0] === 'forecast_lgbm' ? '⚡' : '◈'}</span><h3>${title}</h3><p>${model[2]}</p><div class="model-card-footer-v1033">${button('创建此模型实验', `add-forecast-model-v10:${model[0]}`, 'primary')}<em class="model-feature-badge-v101 ${supports ? 'supports' : ''}">${supports ? '支持外部特征' : '仅使用目标历史'}</em></div></article>`; }).join('')}</div>`);
};

const forecastExperimentPageBeforeV101 = forecastExperimentPageV10;
forecastExperimentPageV10 = function () {
  const item = experiment(), set = ensureForecastDatasetV10(dataset()), displayName = forecastDisplayModelNameV101(item.type, set), selected = forecastSelectedFeaturesV101(set), content = forecastExperimentPageBeforeV101();
  const summary = `<div class="notice ${forecastUsesExternalV101(item.type) && selected.length ? 'success' : ''}"><b>外部特征</b><span>${forecastUsesExternalV101(item.type) ? (selected.length ? `训练将使用：${selected.map(column => esc(column.name)).join('、')}` : '当前未纳入外部特征，将按单变量模式训练。') : '该模型作为目标历史基准，不使用外部特征。'}</span></div>`;
  return content.replace(`${modelName(item.type)} ·`, `${displayName} ·`).replace('<h3>模型自动处理</h3>', `${summary}<h3>模型自动处理</h3>`);
};

const makeForecastResultBeforeV101 = makeForecastResultV10;
makeForecastResultV10 = function (item, variant, index) {
  const set = ensureForecastDatasetV10(state.datasets[item.datasetId]), selected = forecastSelectedFeaturesV101(set), usesExternal = forecastUsesExternalV101(item.type) && selected.length > 0, result = makeForecastResultBeforeV101(item, variant, index);
  result.modelDisplayName = forecastDisplayModelNameV101(item.type, set); result.externalFeatures = usesExternal ? selected.map(column => column.name) : []; result.ignoredExternalFeatures = usesExternal ? [] : selected.map(column => column.name);
  result.featureContributions = usesExternal ? selected.map((column, position) => ({ name: column.name, role: column.forecastMeta?.bestLag ? `滞后 ${column.forecastMeta.bestLag} 期相关` : '同期相关', importance: +(Math.max(.08, .34 - position * .055)).toFixed(3) })) : [];
  if (!usesExternal) return result;
  const improvement = Math.max(.78, 1 - Math.min(.18, selected.length * .035));
  result.backtests = result.backtests.map(round => { const predicted = round.predicted.map((point, pointIndex) => ({ ...point, value: +(round.actual[pointIndex].value + (point.value - round.actual[pointIndex].value) * improvement).toFixed(2) })), metrics = forecastMetricsV10(round.actual.map(point => point.value), predicted.map(point => point.value)); return { ...round, predicted, ...metrics }; });
  const maes = result.backtests.map(round => round.mae), average = values => values.reduce((sum, value) => sum + value, 0) / values.length;
  result.mae = +average(maes).toFixed(2); result.rmse = +average(result.backtests.map(round => round.rmse)).toFixed(2); result.smape = +average(result.backtests.map(round => round.smape)).toFixed(1); result.stability = +Math.sqrt(maes.reduce((sum, value) => sum + (value - result.mae) ** 2, 0) / maes.length).toFixed(2);
  const context = forecastHistoryContextV101(set), timeValues = forecastValuesV10(set, set.timeColumn), timeIndex = new Map(timeValues.map((value, position) => [parseForecastDateV10(value), position]));
  result.forecast = result.forecast.map((point, pointIndex) => { let influence = 0; selected.forEach((column, columnIndex) => { const raw = forecastValuesV10(set, column.name)[timeIndex.get(point.timestamp)], numeric = Number(raw); influence += Number.isFinite(numeric) ? Math.sin(numeric * .13 + columnIndex) * .45 : ((forecastHashV10(raw || column.name) % 11) - 5) * .08; }); const prediction = +(point.prediction + influence).toFixed(2), shift = prediction - point.prediction; return { ...point, prediction, lower80: +(point.lower80 + shift).toFixed(2), upper80: +(point.upper80 + shift).toFixed(2), lower95: +(point.lower95 + shift).toFixed(2), upper95: +(point.upper95 + shift).toFixed(2) }; });
  result.externalFeatureCutoff = formatForecastDateV10(context.cutoff, set.frequency); return result;
};

const forecastReportPageBeforeV101 = forecastReportPageV10;
forecastReportPageV10 = function () {
  const item = experiment(), set = ensureForecastDatasetV10(dataset()), result = forecastReportRenderResultV1011 || item?.results?.find(row => row.id === item.selected) || item?.results?.[0], content = forecastReportPageBeforeV101();
  if (!result) return content;
  const names = result.externalFeatures || [], usesExternal = names.length > 0, title = forecastDisplayModelNameV101(item.type, set);
  const contribution = `<section class="section-block forecast-feature-report-v101"><div class="section-title"><div><h2>外部特征使用情况</h2><p>展示本次训练快照中实际进入模型的上传字段。</p></div><span>${esc(title)}</span></div>${usesExternal ? `<div class="forecast-report-feature-list-v101">${result.featureContributions.map(feature => `<div><b>${esc(feature.name)}</b><span>${esc(feature.role)}</span><strong>${feature.importance.toFixed(3)}</strong></div>`).join('')}</div><div class="notice success"><b>参与训练</b><span>${names.map(esc).join('、')} 已用于回测与未来预测。</span></div>` : `<div class="notice"><b>未使用外部特征</b><span>${forecastUsesExternalV101(item.type) ? '本次训练没有纳入外部特征。' : '该模型仅使用目标列历史数据，适合作为比较基准。'}</span></div>`}</section>`;
  return content
    .replace(`<span>${modelName(item.type)}</span>`, `<span>${esc(title)}</span>`)
    .replace(`<code>模型：${esc(modelName(item.type))}</code>`, `<code>模型：${esc(title)}</code><code>外部特征：${usesExternal ? names.map(esc).join('、') : '未使用'}</code>`)
    .replace('<section class="section-block"><div class="section-title"><h2>模型说明</h2>', `${contribution}<section class="section-block"><div class="section-title"><h2>模型说明</h2>`);
};

const forecastTuningPageBeforeV101 = forecastTuningPageV10;
forecastTuningPageV10 = function () {
  return forecastTuningPageBeforeV101()
    .replace('<div class="table-card forecast-table-scroll-v10"><table class="forecast-result-table-v10">', '<div class="table-card forecast-table-scroll-v10 forecast-table-scroll-v101"><table class="forecast-result-table-v10 result-table sticky-action-table-v101">')
    .replace('<th class="compact-col-v10">保存</th>', '<th class="result-save-column-v101">保存</th>')
    .replace('<th class="compact-col-v10">排名</th>', '<th class="result-rank-column-v101">排名</th>')
    .replace('<th class="scheme-col-v10">参数方案</th>', '<th class="result-scheme-column-v101">参数方案</th>')
    .replaceAll('<th><button class="metric-sort-v10"', '<th class="result-metric-column-v101"><button class="metric-sort-v10"')
    .replace('<th class="action-col-v10">操作</th>', '<th class="result-action-column-v101">操作</th>')
    .replaceAll('<td><label class="forecast-save-check-v10">', '<td class="result-save-column-v101"><label class="forecast-save-check-v10">')
    .replace(/<td><b>#(\d+)<\/b><\/td>/g, '<td class="result-rank-column-v101"><b>#$1</b></td>')
    .replaceAll('<td><b>方案 #', '<td class="result-scheme-column-v101"><b>方案 #')
    .replace(/<td><b>([\d.]+%?)<\/b><\/td>/g, '<td class="result-metric-column-v101"><b>$1</b></td>')
    .replaceAll('<td><div class="row-actions">', '<td class="result-action-column-v101"><div class="row-actions">');
};

const forecastLibraryDetailBeforeV101 = forecastLibraryDetailV10;
forecastLibraryDetailV10 = function (detail) {
  return forecastLibraryDetailBeforeV101(detail)
    .replace('<div class="table-card forecast-table-scroll-v10"><table class="forecast-library-table-v10">', '<div class="table-card forecast-table-scroll-v10 forecast-table-scroll-v101"><table class="forecast-library-table-v10 sticky-action-table-v101">')
    .replaceAll('<th><button class="metric-sort-v10"', '<th class="result-metric-column-v101"><button class="metric-sort-v10"')
    .replace('<th class="action-col-v10">操作</th>', '<th class="result-action-column-v101">操作</th>')
    .replaceAll('<td><b>方案 #', '<td class="library-scheme-column-v101"><b>方案 #')
    .replace(/<td><b>([\d.]+%?)<\/b><\/td>/g, '<td class="result-metric-column-v101"><b>$1</b></td>')
    .replaceAll('<td><div class="row-actions">', '<td class="result-action-column-v101"><div class="row-actions">');
};

const actionBeforeForecastV101 = action;
action = function (name) {
  if (isForecastingV10() && name === 'forecast-recommend-thresholds-v101') {
    const defaults = { missing: 90, variance: .01, futureCoverage: 80 };
    app.querySelectorAll('[data-forecast-threshold]').forEach(input => { input.value = defaults[input.dataset.forecastThreshold]; input.classList.remove('range-invalid'); });
    return toast('已填入推荐值，点击“应用筛选”后生效。', 'success');
  }
  if (isForecastingV10() && name === 'forecast-apply-thresholds-v101') {
    const set = ensureForecastDatasetV10(dataset()), inputs = [...app.querySelectorAll('[data-forecast-threshold]')], values = Object.fromEntries(inputs.map(input => [input.dataset.forecastThreshold, Number(input.value)]));
    let invalid = false; inputs.forEach(input => { const bad = !input.value.trim() || !input.checkValidity(); input.classList.toggle('range-invalid', bad); invalid ||= bad; });
    if (invalid) return toast('请修正标红的筛选阈值。', 'warning');
    set.forecastFeatureThresholds = values;
    forecastExternalColumnsV101(set).forEach(column => { const meta = forecastFeatureStatsV101(set, column), passes = meta.trainable && meta.missingRate * 100 <= values.missing && (meta.variance === null || meta.variance >= values.variance) && meta.futureCoverage >= values.futureCoverage; column.forecastMeta = meta; column.included = passes; });
    set.feature.revision = (set.feature.revision || 0) + 1; markForecastExperimentsStaleV10(set); save(); render(); return setTimeout(() => toast('已按当前阈值更新外部特征。', 'success'), 0);
  }
  if (isForecastingV10() && name === 'save-library-results') {
    const item = experiment(), selected = [...(state.tuningSelections[item.id] || [])], outcome = actionBeforeForecastV101(name);
    selected.forEach(resultId => { const result = item.results.find(row => row.id === resultId), snapshot = state.savedResultSnapshots[`${item.id}:${resultId}`]; if (!result || !snapshot?.trainingSnapshot) return; snapshot.trainingSnapshot.schemaVersion = 2; snapshot.trainingSnapshot.externalFeatures = cloneV7(result.externalFeatures || []); snapshot.trainingSnapshot.ignoredExternalFeatures = cloneV7(result.ignoredExternalFeatures || []); snapshot.trainingSnapshot.featureContributions = cloneV7(result.featureContributions || []); snapshot.trainingSnapshot.featureThresholds = cloneV7(state.datasets[item.datasetId]?.forecastFeatureThresholds || {}); snapshot.trainingSnapshot.futurePolicies = cloneV7(state.datasets[item.datasetId]?.forecastFuturePolicies || {}); });
    save(); return outcome;
  }
  return actionBeforeForecastV101(name);
};

const bindBeforeForecastV101 = bind;
bind = function () {
  bindBeforeForecastV101();
  if (!isForecastingV10()) return;
  const set = ensureForecastDatasetV10(dataset());
  app.querySelectorAll('[data-forecast-threshold]').forEach(input => input.addEventListener('input', () => input.classList.toggle('range-invalid', !input.value.trim() || !input.checkValidity())));
  app.querySelectorAll('[data-forecast-feature-toggle]').forEach(input => input.addEventListener('change', event => {
    const column = forecastExternalColumnsV101(set).find(item => item.name === event.target.dataset.forecastFeatureToggle); if (!column) return;
    const meta = column.forecastMeta || forecastFeatureStatsV101(set, column), policy = set.forecastFuturePolicies[column.name] || 'pending';
    if (event.target.checked && meta.futureCoverage < 100 && policy === 'pending') { event.target.checked = false; column.included = false; save(); return toast('请先为该特征选择未来缺失处理方式。', 'warning'); }
    column.included = event.target.checked; set.feature.revision = (set.feature.revision || 0) + 1; markForecastExperimentsStaleV10(set); save(); render();
  }));
  app.querySelectorAll('[data-forecast-future-policy]').forEach(select => select.addEventListener('change', event => { const column = forecastExternalColumnsV101(set).find(item => item.name === event.target.dataset.forecastFuturePolicy); if (!column) return; set.forecastFuturePolicies[column.name] = event.target.value; if (event.target.value === 'pending') column.included = false; set.feature.revision = (set.feature.revision || 0) + 1; markForecastExperimentsStaleV10(set); save(); render(); }));
};
...
// V10.1.1 post-activation: this block intentionally follows every V10.1 override.
const makeForecastResultBeforePostV1011 = makeForecastResultV10;
makeForecastResultV10 = function (item, variant, index) { const result = makeForecastResultBeforePostV1011(item, variant, index), set = ensureForecastDatasetV10(state.datasets[item.datasetId]); result.historicalMissingHandling = forecastHistoricalHandlingV1011(set, result.externalFeatures || [], item.type); return result; };

forecastDisplayModelNameV101 = function (type, set = dataset()) { if (forecastReportRenderResultV1011?.modelDisplayName) return forecastReportRenderResultV1011.modelDisplayName; return type === 'forecast_arima' && forecastSelectedFeaturesV101(set).length ? 'ARIMAX' : modelName(type); };

function forecastProcessingSummaryPostV1011(type) {
  const set = ensureForecastDatasetV10(dataset()), snapshotResult = forecastReportRenderResultV1011, selected = snapshotResult ? (snapshotResult.externalFeatures || []).map(name => set.columns.find(column => column.name === name)).filter(Boolean) : forecastSelectedFeaturesV101(set), names = snapshotResult ? snapshotResult.externalFeatures || [] : selected.map(column => column.name), usesExternal = forecastUsesExternalV101(type) && names.length;
  const featureText = usesExternal ? names.join('、') : type === 'forecast_baseline' || type === 'forecast_ets' ? '该模型仅使用目标历史，不读取外部特征' : '当前未纳入外部特征', missingHandling = snapshotResult?.historicalMissingHandling || forecastHistoricalHandlingV1011(set, names, type), missingText = !usesExternal ? '不适用' : missingHandling.length ? missingHandling.map(item => `${item.name}：${item.strategy}`).join('；') : '所选外部特征的历史区间无缺失';
  const rows = { forecast_baseline: [['模型比较', '历史均值、最后值、季节性最后值、漂移、移动平均'], ['外部特征', featureText]], forecast_ets: [['模型结构', '水平、趋势与季节性组件'], ['外部特征', featureText]], forecast_arima: [['模型结构', usesExternal ? 'ARIMAX：自回归、差分、移动平均与外部特征' : 'ARIMA：自回归、差分与移动平均'], ['使用特征', featureText], ['历史缺失处理', missingText], ['自动处理', '仅在每个训练窗口内确定填充值、差分与阶数']], forecast_prophet: [['模型结构', '趋势、季节性、日历效应与外部回归量'], ['使用特征', featureText], ['历史缺失处理', missingText], ['自动处理', '每轮训练窗口单独确定填充值、变点范围与季节性']], forecast_lgbm: [['模型输入', '目标滞后、滚动统计、趋势序号、日历特征与外部特征'], ['使用特征', featureText], ['历史缺失处理', missingText], ['自动处理', '每轮回测只使用当时已知的信息生成特征']] }[type] || [];
  return `<div class="forecast-processing-v10">${rows.map(row => `<div><b>${row[0]}</b><span>${esc(row[1])}</span></div>`).join('')}</div>`;
}
forecastProcessingSummaryV10 = forecastProcessingSummaryPostV1011;

const forecastReportPageBeforePostV1011 = forecastReportPageV10;
forecastReportPageV10 = function () { const item = experiment(), snapshot = state.savedResultSnapshots?.[state.forecastReportSnapshotKeyV1011]; if (!item || !snapshot?.result || snapshot.experimentId !== item.id) return forecastReportPageBeforePostV1011(); const originalResults = item.results, result = cloneV7(snapshot.result); item.results = [...(originalResults || []).filter(row => row.id !== result.id), result]; forecastReportRenderResultV1011 = result; try { return forecastReportPageBeforePostV1011(); } finally { forecastReportRenderResultV1011 = null; item.results = originalResults; } };

const actionBeforePostV1011 = action;
action = function (name) {
  if (isForecastingV10() && name === 'forecast-recommend-thresholds-v101') { const defaults = { missing: 90, variance: .01, futureCoverage: 80 }; app.querySelectorAll('[data-forecast-threshold]').forEach(input => { input.value = defaults[input.dataset.forecastThreshold]; forecastThresholdFeedbackV1011(input); }); return toast('已填入推荐值，点击“应用筛选”后生效。', 'success'); }
  if (isForecastingV10() && name === 'forecast-apply-thresholds-v101') { const set = ensureForecastDatasetV10(dataset()), inputs = [...app.querySelectorAll('[data-forecast-threshold]')], valid = inputs.map(forecastThresholdFeedbackV1011); if (valid.some(value => !value)) { inputs[valid.findIndex(value => !value)]?.focus(); return toast('请修正标红的筛选阈值。', 'warning'); } const values = Object.fromEntries(inputs.map(input => [input.dataset.forecastThreshold, Number(input.value)])); set.forecastFeatureThresholds = values; forecastExternalColumnsV101(set).forEach(column => { const meta = forecastFeatureStatsV101(set, column), policyReady = meta.futureCoverage >= 100 || ['recent', 'linear', 'zero'].includes(set.forecastFuturePolicies[column.name]), passes = meta.trainable && meta.missingRate * 100 <= values.missing && (meta.variance === null || meta.variance >= values.variance) && meta.futureCoverage >= values.futureCoverage && policyReady; column.forecastMeta = meta; column.included = passes; }); set.feature.revision = (set.feature.revision || 0) + 1; markForecastExperimentsStaleV10(set); save(); render(); return setTimeout(() => toast('已按当前阈值和缺失策略更新外部特征。', 'success'), 0); }
  return actionBeforePostV1011(name);
};

const bindBeforePostV1011 = bind;
bind = function () { bindBeforePostV1011(); if (!isForecastingV10()) return; app.querySelectorAll('[data-forecast-threshold]').forEach(input => input.addEventListener('input', () => forecastThresholdFeedbackV1011(input))); };
...
// ============================================================================
// V10.2 final activation — historical-only multi-feature forecasting
// ============================================================================
forecastSummaryV10 = forecastSummaryV102;
forecastFeatureStatsV101 = forecastFeatureStatsV102;
ensureForecastDatasetV10 = ensureForecastDatasetV102;
createForecastExampleV10 = createForecastExampleV102;
buildForecastCsvDatasetV10 = buildForecastCsvDatasetV102;
datasetSourceModal = datasetSourceModalV102;
forecastDatasetPageV10 = forecastDatasetPageV102;
forecastFeaturePageV10 = function () { return forecastFeaturePageV102().replaceAll('data-forecast-feature-toggle=', 'data-forecast-feature-toggle-v102='); };
forecastThresholdFeedbackV1011 = forecastThresholdFeedbackV102;
forecastExperimentPageV10 = forecastExperimentPageV102;
startForecastTrainingV10 = startForecastTrainingV102;
forecastTuningPageV10 = forecastTuningPageV102;
forecastBacktestModalV10 = forecastBacktestModalV102;
trainingSnapshotPanelV910 = trainingSnapshotPanelV102;
forecastReportPageV10 = forecastReportPageV102;
forecastLibraryDetailV10 = forecastLibraryDetailV102;
forecastResolveFutureValueV1011 = forecastResolveFutureValueV102;
forecastApiDefaultRowsV1011 = forecastApiDefaultRowsV102;
openForecastApiModalV10 = openForecastApiModalV102;
forecastApiPageV10 = forecastApiPageV102;

// Classification/regression result enhancers run after the page HTML is
// rendered. They must not rewrite forecasting tables with AUC/KS/F1 columns.
const enhanceCompleteResultTableBeforeV102 = enhanceCompleteResultTableV7;
enhanceCompleteResultTableV7 = function () {
  if (isForecastingV10()) return;
  return enhanceCompleteResultTableBeforeV102();
};
const enhanceTrainingResultsBeforeV102 = enhanceTrainingResultsV8;
enhanceTrainingResultsV8 = function () {
  if (isForecastingV10()) return;
  return enhanceTrainingResultsBeforeV102();
};

function forecastHorizonFeedbackV102(input, set) {
  const value = Number(input.value), maximum = forecastHorizonLimitV102(set), valid = Number.isInteger(value) && value >= 1 && value <= maximum;
  input.classList.toggle('range-invalid', !valid);
  input.setAttribute('aria-invalid', String(!valid));
  const message = input.closest('label')?.querySelector('.threshold-range-error');
  if (message) { message.textContent = valid ? '' : `预测期数请输入 1–${maximum}。`; message.hidden = valid; }
  return valid;
}

const actionBeforeForecastV102 = action;
action = function (name) {
  if (isForecastingV10() && name === 'save-library-results') {
    const item = experiment(), selected = [...(state.tuningSelections[item.id] || [])];
    if (!selected.length) return toast('请至少选择一个结果。', 'warning');
    ...
    selected.forEach(resultId => {
      ...
      if (!result) return;
      saved.add(resultId);
      const key = `${item.id}:${resultId}`, createdAt = state.savedResultMeta[key]?.createdAt || now();
      state.savedResultMeta[key] ||= { createdAt };
      state.savedResultSnapshots[key] ||= { experimentId: item.id, projectId: item.projectId, datasetId: item.datasetId, experimentName: item.name, type: item.type, result: cloneV7(result), createdAt };
      state.savedResultSnapshots[key].trainingSnapshot = { schemaVersion: 3, resultMode: 'demo', task: 'forecasting', projectId: item.projectId, projectName: project().name, datasetId: item.datasetId, datasetName: dataset().name, timeColumn: result.timeColumn || dataset().timeColumn, target: result.target || dataset().target, frequency: result.frequency || dataset().frequency, forecastConfig: cloneV7(result.forecastConfig || dataset().forecastConfig), timeSummary: cloneV7(result.timeSummary || dataset().timeSummary), modelType: item.type, parameters: cloneV7(item.parameters || {}), externalFeatures: cloneV7(result.externalFeatures || []), ignoredExternalFeatures: cloneV7(result.ignoredExternalFeatures || []), featureContributions: cloneV7(result.featureContributions || []), featureThresholds: cloneV7(dataset().forecastFeatureThresholds || {}), trainedAt: result.generatedAt };
    });
    state.savedResults[item.id] = [...saved];
    state.libraryReturnContext = { page: 'tuning', projectId: item.projectId, datasetId: item.datasetId, experimentId: item.id };
    state.libraryDetailScope = { type: 'experiment', id: item.id };
    state.libraryDetailDatasetId = 'all'; state.libraryDetailExperimentId = 'all'; state.libraryDetailModelType = 'all'; state.libraryTab = 'compare'; state.forecastLibrarySortV10 = 'mae'; state.forecastLibrarySortDirectionV10 = 'asc';
    save(); go('models');
    return setTimeout(() => toast(additions.length ? '结果已保存并进入模型库。' : '结果已存在，已进入模型库。', 'success'), 0);
  }
  if (isForecastingV10() && name === 'forecast-recommend-thresholds-v101') {
    const defaults = { missing: 90, variance: .01 };
    app.querySelectorAll('[data-forecast-threshold]').forEach(input => { input.value = defaults[input.dataset.forecastThreshold]; forecastThresholdFeedbackV1011(input); });
    return toast('已填入推荐值，点击“应用筛选”后生效。', 'success');
  }
  if (isForecastingV10() && name === 'forecast-apply-thresholds-v101') {
    const set = ensureForecastDatasetV10(dataset()), inputs = [...app.querySelectorAll('[data-forecast-threshold]')], valid = inputs.map(forecastThresholdFeedbackV1011);
    if (valid.some(value => !value)) { inputs[valid.findIndex(value => !value)]?.focus(); return toast('请修正标红的筛选阈值。', 'warning'); }
    const values = Object.fromEntries(inputs.map(input => [input.dataset.forecastThreshold, Number(input.value)]));
    set.forecastFeatureThresholds = { ...set.forecastFeatureThresholds, ...values };
    forecastExternalColumnsV101(set).forEach(column => { const meta = forecastFeatureStatsV101(set, column); column.forecastMeta = meta; column.included = meta.trainable && meta.missingRate * 100 <= values.missing && (meta.variance === null || meta.variance >= values.variance); });
    set.feature.revision = (set.feature.revision || 0) + 1; markForecastExperimentsStaleV10(set); save(); render();
    return setTimeout(() => toast('已按历史缺失率和归一化方差更新外部特征。', 'success'), 0);
  }
  return actionBeforeForecastV102(name);
};

const bindBeforeForecastV102 = bind;
bind = function () {
  bindBeforeForecastV102();
  if (!isForecastingV10()) return;
  const set = ensureForecastDatasetV10(dataset());
  app.querySelectorAll('[data-forecast-column-type-v102]').forEach(select => select.addEventListener('change', event => {
    const column = set.columns[Number(event.target.dataset.forecastColumnTypeV102)], previous = column?.type, next = event.target.value;
    if (!column) return;
    const values = forecastValuesV10(set, column.name).filter(value => String(value ?? '').trim());
    if (next === 'date' && values.some(value => !Number.isFinite(parseForecastDateV10(value)))) { event.target.value = previous; return toast(`“${column.name}”包含无法解析的日期，不能改为日期类型。`, 'warning'); }
    if (column.name === set.target && next !== 'number') { event.target.value = previous; return toast('预测目标必须保持数值类型。', 'warning'); }
    column.type = next;
    if (column.name === set.timeColumn && next !== 'date') set.timeColumn = set.columns.find(item => item.type === 'date')?.name || '';
    if (!set.timeColumn && next === 'date') set.timeColumn = column.name;
    set.feature.revision = (set.feature.revision || 0) + 1; markForecastExperimentsStaleV10(set); ensureForecastDatasetV10(set); save(); render();
  }));
  const horizon = app.querySelector('[data-forecast-horizon-v102]');
  horizon?.addEventListener('input', () => forecastHorizonFeedbackV102(horizon, set));
  horizon?.addEventListener('change', () => { if (!forecastHorizonFeedbackV102(horizon, set)) return horizon.focus(); const value = Number(horizon.value); if (value === set.forecastConfig.horizon) return; set.forecastConfig.horizon = value; set.feature.revision = (set.feature.revision || 0) + 1; markForecastExperimentsStaleV10(set); save(); render(); });
  app.querySelectorAll('[data-forecast-feature-toggle-v102]').forEach(input => input.addEventListener('change', event => { const column = forecastExternalColumnsV101(set).find(item => item.name === event.target.dataset.forecastFeatureToggleV102); if (!column) return; column.included = event.target.checked; set.feature.revision = (set.feature.revision || 0) + 1; markForecastExperimentsStaleV10(set); save(); render(); }));
  // V10.2.4：相关性阈值由末尾覆盖层统一处理（视觉反馈 + 确定修改），此处不再监听 change。
  const locked = experiment()?.status === 'completed' && experiment()?.results?.length;
  if (locked) app.querySelectorAll('[data-forecast-param], [data-forecast-tuning]').forEach(control => { control.disabled = true; });
};
...
forecastTuningOptionsV10 = function (item) {
  ...
  const detail = item.tuning === 'grid' ? `<div class="grid-settings"><div class="panel-head"><div><h3>网格候选值</h3><p>可直接增删逗号分隔的候选值。</p></div>${locked ? '' : button('恢复推荐值', 'forecast-restore-grid-v1024')}</div><b data-forecast-grid-count-v1024 class="${summary.valid ? '' : 'warning-text'}">${esc(summary.message)}</b>${definitions.map((field, index) => `<label><span>${esc(field.label)}</span><input data-forecast-grid-v1024="${field.key}" value="${esc(grid[field.key])}" ${locked ? 'disabled' : ''}><small class="threshold-range-error" ${summary.fieldResults[index].valid ? 'hidden' : ''}>${esc(summary.fieldResults[index].message)}</small></label>`).join('')}</div>` : '';
...
forecastFeaturePageV10 = function () {
  ...
  return forecastFeaturePageBeforeV1022()
    .replace(/<input type="checkbox" data-forecast-calendar([^>]*)><span><\/span><\/label>/, `<b>${enabled ? '已开启' : '已关闭'}</b><span class="switch"><input type="checkbox" data-forecast-calendar$1><span></span></span></label>`)
    .replace(/class="switch">(<b>已)/g, 'class="forecast-calendar-control-v1022">$1')
    .replaceAll('>建议排除<', '>自动排除<')
    .replace('<details class="metric-guide compact-guide">', `${summary ? `<div class="forecast-auto-filter-result-v1022"><b>最近一次筛选结果</b><span>纳入 ${summary.included} 个，排除 ${summary.excluded} 个；红色指标为主要排除原因。</span></div>` : ''}<details class="metric-guide compact-guide">`);
};
...
action = function (name) {
  ...
  if (isForecastingV10() && name === 'forecast-recommend-thresholds-v101') { const defaults = { missing: 90, variance: .01 }; app.querySelectorAll('[data-forecast-threshold]').forEach(input => { input.value = defaults[input.dataset.forecastThreshold]; forecastThresholdFeedbackV1011(input); }); return toast('已填入推荐阈值，点击“确定修改”后生效。', 'success'); }
  if (isForecastingV10() && name === 'forecast-apply-thresholds-v101') {
    ...
    set.forecastAutoFilterSummaryV1022 = { included, excluded }; set.feature.revision = (set.feature.revision || 0) + 1; markForecastExperimentsStaleV10(set); save(); render(); return setTimeout(() => toast(`筛选完成：纳入 ${included} 个，排除 ${excluded} 个。`, 'success'), 0);
  }
...
// ============================================================================
// V10.2.3 — auto filter button, live selection count and forecasting snapshot
// ============================================================================

// 「自动筛选」使用系统固定推荐值，不读取输入框；「确定修改」仍按用户输入值筛选。
const actionBeforeV1023 = action;
action = function (name) {
  if (isForecastingV10() && name === 'forecast-auto-filter-v1023') {
    const set = ensureForecastDatasetV10(dataset()), values = { missing: 90, variance: .01 };
    set.forecastFeatureThresholds = { ...set.forecastFeatureThresholds, ...values };
    let included = 0, excluded = 0;
    forecastExternalColumnsV101(set).forEach(column => {
      const meta = forecastFeatureStatsV101(set, column), passes = meta.trainable && meta.missingRate * 100 <= values.missing && (meta.variance === null || meta.variance >= values.variance);
      column.forecastMeta = meta; column.included = passes; passes ? included += 1 : excluded += 1;
    });
    set.forecastAutoFilterSummaryV1022 = { included, excluded };
    set.feature.revision = (set.feature.revision || 0) + 1;
    markForecastExperimentsStaleV10(set); save(); render();
    return setTimeout(() => toast(`已按推荐值自动筛选：纳入 ${included} 个，排除 ${excluded} 个。`, 'success'), 0);
  }
  return actionBeforeV1023(name);
};

// 勾选/取消结果时同步刷新“已选 n 个方案”文案与保存按钮样式。
const enhanceSaveButtonBeforeV1023 = enhanceSaveButton;
enhanceSaveButton = function () {
  enhanceSaveButtonBeforeV1023();
  const label = app.querySelector('[data-forecast-selected-count-v1023]');
  if (!label) return;
  const item = experiment(); if (!item) return;
  const count = (state.tuningSelections[item.id] || []).length;
  label.textContent = `已选 ${count} 个方案`;
  const saveButton = app.querySelector('[data-action="save-library-results"]');
  if (saveButton) saveButton.classList.toggle('primary', count > 0);
};

// 时序结果在保存到模型库之前会回退到通用快照，被 taskKeyV9 误判为分类任务，这里补上时序专用快照。
const trainingSnapshotBeforeV1023 = trainingSnapshotV910;
trainingSnapshotV910 = function (item, result) {
  const owner = state.projects.find(entry => entry.id === item?.projectId);
  if (!isForecastingV10(owner)) return trainingSnapshotBeforeV1023(item, result);
  const set = state.datasets[item.datasetId];
  if (!set || !result) return null;
  return {
    schemaVersion: 3, resultMode: 'demo', task: 'forecasting',
    metricSource: result.metricSource || DEMO_RESULT_SOURCE_V910,
    projectId: owner.id, projectName: owner.name,
    datasetId: set.id, datasetName: set.name,
    timeColumn: result.timeColumn || set.timeColumn,
    target: result.target || set.target,
    frequency: result.frequency || set.frequency,
    forecastConfig: cloneV7(result.forecastConfig || set.forecastConfig),
    timeSummary: cloneV7(result.timeSummary || set.timeSummary),
    modelType: item.type,
    externalFeatures: cloneV7(result.externalFeatures || []),
    trainedAt: result.generatedAt || item.updatedAt || now()
  };
};
...
bind = function () {
  ...
  if (corrInput) {
    const showUnsaved = () => { const note = app.querySelector('[data-forecast-correlation-unsaved-v1032]'); if (note) note.hidden = false; };
    corrInput.addEventListener('input', () => { forecastCorrelationFeedbackV1024(corrInput); showUnsaved(); });
    corrInput.addEventListener('change', () => { forecastCorrelationFeedbackV1024(corrInput); showUnsaved(); });
  }

  // 「确定修改」按钮
  app.querySelectorAll('[data-action]').forEach(el => {
    const act = el.dataset.action;
    if (act === 'forecast-apply-correlation-v1024') {
  ...
  // V10.2.4 修复：V6 enhanceExperimentPage 对全部实验页无差别注入，会污染时序任务：
  //   (a) 用 innerHTML 覆盖 details.advanced 内容为 V6 高级参数集
  //   (b) 在 .tuning-options 后插入一份 V6 风格的 .grid-settings（含"更多调参参数"折叠）
  // 这里只对时序任务做修复：恢复 V10.2.4 自己的高级参数渲染，并移除 V6 注入的多余 .grid-settings。
  // 分类 / 回归保持原状（V6 的注入恰好与现有样式一致，不破坏视觉）。
  const forecastItem = experiment();
  const forecastSection = app.querySelector('.forecast-parameter-section-v1022');
  if (forecastItem && forecastSection) {
    const forecastSet = ensureForecastDatasetV10(dataset());
    const locked = forecastItem.status === 'completed' && forecastItem.results?.length;
    const detailsAdvanced = forecastSection.querySelector('details.advanced');
    if (detailsAdvanced) {
      const advancedFields = forecastParameterSchemaV1022(forecastItem.type, forecastSet).filter(f => f.advanced);
      if (advancedFields.length) {
        detailsAdvanced.innerHTML = `<summary>高级参数设置</summary><div class="forecast-param-grid-v1022">${advancedFields.map(f => forecastParameterControlV1022(forecastItem, f, locked, forecastSet)).join('')}</div>`;
        detailsAdvanced.querySelectorAll('[data-forecast-param-v1022]').forEach(control => {
          control.addEventListener('input', () => forecastParameterFeedbackV1022(control));
          control.addEventListener('change', () => {
            if (!forecastParameterFeedbackV1022(control)) return;
            ensureForecastParametersV1022(forecastItem);
            forecastItem.parameters[control.dataset.forecastParamV1022] = control.type === 'checkbox' ? control.checked : control.type === 'number' ? Number(control.value) : control.value;
            forecastItem.status = forecastItem.results?.length ? 'stale' : 'draft';
            save();
            if (control.type === 'checkbox' || control.tagName === 'SELECT') render();
          });
        });
      }
    }
    // 移除 V6 注入的 .grid-settings（没有 V10.2.4 标识 data-forecast-grid-v1024 的那个）
    const injectedGrids = [...app.querySelectorAll('.grid-settings')].filter(el => !el.querySelector('[data-forecast-grid-v1024]'));
    injectedGrids.forEach(el => el.remove());

    // 移除 V6 在 .config-grid > section.panel 内通过 details.innerHTML 或 insertAdjacentHTML 注入的
    // 空 <details class="advanced"> 残骸（里面只有空的 .advanced-grid 和 V6 版"恢复默认值"按钮）。
    // 特征：包含子元素 .advanced-grid（V10.2.4 的 details.advanced 用的是 .forecast-param-grid-v1022，不会匹配到）。
    app.querySelectorAll('details.advanced').forEach(el => {
      if (el.querySelector('.advanced-grid')) el.remove();
    });
  }
};

// ============================================================================
// V10.3.0 — 智能选型（Smart Selection）
// ============================================================================

// 恢复完整 render，后续覆盖层和最终激活都需要使用最终页面函数
render = safeRenderV103;

// 状态默认值与旧数据兼容
state.smartSelections ||= [];
state.smartSelectionId ||= '';
state.smartSelectionPool ||= [];
state.smartSelectionShowAllResults ||= false;
state.smartSelectionShowAllHistory ||= false;
state.smartSelectionRun ||= null;
state.smartSelectionRunToken ||= 0;

// 如果当前记录 ID 指向不存在记录，清空
if (state.smartSelectionId && !state.smartSelections.find(r => r.id === state.smartSelectionId)) state.smartSelectionId = '';

// 常量
const SMART_SELECTION_PARAM_VERSION_V103 = 'V1';
const SMART_SELECTION_MAX_POOL_V103 = { classification: 5, multiclass: 5, regression: 4, forecasting: 5 };
const SMART_SELECTION_MIN_POOL_V103 = { classification: 2, multiclass: 2, regression: 2, forecasting: 2 };

// 统一任务键适配器：返回 'classification' | 'multiclass' | 'regression' | 'forecasting'
function smartSelectionTaskKeyV103(owner = project()) {
  if (isForecastingV10(owner)) return 'forecasting';
  const key = taskKeyV9(owner);
  return key === 'multiclass' ? 'multiclass' : key === 'regression' ? 'regression' : 'classification';
}

// 智能选型模型目录（ID 沿用现有）
function smartSelectionModelCatalogV103(taskKey) {
  if (taskKey === 'forecasting') return [
    { id: 'forecast_baseline', name: '基线预测', order: 1 },
    { id: 'forecast_ets', name: '指数平滑', order: 2 },
    { id: 'forecast_arima', name: 'ARIMA / ARIMAX', order: 3 },
    { id: 'forecast_prophet', name: 'Prophet', order: 4 },
    { id: 'forecast_lgbm', name: 'LightGBM 时序', order: 5 }
  ];
  if (taskKey === 'regression') return [
    { id: 'linear', name: '线性回归', order: 1 },
    { id: 'regtree', name: '回归树', order: 2 },
    { id: 'regforest', name: '随机森林回归', order: 3 },
    { id: 'lgbmreg', name: 'LGBM 回归', order: 4 }
  ];
  return [
    { id: 'logistic', name: '逻辑回归', order: 1 },
    { id: 'tree', name: '决策树', order: 2 },
    { id: 'knn', name: 'KNN', order: 3 },
    { id: 'forest', name: '随机森林', order: 4 },
    { id: 'lgbm', name: 'LGBM', order: 5 }
  ];
}

// 复杂度顺序（越靠前越简单）
function smartSelectionComplexityRankV103(taskKey, modelId) {
  const list = smartSelectionModelCatalogV103(taskKey);
  const entry = list.find(item => item.id === modelId);
  return entry ? entry.order : Infinity;
}

// 就绪检查：是否允许进入智能选型
function smartSelectionReadyV103(owner = project(), set = dataset()) {
  const taskKey = smartSelectionTaskKeyV103(owner);
  const reasons = [];
  let ok = true;

  if (taskKey === 'forecasting') {
    ensureForecastDatasetV10(set);
    if (set.timeSummary?.blockers?.length) { ok = false; reasons.push('请先完成数据检查和特征准备'); }
    const horizon = forecastHorizonValidationV102(set);
    if (!horizon.valid) { ok = false; reasons.push(horizon.message); }
  } else if (taskKey === 'regression') {
    const targetCheck = regressionTargetCheckV910(set, set.target);
    if (!targetCheck.ok) { ok = false; reasons.push(targetCheck.message); }
    const included = set.columns.filter(c => c.included && c.trainable && !c.target);
    if (!included.length) { ok = false; reasons.push('请至少纳入一个可用于训练的特征'); }
  } else {
    const check = validateClassesV9(set, owner.classificationModeLocked ? owner.classificationMode : null);
    if (!check.ok) { ok = false; reasons.push(check.message); }
    const included = set.columns.filter(c => c.included && c.trainable && !c.target);
    if (!included.length) { ok = false; reasons.push('请至少纳入一个可用于训练的特征'); }
  }

  return { ok, message: ok ? '' : (reasons[0] || '请先完成数据检查和特征准备'), reasons };
}

// 数据画像
function smartSelectionProfileV103(owner = project(), set = dataset()) {
  const taskKey = smartSelectionTaskKeyV103(owner);
  const splitShare = Number((set.feature?.split || '80-20').split('-')[0] || 80) / 100;
  const trainRows = Math.floor(set.rows * splitShare);

  if (taskKey === 'forecasting') {
    ensureForecastDatasetV10(set);
    const windows = forecastBacktestWindowsV10(set);
    const firstTrain = windows[0]?.trainingIndexes?.length || trainRows;
    const numeric = forecastExternalColumnsV101(set).filter(c => c.type === 'number' && c.included);
    const firstLagCorrs = numeric.map(c => {
      const meta = forecastFeatureStatsV101(set, c);
      return meta.temporalCorrelation;
    }).filter(v => v !== null);
    const season = forecastSeasonPeriodV1022(set);
    return {
      trainRows: firstTrain,
      featureCount: numeric.length,
      externalCount: forecastExternalColumnsV101(set).filter(c => c.included).length,
      frequency: set.frequency,
      season,
      hasCalendar: set.forecastConfig?.calendarFeatures,
      firstLagCorr: firstLagCorrs.length ? firstLagCorrs.reduce((a, b) => a + Math.abs(b), 0) / firstLagCorrs.length : null,
      historyPoints: set.timeSummary?.historyPoints || trainRows
    };
  }
  ...
  let encodedDims = 0;
  included.forEach(c => {
    if (c.type === 'category') encodedDims += Math.max(2, c.unique || 2) + (c.missing > 0 && c.missing < 1 ? 1 : 0);
    else encodedDims += 1;
  });
  const numericCount = included.filter(c => c.type === 'number' || c.type === 'boolean').length;

  let minClassRatio = null;
  if (taskKey !== 'regression') {
    const summary = classSummaryV9(set);
    if (summary.entries.length) {
      minClassRatio = Math.min(...summary.entries.map(e => e[1])) / summary.total;
    }
  }

  return {
    trainRows,
    featureCount: included.length,
    encodedDims,
    numericCount,
    numericRatio: included.length ? numericCount / included.length : 0,
    hasCategory: included.some(c => c.type === 'category'),
    smallData: trainRows <= 5000,
    bigData: trainRows > 50000,
    fewFeatures: encodedDims <= 20,
    highDim: encodedDims > 100,
    imbalanced: minClassRatio !== null && minClassRatio < 0.1
  };
}

// 模型可用性判断
function smartSelectionAvailabilityV103(owner = project(), set = dataset()) {
  const taskKey = smartSelectionTaskKeyV103(owner);
  const profile = smartSelectionProfileV103(owner, set);
  const catalog = smartSelectionModelCatalogV103(taskKey);

  return catalog.map(entry => {
    const modelId = entry.id;
    let status = 'available';
    let reason = '可参与固定参数对比';

    if (modelId === 'knn') {
      if (profile.trainRows > 50000) { status = 'unavailable'; reason = '训练样本超过 KNN 的平台选型上限'; }
      else if (profile.encodedDims > 200) { status = 'unavailable'; reason = '编码后特征维度超过 KNN 的平台选型上限'; }
    }

    if (modelId === 'forecast_lgbm') {
      const season = forecastSeasonPeriodV1022(set);
      const lag = season * 2;
      const effectivePoints = Math.max(0, (set.timeSummary?.validPoints || profile.trainRows) - lag);
      if (effectivePoints < 30) { status = 'unavailable'; reason = '生成滞后特征后有效训练点少于 30'; }
    }

    // 预处理覆盖检查（仅标记明确无法处理的情况）
    if (status === 'available' && taskKey !== 'forecasting' && profile.featureCount === 0) {
      status = 'unavailable'; reason = '当前无有效入模特征';
    }

    return { modelId, status, reason };
  });
}

// 预推荐评分
function smartSelectionScoreV103(owner = project(), set = dataset()) {
  const taskKey = smartSelectionTaskKeyV103(owner);
  const profile = smartSelectionProfileV103(owner, set);
  const availability = smartSelectionAvailabilityV103(owner, set);
  const catalog = smartSelectionModelCatalogV103(taskKey);

  const scores = {};
  catalog.forEach(entry => {
    const modelId = entry.id;
    if (availability.find(a => a.modelId === modelId)?.status === 'unavailable') { scores[modelId] = { score: -Infinity, reason: '' }; return; }
    let score = 0;
    const reasons = [];

    if (taskKey === 'forecasting') {
      if (modelId === 'forecast_baseline') { score += 1; reasons.push('基准参考'); if ((profile.historyPoints || profile.trainRows) <= (set.timeSummary?.minimum || 60) + 10) { score += 2; reasons.push('历史长度接近最低要求'); } }
      if (modelId === 'forecast_ets') { if (profile.firstLagCorr !== null && profile.firstLagCorr >= 0.3) { score += 3; reasons.push('季节滞后相关明显'); } if ((profile.historyPoints || profile.trainRows) <= 120) { score += 1; reasons.push('历史较短'); } }
      if (modelId === 'forecast_arima') { if (profile.firstLagCorr !== null && profile.firstLagCorr >= 0.5) { score += 3; reasons.push('一阶自相关明显'); } if (profile.externalCount) { score += 2; reasons.push('存在外部特征'); } }
      if (modelId === 'forecast_prophet') { if (profile.firstLagCorr !== null && profile.firstLagCorr >= 0.3) { score += 3; reasons.push('季节滞后相关明显'); } if (profile.externalCount) { score += 2; reasons.push('存在外部特征'); } if ((profile.historyPoints || profile.trainRows) >= 180) { score += 1; reasons.push('历史长度充足'); } }
      if (modelId === 'forecast_lgbm') { if (profile.externalCount) { score += 3; reasons.push('存在外部特征'); } if ((profile.historyPoints || profile.trainRows) >= 180) { score += 1; reasons.push('历史长度充足'); } if (profile.externalCount > 2) { score += 1; reasons.push('外部特征较多'); } }
    } else if (taskKey === 'regression') {
      if (modelId === 'linear') { if (profile.smallData) { score += 2; reasons.push('小数据'); } if (profile.fewFeatures) { score += 2; reasons.push('少特征'); } if (profile.highDim) { score += 2; reasons.push('高维特征'); } if (profile.numericRatio >= 0.6) { score += 1; reasons.push('数值特征为主'); } }
      if (modelId === 'regtree') { if (profile.smallData) { score += 2; reasons.push('小数据'); } if (profile.fewFeatures) { score += 1; reasons.push('少特征'); } if (profile.hasCategory) { score += 1; reasons.push('存在类别特征'); } }
      if (modelId === 'regforest') { if (profile.hasCategory) { score += 2; reasons.push('存在类别特征'); } if (!profile.smallData) { score += 1; reasons.push('非小数据'); } if (profile.featureCount > 20) { score += 1; reasons.push('特征数超过 20'); } }
      if (modelId === 'lgbmreg') { if (profile.bigData) { score += 3; reasons.push('大数据'); } if (profile.hasCategory) { score += 2; reasons.push('存在类别特征'); } if (profile.highDim) { score += 1; reasons.push('高维特征'); } if (profile.featureCount > 20) { score += 1; reasons.push('特征数超过 20'); } }
    } else {
      if (modelId === 'logistic') { if (profile.smallData) { score += 2; reasons.push('小数据'); } if (profile.fewFeatures) { score += 2; reasons.push('少特征'); } if (profile.highDim) { score += 2; reasons.push('高维特征'); } if (profile.imbalanced) { score += 1; reasons.push('类别不均衡'); } }
      if (modelId === 'tree') { if (profile.smallData) { score += 2; reasons.push('小数据'); } if (profile.fewFeatures) { score += 1; reasons.push('少特征'); } if (profile.hasCategory) { score += 1; reasons.push('存在类别特征'); } }
      if (modelId === 'forest') { if (profile.imbalanced) { score += 2; reasons.push('类别不均衡'); } if (profile.hasCategory) { score += 2; reasons.push('存在类别特征'); } if (!profile.smallData) { score += 1; reasons.push('非小数据'); } }
      if (modelId === 'knn') { if (profile.trainRows <= 5000 && profile.encodedDims <= 20 && profile.numericRatio >= 0.6) { score += 4; reasons.push('样本量小、维度低且数值特征为主'); } }
      if (modelId === 'lgbm') { if (profile.bigData) { score += 3; reasons.push('大数据'); } if (profile.hasCategory) { score += 2; reasons.push('存在类别特征'); } if (profile.imbalanced) { score += 2; reasons.push('类别不均衡'); } if (profile.highDim) { score += 1; reasons.push('高维特征'); } }
    }

    scores[modelId] = { score, reason: reasons[0] || '可参与固定参数对比' };
  });
...
// 固定参数预设
function smartSelectionFixedParamsV103(modelId, owner = project(), set = dataset()) {
  if (modelId === 'logistic') return { standardize: true, C: 1.0, solver: 'lbfgs', maxIter: 100 };
  if (modelId === 'tree') return { maxDepth: 6 };
  if (modelId === 'forest') return { nEstimators: 200, maxDepth: 10 };
  if (modelId === 'knn') return { nNeighbors: 5, weights: 'distance' };
  if (modelId === 'lgbm') return { trees: 200, learningRate: 0.05, leaves: 31 };
  if (modelId === 'linear') return { standardize: true, alpha: 0.0 };
  if (modelId === 'regtree') return { maxDepth: 6 };
  if (modelId === 'regforest') return { nEstimators: 200, maxDepth: 10 };
  if (modelId === 'lgbmreg') return { trees: 200, learningRate: 0.05, leaves: 31 };

  if (modelId.startsWith('forecast_')) {
    const season = forecastSeasonPeriodV1022(set);
    if (modelId === 'forecast_baseline') return { strategy: 'seasonal_naive', fallback: 'last' };
    if (modelId === 'forecast_ets') return { trend: 'add', seasonalityMode: 'add', season, damped: true, advanced: true };
    if (modelId === 'forecast_arima') {
      const hasSeason = season >= 2;
      return { p: 1, d: 1, q: 1, seasonal: hasSeason, P: 1, D: 1, Q: 1, m: season, advanced: true };
    }
    if (modelId === 'forecast_prophet') return { changepoint: 0.05, seasonality: 10, seasonalityMode: 'additive', advanced: true };
    if (modelId === 'forecast_lgbm') return { trees: 200, learningRate: 0.05, leaves: 31, depth: 6, lag: season * 2, advanced: true };
  }
  return {};
}

// 中文格式化固定参数（V10.3.1）
function smartSelectionFormatParamsV1031(modelId, owner = project(), set = dataset()) {
  const params = { ...smartSelectionFixedParamsV103(modelId, owner, set) };
  delete params.advanced;
  const valueMap = { true: '是', false: '否', add: '加法', additive: '加法', mul: '乘法', multiplicative: '乘法', none: '无', distance: '距离加权', uniform: '等权', seasonal_naive: '季节朴素法', last: '使用上一期值' };
  const keyMap = { standardize: '特征标准化', C: '正则化强度', solver: '求解器', maxIter: '最大迭代次数', maxDepth: '最大深度', nEstimators: '树数量', nNeighbors: '邻居数量', weights: '距离权重', trees: '树数量', learningRate: '学习率', leaves: '叶子数', alpha: '正则化强度', strategy: '基线策略', fallback: '历史不足时', trend: '趋势类型', seasonalityMode: '季节性模式', season: '季节周期', damped: '阻尼趋势', p: '自回归阶数 p', d: '差分阶数 d', q: '移动平均阶数 q', seasonal: '季节项', P: '季节自回归 P', D: '季节差分 D', Q: '季节移动平均 Q', m: '季节周期 m', changepoint: '趋势灵活度', seasonality: '季节性强度', depth: '最大深度', lag: '最大滞后期' };
  const booleanOverrides = { standardize: { true: '开启', false: '关闭' }, damped: { true: '启用', false: '停用' }, seasonal: { true: '启用', false: '停用' } };
  return Object.entries(params).map(([key, value]) => {
    const displayValue = booleanOverrides[key]?.[String(value)] ?? valueMap[String(value)] ?? value;
    return `${keyMap[key] || key}：${displayValue}`;
  }).join(' · ');
}

// 配置快照
function smartSelectionSnapshotV103(owner = project(), set = dataset(), pool = []) {
  const taskKey = smartSelectionTaskKeyV103(owner);
  const included = set.columns.filter(c => c.included && c.trainable && !c.target).map(c => c.name).sort();
  const base = {
    projectId: owner.id,
    datasetId: set.id,
    taskKey,
    target: set.target,
    positive: owner.task === 'classification' ? (set.positive || owner.positive) : null,
    classificationMode: owner.classificationMode || null,
    features: included,
    featureRevision: set.feature?.revision || 0,
    split: set.feature?.split || '80-20',
    pool: [...pool].sort(),
    paramVersion: SMART_SELECTION_PARAM_VERSION_V103
  };
  if (taskKey === 'forecasting') {
    base.timeColumn = set.timeColumn;
    base.frequency = set.frequency;
    base.horizon = set.forecastConfig?.horizon;
    base.calendarFeatures = set.forecastConfig?.calendarFeatures;
    base.externalFeatures = forecastExternalColumnsV101(set).filter(c => c.included).map(c => c.name).sort();
    base.backtestRounds = 3;
  }
  return base;
}

// 稳定指纹
function smartSelectionFingerprintV103(owner = project(), set = dataset(), pool = []) {
  const snapshot = smartSelectionSnapshotV103(owner, set, pool);
  const normalized = JSON.stringify(snapshot, Object.keys(snapshot).sort());
  let hash = 0;
  for (let i = 0; i < normalized.length; i += 1) { hash = ((hash << 5) - hash) + normalized.charCodeAt(i); hash |= 0; }
  return 'ss-' + Math.abs(hash).toString(36);
}

// 确定性伪随机数（同一指纹 + 模型 ID 得到同一序列）
function smartSelectionSeededRandomV103(seed) {
  let s = 0;
  for (let i = 0; i < seed.length; i += 1) { s = ((s << 5) - s) + seed.charCodeAt(i); s |= 0; }
  return () => {
...
// 失败原因（确定性）
function smartSelectionFailureReasonV103(modelId, owner = project(), set = dataset()) {
  const availability = smartSelectionAvailabilityV103(owner, set).find(a => a.modelId === modelId);
  if (availability?.status === 'unavailable') return availability.reason;
  // 只有明确预处理无法覆盖时才失败；默认不失败
  return '';
}

// 稳定 Mock 结果
function smartSelectionMockResultV103(modelId, owner = project(), set = dataset()) {
  const taskKey = smartSelectionTaskKeyV103(owner);
  const reason = smartSelectionFailureReasonV103(modelId, owner, set);
  if (reason) return { modelId, status: 'failed', reason };

  const fingerprint = smartSelectionFingerprintV103(owner, set, [modelId]);
  const rng = smartSelectionSeededRandomV103(`${fingerprint}:${modelId}`);
  const complexity = smartSelectionComplexityRankV103(taskKey, modelId);

  if (taskKey === 'forecasting') {
    const season = forecastSeasonPeriodV1022(set);
    const baseMae = 0.08 + complexity * 0.015 + rng() * 0.03;
    const backtests = [1, 2, 3].map(round => {
      const noise = rng() * 0.02 - 0.01;
      const mae = Math.max(0.01, baseMae + noise);
      const rmse = +(mae * (1.15 + rng() * 0.25)).toFixed(3);
      const smape = +(mae * (80 + rng() * 40)).toFixed(1);
      return { round, mae: +mae.toFixed(3), rmse, smape };
    });
    const mae = +(backtests.reduce((a, b) => a + b.mae, 0) / 3).toFixed(3);
    const rmse = +(backtests.reduce((a, b) => a + b.rmse, 0) / 3).toFixed(3);
    const smape = +(backtests.reduce((a, b) => a + b.smape, 0) / 3).toFixed(1);
    const stability = +Math.sqrt(backtests.reduce((a, b) => a + Math.pow(b.mae - mae, 2), 0) / 3).toFixed(3);
    return { modelId, status: 'success', metrics: { mae, rmse, smape, stability, backtests } };
  }

  if (taskKey === 'regression') {
    const targetValues = set.preview.map(row => Number(row[set.columns.findIndex(c => c.target)])).filter(Number.isFinite);
    const scale = targetValues.length ? Math.max(1, targetValues.reduce((a, b) => a + b, 0) / targetValues.length) : 100;
    const baseRmse = scale * (0.12 + complexity * 0.03 + rng() * 0.05);
    const rmse = +baseRmse.toFixed(2);
    const mae = +(rmse * (0.7 + rng() * 0.15)).toFixed(2);
    const r2 = +(0.92 - complexity * 0.03 - rng() * 0.06).toFixed(3);
    const trainRmse = +(rmse * (0.92 + rng() * 0.12)).toFixed(2);
    const gap = +(trainRmse - rmse).toFixed(2);
    return { modelId, status: 'success', metrics: { rmse, mae, r2, gap, trainRmse } };
  }

  if (taskKey === 'multiclass') {
    const baseMacroF1 = 0.88 - complexity * 0.03 - rng() * 0.05;
    const macroF1 = +Math.max(0.1, Math.min(0.99, baseMacroF1)).toFixed(3);
    const accuracy = +Math.max(0.1, Math.min(0.99, macroF1 + 0.02 + rng() * 0.04)).toFixed(3);
    const weightedF1 = +Math.max(0.1, Math.min(0.99, macroF1 + 0.01 + rng() * 0.03)).toFixed(3);
    const logLoss = +(0.35 + complexity * 0.05 + rng() * 0.1).toFixed(3);
    const trainMacroF1 = +Math.min(0.99, macroF1 + 0.02 + rng() * 0.03).toFixed(3);
    const gap = +(trainMacroF1 - macroF1).toFixed(3);
    return { modelId, status: 'success', metrics: { macroF1, accuracy, weightedF1, logLoss, gap, trainMacroF1 } };
  }

  // 二分类
  const baseAuc = 0.92 - complexity * 0.02 - rng() * 0.04;
  const auc = +Math.max(0.5, Math.min(0.99, baseAuc)).toFixed(3);
  const f1 = +Math.max(0.1, Math.min(0.99, 0.78 - complexity * 0.02 + rng() * 0.06)).toFixed(2);
  const ks = +Math.max(0.1, Math.min(0.99, 0.62 - complexity * 0.02 + rng() * 0.08)).toFixed(2);
  const accuracy = +Math.max(0.1, Math.min(0.99, 0.9 - complexity * 0.01 + rng() * 0.04)).toFixed(2);
  const trainAuc = +Math.min(0.99, auc + 0.01 + rng() * 0.02).toFixed(3);
  const gap = +(trainAuc - auc).toFixed(3);
  return { modelId, status: 'success', metrics: { auc, f1, ks, accuracy, gap, trainAuc } };
}

// 主指标取值与方向
function smartSelectionPrimaryMetricV103(metrics, taskKey) {
  if (taskKey === 'forecasting') return { key: 'mae', value: metrics.mae, direction: 'asc' };
  if (taskKey === 'regression') return { key: 'rmse', value: metrics.rmse, direction: 'asc' };
  if (taskKey === 'multiclass') return { key: 'macroF1', value: metrics.macroF1, direction: 'desc' };
  return { key: 'auc', value: metrics.auc, direction: 'desc' };
}

// 最终推荐
function smartSelectionRecommendV103(results, taskKey) {
  const success = results.filter(r => r.status === 'success');
  if (success.length < 2) return { ranking: success, recommendedModelId: null, tie: false, message: '成功模型不足 2 个，暂不生成推荐' };

  const primary = r => smartSelectionPrimaryMetricV103(r.metrics, taskKey);
  success.sort((a, b) => {
    const pa = primary(a), pb = primary(b);
    if (pa.direction === 'asc') return pa.value - pb.value;
    return pb.value - pa.value;
  });

  const best = success[0], second = success[1];
  const pBest = primary(best), pSecond = primary(second);
  let tie = false;
  if (pBest.direction === 'desc') {
    tie = Math.abs(pBest.value - pSecond.value) <= 0.01;
  } else {
    const denom = pBest.value === 0 ? (pSecond.value === 0 ? 1 : pSecond.value) : pBest.value;
    tie = Math.abs(pSecond.value - pBest.value) / Math.abs(denom) <= 0.02;
  }

  let recommendedModelId = best.modelId;
  if (tie) {
    const rankBest = smartSelectionComplexityRankV103(taskKey, best.modelId);
    const rankSecond = smartSelectionComplexityRankV103(taskKey, second.modelId);
    if (rankSecond < rankBest) recommendedModelId = second.modelId;
  }

  return { ranking: success, recommendedModelId, tie, message: '' };
}

// 清理模型池草稿：保留当前任务可用模型，时序补回基线
function smartSelectionSanitizePoolV103(owner = project(), set = dataset()) {
  const taskKey = smartSelectionTaskKeyV103(owner);
  const catalogIds = new Set(smartSelectionModelCatalogV103(taskKey).map(entry => entry.id));
  const availability = smartSelectionAvailabilityV103(owner, set);
  let pool = (state.smartSelectionPool || []).filter(id => catalogIds.has(id) && availability.find(a => a.modelId === id)?.status !== 'unavailable');
  if (taskKey === 'forecasting') {
    if (!pool.includes('forecast_baseline')) pool.unshift('forecast_baseline');
    pool = pool.slice(0, SMART_SELECTION_MAX_POOL_V103.forecasting);
  }
  if (pool.length < SMART_SELECTION_MIN_POOL_V103[taskKey]) pool = smartSelectionDefaultPoolV103(owner, set);
  return pool;
}

// 默认模型池
function smartSelectionDefaultPoolV103(owner = project(), set = dataset()) {
  const taskKey = smartSelectionTaskKeyV103(owner);
  const availability = smartSelectionAvailabilityV103(owner, set);
  const scores = smartSelectionScoreV103(owner, set);
  const available = smartSelectionModelCatalogV103(taskKey)
    .filter(entry => availability.find(a => a.modelId === entry.id)?.status !== 'unavailable')
    .sort((a, b) => (scores[b.id]?.score || 0) - (scores[a.id]?.score || 0) || a.order - b.order)
    .map(entry => entry.id);

  if (taskKey === 'forecasting') {
    const nonBaseline = available.filter(id => id !== 'forecast_baseline');
    const top = nonBaseline.slice(0, 2);
    return ['forecast_baseline', ...top];
  }
  return available.slice(0, 3);
}

// 失效检查
function smartSelectionIsInvalidV103(record, owner = project(), set = dataset()) {
  if (!record || record.projectId !== owner.id || record.datasetId !== set.id) return true;
  const current = smartSelectionSnapshotV103(owner, set, record.pool);
  return JSON.stringify(record.snapshot, Object.keys(record.snapshot).sort()) !== JSON.stringify(current, Object.keys(current).sort()) ||
    record.paramVersion !== SMART_SELECTION_PARAM_VERSION_V103;
}

// 当前数据集最近一条有效且未失效的推荐记录（V10.3.1）
function smartSelectionLatestValidRecordV1031(owner = project(), set = dataset()) {
  const records = state.smartSelections.filter(r =>
    r.datasetId === set.id && r.status === 'completed' && r.recommendedModelId && !smartSelectionIsInvalidV103(r, owner, set)
  );
...
// 智能选型卡片（插入到模型选择页）
function smartSelectionCardV103(owner = project(), set = dataset()) {
  const ready = smartSelectionReadyV103(owner, set);
  if (!ready.ok) {
    return `<article class="model-card smart-selection-card-v103 disabled"><span class="smart-selection-icon">✦</span><div class="smart-selection-tag-v103">推荐入口</div><h3>智能选型</h3><p>使用多个模型的固定方案快速比较并推荐后续重点模型。</p><span class="status blocked">${esc(ready.message)}</span></article>`;
  }
  const record = smartSelectionLatestValidRecordV1031(owner, set);
  if (record) {
    const recModel = smartSelectionModelCatalogV103(record.taskKey).find(e => e.id === record.recommendedModelId);
    return `<article class="model-card smart-selection-card-v103" data-action="open-smart-selection-v103"><span class="smart-selection-icon">✦</span><h3>智能选型</h3><p>使用多个模型的固定方案快速比较并推荐后续重点模型。</p><div class="smart-selection-card-actions-v1031">${button('查看选型结果', `open-smart-selection-result-v103:${record.id}`, 'primary')}${button('重新选型', 'open-smart-selection-config-v1031')}<div class="smart-selection-reco-banner-v1034"><span>★</span> 推荐模型：<b>${esc(recModel?.name || record.recommendedModelId)}</b></div></div></article>`;
  }
  return `<article class="model-card smart-selection-card-v103" data-action="open-smart-selection-v103"><span class="smart-selection-icon">✦</span><div class="smart-selection-tag-v103">推荐入口</div><h3>智能选型</h3><p>使用多个模型的固定方案快速比较并推荐后续重点模型。</p>${button('开始智能选型', 'open-smart-selection-v103', 'primary')}</article>`;
}

// 模型池配置页
function smartSelectionConfigPageV103() {
  const owner = project(), set = dataset(), taskKey = smartSelectionTaskKeyV103(owner);
  const ready = smartSelectionReadyV103(owner, set);
  if (!ready.ok) return shell(`${steps('experiments')}${pageHead('智能选型', esc(set.name))}<div class="notice warning"><b>暂不可用</b><span>${esc(ready.message)}</span></div><div class="flow-footer">${button('返回模型训练', 'go-experiments')}</div>`);

  const sanitized = smartSelectionSanitizePoolV103(owner, set);
  if (JSON.stringify(state.smartSelectionPool) !== JSON.stringify(sanitized)) state.smartSelectionPool = sanitized;
  const pool = state.smartSelectionPool;
  const availability = smartSelectionAvailabilityV103(owner, set);
  const scores = smartSelectionScoreV103(owner, set);
  const max = SMART_SELECTION_MAX_POOL_V103[taskKey];
  const min = SMART_SELECTION_MIN_POOL_V103[taskKey];
  const catalog = smartSelectionModelCatalogV103(taskKey);

  const cards = catalog.map(entry => {
    const avail = availability.find(a => a.modelId === entry.id);
    const isUnavailable = avail?.status === 'unavailable';
    const isBaseline = taskKey === 'forecasting' && entry.id === 'forecast_baseline';
    const checked = isBaseline || pool.includes(entry.id);
    const disabled = isUnavailable || (isBaseline && checked);
    const atMax = pool.length >= max && !checked;
    const score = scores[entry.id];
    const reason = isUnavailable ? avail.reason : (score?.reason || '可参与固定参数对比');
    const statusClass = isUnavailable ? 'unavailable' : (score?.score > 0 ? 'recommended' : 'available');
    return `<article class="smart-selection-model-card-v103 ${statusClass} ${checked ? 'selected' : ''} ${disabled || atMax ? 'disabled' : ''}" data-smart-selection-model-v103="${entry.id}"><div class="smart-selection-card-header-v103"><h4>${esc(entry.name)}</h4><span class="smart-selection-status-v103 ${statusClass}">${isUnavailable ? '不可用' : score?.score > 0 ? '推荐' : '可用'}</span></div><p>${esc(reason)}</p><label class="smart-selection-check-v103"><input type="checkbox" data-smart-selection-toggle-v103="${entry.id}" ${checked ? 'checked' : ''} ${disabled || atMax ? 'disabled' : ''}><span>${isBaseline ? '基线参与对比' : '加入模型池'}</span></label></article>`;
  }).join('');

  const canStart = pool.length >= min && (taskKey !== 'forecasting' || pool.includes('forecast_baseline') && pool.filter(id => id !== 'forecast_baseline').length >= 1);
  return shell(`${steps('experiments')}${pageHead('智能选型', `${esc(set.name)} · ${taskKey === 'classification' ? '二分类' : taskKey === 'multiclass' ? '多分类' : taskKey === 'regression' ? '回归' : '时序预测'}`)}<div class="smart-selection-config-v103"><div class="panel-head"><div><h2>模型池配置</h2><p>本次将运行 ${pool.length} 个固定参数方案，最少 ${min} 个，最多 ${max} 个。</p></div><span class="status good">已选择 ${pool.length} / ${max}</span></div><div class="smart-selection-pool-v103">${cards}</div><details class="advanced"><summary>平台基准配置</summary><div class="smart-selection-baseline-v1031"><p>所有模型使用相同的目标列、入模特征和训练/验证划分；参数不可编辑。</p><div>${pool.map(id => `<div class="smart-selection-baseline-row-v1031"><b>${esc(smartSelectionModelCatalogV103(taskKey).find(e => e.id === id)?.name || id)}</b><span>${esc(smartSelectionFormatParamsV1031(id, owner, set))}</span></div>`).join('')}</div></div></details></div><div class="flow-footer"><div>${button('返回模型训练', 'go-experiments')}</div><div>${button('开始智能选型', 'start-smart-selection-v103', canStart ? 'primary' : '')}</div></div>`);
}

// 智能选型结果页
function smartSelectionResultPageV103() {
  const owner = project(), set = dataset();
  const record = state.smartSelections.find(r => r.id === state.smartSelectionId);
  if (!record) { go('experiments'); return ''; }
  const invalid = smartSelectionIsInvalidV103(record, owner, set);
  const taskKey = record.taskKey;

  let body;
  if (invalid) {
    body = `<div class="notice warning"><b>配置已变化，请重新选型</b><span>当前数据集或特征配置与本次选型记录不一致。</span></div>`;
  } else if (record.recommendedModelId) {
    const rec = smartSelectionModelCatalogV103(taskKey).find(e => e.id === record.recommendedModelId);
    const result = record.results.find(r => r.modelId === record.recommendedModelId);
    const pm = smartSelectionPrimaryMetricV103(result.metrics, taskKey);
    body = `<div class="smart-selection-recommend-v103"><div class="section-title"><h2>推荐模型：${esc(rec?.name || record.recommendedModelId)}</h2><span>模型池排名第 1 · 主指标 ${pm.key.toUpperCase()} ${pm.value}</span></div><div class="smart-selection-recommend-actions-v103">${button('使用推荐模型创建实验', `create-from-smart-selection-v103:${record.recommendedModelId}`, 'primary')}${button('选择其他模型', 'goto-models-from-selection-v1031')}${button('查看全部对比结果', 'toggle-smart-selection-results-v103')}</div></div>`;
  } else {
    body = `<div class="notice"><b>${esc(record.message || '成功模型不足 2 个，暂不生成推荐')}</b><span>可从下方成功模型中选择一个创建正式实验。</span></div><div>${button('查看全部对比结果', 'toggle-smart-selection-results-v103')}</div>`;
  }

  const showAll = state.smartSelectionShowAllResults;
  const table = showAll ? smartSelectionResultTableV103(record, invalid) : '';

  return shell(`${steps('experiments')}${pageHead('智能选型结果', `${esc(set.name)} · ${record.completedAt || ''}`)}<div class="smart-selection-result-v103">${body}${table}</div><div class="flow-footer"><div>${button('返回模型训练', 'go-experiments')}</div></div>`);
}

// 结果对比表
function smartSelectionResultTableV103(record, invalid) {
  const taskKey = record.taskKey;
  const success = record.ranking.map(modelId => record.results.find(r => r.modelId === modelId)).filter(Boolean);
  const failed = record.results.filter(r => r.status === 'failed');
  const primary = r => smartSelectionPrimaryMetricV103(r.metrics, taskKey);

  const headers = taskKey === 'forecasting' ? ['排名', '模型类型', 'MAE', 'RMSE', 'sMAPE', '稳定性', '操作']
    : taskKey === 'regression' ? ['排名', '模型类型', 'RMSE', 'MAE', 'R²', '训练/验证差值', '操作']
    : taskKey === 'multiclass' ? ['排名', '模型类型', 'Macro F1', 'Accuracy', 'Weighted F1', 'Log Loss', '操作']
    : ['排名', '模型类型', 'AUC', 'F1', 'KS', '训练/验证差值', '操作'];

  const successRows = success.map((result, index) => {
    const model = smartSelectionModelCatalogV103(taskKey).find(e => e.id === result.modelId);
    const existingId = record.experimentMap?.[result.modelId];
    const hasExperiment = existingId && state.experiments[existingId];
    return `<tr><td><b>#${index + 1}</b></td><td>${esc(model?.name || result.modelId)}</td>${smartSelectionMetricCellsV103(result.metrics, taskKey)}<td class="smart-selection-action-col-v103">${invalid ? '' : (hasExperiment ? button('查看实验', `open-smart-selection-experiment-v103:${existingId}`) : button('创建实验', `create-from-smart-selection-v103:${result.modelId}`, 'primary'))}</td></tr>`;
  }).join('');

  const failedRows = failed.map(result => {
    const model = smartSelectionModelCatalogV103(taskKey).find(e => e.id === result.modelId);
    return `<tr class="smart-selection-failed-v103"><td>—</td><td>${esc(model?.name || result.modelId)}</td><td colspan="${headers.length - 2}"><span class="status blocked">${esc(result.reason)}</span></td><td>—</td></tr>`;
  }).join('');

  return `<div class="smart-selection-table-wrap-v103"><table class="smart-selection-table-v103"><thead><tr>${headers.map(h => `<th>${esc(h)}</th>`).join('')}</tr></thead><tbody>${successRows}${failedRows}</tbody></table></div>`;
}

function smartSelectionMetricCellsV103(metrics, taskKey) {
  if (taskKey === 'forecasting') return `<td>${metrics.mae.toFixed(3)}</td><td>${metrics.rmse.toFixed(3)}</td><td>${metrics.smape.toFixed(1)}%</td><td>${metrics.stability.toFixed(3)}</td>`;
  if (taskKey === 'regression') return `<td>${metrics.rmse.toFixed(2)}</td><td>${metrics.mae.toFixed(2)}</td><td>${metrics.r2.toFixed(3)}</td><td>${metrics.gap.toFixed(2)}</td>`;
  if (taskKey === 'multiclass') return `<td>${metrics.macroF1.toFixed(3)}</td><td>${metrics.accuracy.toFixed(3)}</td><td>${metrics.weightedF1.toFixed(3)}</td><td>${metrics.logLoss.toFixed(3)}</td>`;
  return `<td>${metrics.auc.toFixed(3)}</td><td>${metrics.f1.toFixed(2)}</td><td>${metrics.ks.toFixed(2)}</td><td>${metrics.gap.toFixed(3)}</td>`;
}

// 历史记录区
function smartSelectionHistoryV103(set = dataset()) {
  const records = state.smartSelections.filter(r => r.datasetId === set.id).sort((a, b) => new Date(b.completedAt || 0) - new Date(a.completedAt || 0));
  if (!records.length) return '';
  const showAll = state.smartSelectionShowAllHistory;
  const display = showAll ? records : records.slice(0, 3);
  const rows = display.map(record => {
    const taskKey = record.taskKey;
    const rec = record.recommendedModelId ? smartSelectionModelCatalogV103(taskKey).find(e => e.id === record.recommendedModelId) : null;
    const result = record.recommendedModelId ? record.results.find(r => r.modelId === record.recommendedModelId) : null;
    const pm = result ? smartSelectionPrimaryMetricV103(result.metrics, taskKey) : null;
    const invalid = smartSelectionIsInvalidV103(record, project(), set);
    return `<div class="smart-selection-history-item-v103 ${invalid ? 'invalid' : ''}"><div><b>${esc(record.completedAt || '')}</b><span>${record.pool.map(id => esc(smartSelectionModelCatalogV103(taskKey).find(e => e.id === id)?.name || id)).join('、')}</span></div><div>${rec ? `推荐：${esc(rec.name)} ${pm ? `${pm.key.toUpperCase()} ${pm.value}` : ''}` : '未形成推荐'}</div><div><span class="status ${invalid ? 'blocked' : 'good'}">${invalid ? '已失效' : '完成'}</span></div><div>${button('查看结果', `open-smart-selection-result-v103:${record.id}`)}${button('删除', `delete-smart-selection-v103:${record.id}`)}</div></div>`;
  }).join('');
  return `<div class="smart-selection-history-v103"><div class="section-title"><h2>历史智能选型记录</h2>${records.length > 3 ? button(showAll ? '收起' : '查看全部', 'toggle-smart-selection-history-v103') : ''}</div>${rows}</div>`;
}

// 运行进度弹窗（V10.3.1：复用训练弹窗结构）
function smartSelectionRunDialogV103() {
  const run = state.smartSelectionRun;
  if (!run) return '';
  const progress = Math.round(run.index / run.total * 100);
  const currentIndex = Math.min(run.index + 1, run.total);
  const modelName = esc(smartSelectionModelCatalogV103(run.taskKey).find(e => e.id === run.current)?.name || run.current);
  return `<div class="training-overlay" data-smart-selection-backdrop-v103><div class="training-dialog"><div class="panel-head"><div><h2>正在进行智能选型</h2><p>使用平台基准参数</p></div><button class="training-minimize" data-action="minimize-smart-selection-v103" title="最小化后选型将在后台继续"><span aria-hidden="true">—</span><b>最小化</b></button></div><div class="training-stage"><b>正在运行 ${currentIndex} / ${run.total}：${modelName}</b><span>${progress}%</span></div><div class="progress"><i style="width:${progress}%"></i></div>${button('取消选型', 'cancel-smart-selection-v103')}</div></div>`;
}

// 最小化运行卡片（V10.3.1）
function smartSelectionRunMiniV1031() {
  const run = state.smartSelectionRun;
  if (!run || !run.minimized) return '';
  const progress = Math.round(run.index / run.total * 100);
  return `<div class="training-mini" data-smart-selection-mini-v1031><div class="training-stage"><b>智能选型运行中</b><span>${progress}%</span></div><div class="progress"><i style="width:${progress}%"></i></div><div class="row-actions">${button('展开', 'expand-smart-selection-v1031')}${button('取消', 'cancel-smart-selection-v103')}</div></div>`;
}

// 包装模型选择页：在第一位插入智能选型卡片
const modelSelectPageBeforeV103 = modelSelectPage;
modelSelectPage = function () {
  const html = modelSelectPageBeforeV103();
  if (html.includes('smart-selection-card-v103')) return html;
  const owner = project(), set = dataset();
  const card = smartSelectionCardV103(owner, set);
  return html.replace(/<div class="model-grid">/, `<div class="model-grid">${card}`);
};
...
// 包装实验列表页：追加历史记录区
const experimentsPageBeforeV103 = experimentsPage;
experimentsPage = function () {
  const html = experimentsPageBeforeV103();
  if (html.includes('smart-selection-history-v103')) return html;
  const set = dataset();
  const history = smartSelectionHistoryV103(set);
  if (!history) return html;
  return html.replace(/<\/section><\/main>/, `${history}</section></main>`);
};

// 智能选型 action 处理器
const actionBeforeV103 = action;
action = function (name) {
  if (name === 'open-smart-selection-v103') {
    const owner = project(), set = dataset();
    const ready = smartSelectionReadyV103(owner, set);
    if (!ready.ok) return toast(ready.message, 'warning');
    const pool = smartSelectionDefaultPoolV103(owner, set);
    const fingerprint = smartSelectionFingerprintV103(owner, set, pool);
    const existing = state.smartSelections.find(r => r.fingerprint === fingerprint && r.status === 'completed' && !smartSelectionIsInvalidV103(r, owner, set));
    if (existing) {
      state.smartSelectionId = existing.id;
      state.smartSelectionShowAllResults = false;
      toast('当前配置已完成智能选型，已打开已有结果。', 'success');
      return go('smart-selection-result');
    }
    state.smartSelectionPool = pool;
    state.smartSelectionShowAllResults = false;
    return go('smart-selection-config');
  }

  if (name === 'open-smart-selection-config-v1031') {
    const owner = project(), set = dataset();
    const ready = smartSelectionReadyV103(owner, set);
    if (!ready.ok) return toast(ready.message, 'warning');
    state.smartSelectionPool = smartSelectionDefaultPoolV103(owner, set);
    state.smartSelectionShowAllResults = false;
    return go('smart-selection-config');
  }

  if (name === 'start-smart-selection-v103') {
    const owner = project(), set = dataset(), taskKey = smartSelectionTaskKeyV103(owner);
    const ready = smartSelectionReadyV103(owner, set);
    if (!ready.ok) return toast(ready.message, 'warning');
    const pool = smartSelectionSanitizePoolV103(owner, set);
    const max = SMART_SELECTION_MAX_POOL_V103[taskKey], min = SMART_SELECTION_MIN_POOL_V103[taskKey];
    if (pool.length < min) return toast(`请至少选择 ${min} 个模型`, 'warning');
    if (pool.length > max) return toast(`最多选择 ${max} 个模型`, 'warning');
    if (taskKey === 'forecasting' && (!pool.includes('forecast_baseline') || pool.filter(id => id !== 'forecast_baseline').length < 1)) return toast('时序选型必须包含基线和至少一个非基线模型', 'warning');
    ...
    if (state.smartSelectionRun) return toast('已有正在进行的智能选型', 'warning');

    state.smartSelectionRunToken = (state.smartSelectionRunToken || 0) + 1;
    const token = state.smartSelectionRunToken;
    state.smartSelectionRun = { taskKey, pool, index: 0, total: pool.length, current: pool[0], results: [] };
    save(); render();

    const runNext = () => {
      if (token !== state.smartSelectionRunToken) return;
      const run = state.smartSelectionRun;
      if (!run) return;
      if (run.index >= run.total) {
        const results = run.results;
        const recommendation = smartSelectionRecommendV103(results, taskKey);
        if (recommendation.ranking.length === 0) {
          state.smartSelectionRun = null;
          save(); render();
          return toast('本次选型未产生可比较结果，请返回调整模型池', 'warning');
        }
        const record = {
          id: uid('ss'), projectId: owner.id, datasetId: set.id, taskKey,
          status: 'completed', createdAt: now(), completedAt: now(),
          fingerprint, paramVersion: SMART_SELECTION_PARAM_VERSION_V103,
          snapshot: smartSelectionSnapshotV103(owner, set, pool),
          pool, availability: smartSelectionAvailabilityV103(owner, set).filter(a => pool.includes(a.modelId)),
          presets: Object.fromEntries(pool.map(id => [id, smartSelectionFixedParamsV103(id, owner, set)])),
          results, ranking: recommendation.ranking.map(r => r.modelId),
          recommendedModelId: recommendation.recommendedModelId,
          message: recommendation.message, experimentMap: {}
        };
        state.smartSelections.push(record);
        state.smartSelectionId = record.id;
        state.smartSelectionRun = null;
        save(); go('smart-selection-result');
        return;
      }
      run.current = run.pool[run.index];
      const result = smartSelectionMockResultV103(run.current, owner, set);
  ...
  if (name === 'cancel-smart-selection-v103') {
    state.smartSelectionRunToken = (state.smartSelectionRunToken || 0) + 1;
    state.smartSelectionRun = null;
    save(); render();
    return;
  ...
  if (name === 'minimize-smart-selection-v103') {
    if (state.smartSelectionRun) { state.smartSelectionRun.minimized = true; save(); render(); }
    return;
  }

  if (name === 'expand-smart-selection-v1031') {
    if (state.smartSelectionRun) { state.smartSelectionRun.minimized = false; save(); render(); }
    return;
  }

  if (name === 'goto-models-from-selection-v1031') {
    save(); return go('model-select');
  }

  if (name === 'toggle-smart-selection-results-v103') {
    state.smartSelectionShowAllResults = !state.smartSelectionShowAllResults;
    save(); render();
    return;
  }

  if (name === 'toggle-smart-selection-history-v103') {
    state.smartSelectionShowAllHistory = !state.smartSelectionShowAllHistory;
    save(); render();
    return;
  }

  if (name.startsWith('open-smart-selection-result-v103:')) {
    const id = name.split(':')[1];
    const record = state.smartSelections.find(r => r.id === id);
    if (!record) return toast('选型记录不存在', 'warning');
    state.smartSelectionId = id;
    state.smartSelectionShowAllResults = false;
    return go('smart-selection-result');
  }

  if (name.startsWith('open-smart-selection-experiment-v103:')) {
    const id = name.split(':')[1];
    state.experimentId = id;
    return go('experiment');
  }

  if (name.startsWith('create-from-smart-selection-v103:')) {
    const modelId = name.split(':')[1];
    const owner = project(), set = dataset();
    const record = state.smartSelections.find(r => r.id === state.smartSelectionId);
    if (!record || smartSelectionIsInvalidV103(record, owner, set)) return toast('选型记录已失效', 'warning');
    const result = record.results.find(r => r.modelId === modelId);
    if (!result || result.status !== 'success') return toast('该模型无法创建实验', 'warning');

    const existingId = record.experimentMap?.[modelId];
    if (existingId && state.experiments[existingId]) {
      state.experimentId = existingId;
      return go('experiment');
    }

    const eid = uid('e');
    const count = set.experiments.map(id => state.experiments[id]).filter(item => item?.type === modelId).length + 1;
    const expName = `${modelName(modelId)} ${String(count).padStart(2, '0')}`;

    const standardize = ['logistic', 'knn', 'linear'].includes(modelId);
    const experiment = {
      id: eid, projectId: owner.id, datasetId: set.id, name: expName, type: modelId,
      standardize, tuning: 'none', parameters: smartSelectionFixedParamsV103(modelId, owner, set), status: 'draft', updatedAt: now(),
      results: [], selected: null, smartSelectionId: record.id
    };
    if (modelId.startsWith('forecast_')) {
      ensureForecastParametersV1022(experiment, set);
      Object.assign(experiment.parameters, smartSelectionFixedParamsV103(modelId, owner, set));
    }
    state.experiments[eid] = experiment;
    set.experiments = [...(set.experiments || []), eid];
    record.experimentMap = { ...(record.experimentMap || {}), [modelId]: eid };
    state.experimentId = eid;
    save(); go('experiment');
    return;
  }

  if (name.startsWith('delete-smart-selection-v103:')) {
    const id = name.split(':')[1];
    if (!confirm('确定删除这条智能选型记录？')) return;
    state.smartSelections = state.smartSelections.filter(r => r.id !== id);
    if (state.smartSelectionId === id) state.smartSelectionId = '';
    save(); render();
    return;
  }

  return actionBeforeV103(name);
};

// 智能选型 bind 处理器
const bindBeforeV103 = bind;
bind = function () {
  bindBeforeV103();

  // 渲染运行弹窗或最小化卡片，并立即绑定内部动作按钮
  if (state.smartSelectionRun) {
    if (!state.smartSelectionRun.minimized) {
      const dialog = smartSelectionRunDialogV103();
      if (dialog && !app.querySelector('[data-smart-selection-backdrop-v103]')) {
        app.insertAdjacentHTML('beforeend', dialog);
        app.querySelector('[data-smart-selection-backdrop-v103]')?.querySelectorAll('[data-action]').forEach(element => {
          element.onclick = event => { event.stopPropagation(); action(element.dataset.action); };
        });
      }
    } else {
      const mini = smartSelectionRunMiniV1031();
      if (mini && !app.querySelector('[data-smart-selection-mini-v1031]')) {
        app.insertAdjacentHTML('beforeend', mini);
        app.querySelector('[data-smart-selection-mini-v1031]')?.querySelectorAll('[data-action]').forEach(element => {
          element.onclick = event => { event.stopPropagation(); action(element.dataset.action); };
        });
      }
    }
  ...
  // 模型池勾选
  app.querySelectorAll('[data-smart-selection-toggle-v103]').forEach(input => {
    input.addEventListener('change', event => {
      const modelId = event.target.dataset.smartSelectionToggleV103;
      const owner = project(), set = dataset(), taskKey = smartSelectionTaskKeyV103(owner);
      const isBaseline = taskKey === 'forecasting' && modelId === 'forecast_baseline';
      if (isBaseline) { event.target.checked = true; save(); render(); return; }
      const max = SMART_SELECTION_MAX_POOL_V103[taskKey];
      if (event.target.checked) {
        if (state.smartSelectionPool.length >= max) { event.target.checked = false; return toast(`最多选择 ${max} 个模型`, 'warning'); }
        if (!state.smartSelectionPool.includes(modelId)) state.smartSelectionPool.push(modelId);
      } else {
        state.smartSelectionPool = state.smartSelectionPool.filter(id => id !== modelId);
      }
      save(); render();
    });
  });
  ...
  // 卡片点击切换
  app.querySelectorAll('[data-smart-selection-model-v103]').forEach(card => {
    card.addEventListener('click', event => {
      if (event.target.closest('input') || event.target.closest('button')) return;
      const input = card.querySelector('[data-smart-selection-toggle-v103]');
...
// V10.3.2 一次性迁移：将名称中含"智能选型"的实验改为普通命名
Object.values(state.experiments).forEach(exp => {
  if (!exp.name.includes('智能选型')) return;
  const set = state.datasets[exp.datasetId];
  if (!set) return;
  const baseName = modelName(exp.type);
  let index = set.experiments.map(id => state.experiments[id]).filter(item => item && item.type === exp.type && !item.name.includes('智能选型')).length + 1;
  let newName = `${baseName} ${String(index).padStart(2, '0')}`;
  while (Object.values(state.experiments).some(item => item.name === newName)) { index += 1; newName = `${baseName} ${String(index).padStart(2, '0')}`; }
  exp.name = newName;
});
