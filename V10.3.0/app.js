...
forecastBacktestModalV10 = function (result) {
  ...
  modal(`<h2>滚动回测明细</h2><p>${esc(experiment().name)} · ${esc(result.params)}</p><div class="forecast-backtest-modal-v102">${rounds}</div><div class="modal-actions">${button('关闭', 'close-modal', 'primary')}</div>`); bindDynamicModalV909();
};
...
function smartSelectionCardV103(owner = project(), set = dataset()) {
  ...
  return `<article class="model-card smart-selection-card-v103 ${ready.ok ? '' : 'disabled'}" data-action="${ready.ok ? 'open-smart-selection-v103' : ''}"><span class="smart-selection-icon">✦</span><div class="smart-selection-tag-v103">推荐入口</div><h3>智能选型</h3><p>使用多个模型的固定方案快速比较并推荐后续重点模型。</p>${ready.ok ? button('开始智能选型', 'open-smart-selection-v103', 'primary') : `<span class="status blocked">${esc(ready.message)}</span>`}</article>`;
}
...
function smartSelectionConfigPageV103() {
  ...
  return shell(`${steps('experiments')}${pageHead('智能选型', `${esc(set.name)} · ${taskKey === 'classification' ? '二分类' : taskKey === 'multiclass' ? '多分类' : taskKey === 'regression' ? '回归' : '时序预测'}`)}<div class="smart-selection-config-v103"><div class="panel-head"><div><h2>模型池配置</h2><p>本次将运行 ${pool.length} 个固定参数方案，最少 ${min} 个，最多 ${max} 个。</p></div><span class="status good">已选择 ${pool.length} / ${max}</span></div><div class="smart-selection-pool-v103">${cards}</div><details class="advanced"><summary>平台基准配置</summary><div class="smart-selection-baseline-v103"><p>所有模型使用相同的目标列、入模特征和训练/验证划分；参数不可编辑。</p><div>${pool.map(id => `<span><b>${esc(smartSelectionModelCatalogV103(taskKey).find(e => e.id === id)?.name || id)}</b>${esc(JSON.stringify(smartSelectionFixedParamsV103(id, owner, set)))}</span>`).join('')}</div></div></details></div><div class="flow-footer"><div>${button('返回模型训练', 'go-experiments')}</div><div>${button('开始智能选型', 'start-smart-selection-v103', canStart ? 'primary' : '')}</div></div>`);
}
...
function smartSelectionResultPageV103() {
  ...
  if (invalid) {
    ...
    body = `<div class="smart-selection-recommend-v103"><div class="section-title"><h2>推荐模型：${esc(rec?.name || record.recommendedModelId)}</h2><span>模型池排名第 1 · 主指标 ${pm.key.toUpperCase()} ${pm.value}</span></div><div class="smart-selection-recommend-actions-v103">${button('使用推荐模型创建实验', `create-from-smart-selection-v103:${record.recommendedModelId}`, 'primary')}${button('选择其他模型', 'toggle-smart-selection-results-v103')}</div></div>`;
  } else {
...
function smartSelectionRunDialogV103() {
  ...
  if (!run) return '';
  return `<div class="modal-backdrop" data-smart-selection-backdrop-v103><div class="training-dialog"><div class="training-header"><div><h3>正在进行智能选型</h3><p>使用平台基准参数</p></div><div class="training-controls"><button data-action="minimize-smart-selection-v103">_</button><button data-action="cancel-smart-selection-v103">✕</button></div></div><div class="training-body"><div class="progress-bar"><div style="width:${(run.index / run.total) * 100}%"></div></div><div class="training-status"><b>正在运行 ${run.index} / ${run.total}：${esc(smartSelectionModelCatalogV103(run.taskKey).find(e => e.id === run.current)?.name || run.current)}</b></div></div></div></div>`;
}
...
action = function (name) {
  ...
  if (name.startsWith('create-from-smart-selection-v103:')) {
    ...
    const eid = uid('e');
    const modelNameDisplay = smartSelectionModelCatalogV103(record.taskKey).find(e => e.id === modelId)?.name || modelName(modelId);
    const baseName = `${modelNameDisplay} 智能选型`;
    let expName = baseName;
    let suffix = 1;
    while (Object.values(state.experiments).some(e => e.name === expName)) { expName = `${baseName} ${suffix += 1}`; }
...
bind = function () {
  ...
  // 渲染运行弹窗
  if (state.smartSelectionRun && !state.smartSelectionRun.minimized) {
